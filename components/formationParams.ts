...
// R6(#131): 2선 커버 x = ~~라인 쪽 고정~~ → **맡는 선수의 존(스위칭 레인) 쪽으로 스냅**. 히터 뒤 중앙(attackX)에서
//   그 선수 존 x까지의 이 비율만큼 이동 — 커버→전환 복귀가 코트 횡단(X자)이 안 되게(한 사이클 설계).
//   R5 오답 교정: 실측 한 장면(차니 존≈라인 쪽 우연)을 "라인 고정"으로 오일반화했음 — 2선 기능(깊은 리바운드 백업)은 라인 끝일 필요 없다.
export const COVER_SECOND_SNAP = 0.55;    // 0=히터 뒤 중앙 · 1=선수 존 — 중앙↔존 사이(복귀 횡단 최소).
