...
export function receiveFormation(side: Side, lu: Lineup, rot: number, W: number, H: number): Record<number, Px> {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (i === setterIdx) {
      ...
      // 전위 패서(주로 OH)는 **3m 라인 부근(0.665≈3m)** 에서 받는다 — 후위 패서(리베로·백OH, 0.80~0.82)와
      // 함께 3인 **W**(전위 얕고·후위 깊은). 전위 패서가 후위밴드(≥0.72)로 내려가 "전위가 후위 라인까지" 가던 걸
      // 교정(2026-06-21 사용자 보고·측정 84/252→0). 깊은 중앙 커버는 리베로(zone6). 비패서(공격수)는 네트 앞 대기.
      // 오버랩: 전위(0.665) < 후위 같은 열(0.80) → 전위가 네트에 더 가까움(합법, 룰 Q).
      yf = recv.includes(i) ? 0.665 + jit(seed + 2, 0.012) : 0.575 + jit(seed + 2, 0.02);
    } else {
