...
export function BootGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!userId) return;
    // 지갑 리싱크(BACKEND §13.12 P0-3) — 로그인·포그라운드 복귀마다 서버 잔액·패스 상태·우편 카운트(unread/unclaimed)로 캐시 정합.
    //   다이아 패스 일일 지급은 서버 스케줄러가 우편함으로 발송하므로(DIAMOND_PASS §2.3 재개정) 클라 자동 수령·리셋경계 타이머는 폐기.
    //   수령은 유저가 우편함에서(MAILBOX §5.1). 빨간 점(unreadMailCount)이 syncWallet 응답에 실려와 도착을 알린다.
