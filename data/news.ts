...
const POOLS: Record<string, { open: string[]; close: string[] }> = {
  generic: {
    open: ['소식이 전해졌다.', '한 장면이 기록됐다.', '리그가 주목한 순간이다.'],
    close: ['시즌의 한 페이지로 기록됐다.', '기록은 오래 회자될 것이다.', '세월이 쌓여 만들어진 장면이다.'],
  },
  champion: {
    open: ['길고 치열했던 정규리그와 포스트시즌 끝에, 마지막 승자가 결정됐다.', '정규리그부터 챔피언결정전까지, 모든 길의 끝에 한 팀이 우뚝 섰다.', '챔피언의 이름이 마침내 새겨졌다.', '긴 시즌의 마지막 승자가 결정됐다.', '우승 트로피의 주인이 가려졌다.', '봄배구의 마지막 주인공이 정해졌다.'],
    close: ['우승의 여운도 잠시, 다음 시즌 준비가 시작됐다.', '챔피언의 이름이 리그 역사에 새겨졌다.', '팬들은 오래도록 이 시즌을 기억할 것이다.', '한 시즌을 관통한 저력이 정상에서 증명됐다.', '우승의 무게는 다음 시즌 도전으로 이어질 전망이다.'],
  },
  award: {
    open: ['한 시즌의 활약이 수상으로 이어졌다.', '코트 위에서 쌓아 올린 생산이 만든 결과다.', '시즌 내내 이어진 꾸준함이 결실을 맺었다.', '기록이 증명한 한 시즌이었다.'],
    close: ['그해를 대표하는 이름으로 기록됐다.', '다음 시즌을 향한 기대도 함께 커졌다.', '한 시즌을 대표하는 장면으로 자리매김했다.', '수상 이력에 훈장 하나가 더해졌다.'],
  },
  king: {
    open: ['부문 1위는 한 시즌의 꾸준함이 만든다.', '시즌 내내 선두 자리를 지켜낸 결과다.', '기록은 끝내 거짓말을 하지 않았다.', '한 부문에서만큼은 누구도 앞서지 못했다.'],
    close: ['타이틀은 통산 기록에 더해졌다.', '한 부문에서 가장 앞선 한 해였다.', '다음 시즌에도 정상을 지킬지 주목된다.', '부문 최고의 자리를 기록으로 증명했다.'],
  },
  best7: {
    open: ['시즌을 빛낸 최고의 선수들이 호명됐다.', '포지션별 최고가 한자리에 모였다.', '한 시즌의 베스트 라인업이 가려졌다.', '한 시즌을 수놓은 일곱 이름이 발표됐다.'],
    close: ['선정 자체가 한 시즌의 훈장이다.', '이름들은 시즌 아카이브에 기록됐다.', '기록과 활약이 함께 만든 영예다.', '한 시즌 최고의 무대를 채운 얼굴들이다.'],
  },
  streakWin: {
    open: ['거침없는 질주였다.', '아무도 막아서지 못한 흐름이었다.', '승리가 또 다른 승리를 불렀다.', '상승세에 제동이 걸리지 않았다.'],
    close: ['연승의 기억은 시즌의 자산으로 남았다.', '상승세는 순위 싸움의 든든한 발판이 됐다.', '코트 위 자신감이 결과로 증명됐다.', '분위기를 탄 팀은 좀처럼 멈추지 않았다.'],
  },
  streakLose: {
  ...
  milestone: {
    ...
    close: ['기록은 리그 역사 속에 남게 됐다.', '다음 고지가 벌써 눈앞에 다가왔다.', '통산의 발자취에 또 한 칸이 더해졌다.', '커리어에 굵은 이정표가 하나 세워졌다.'],
  },
  hof: {
    open: ['한 시대가 막을 내렸다.', '오랜 커리어가 끝내 명예로 마무리됐다.', '코트를 떠나는 이름이 전당에 새겨졌다.', '긴 여정 끝에 전당에 이름을 올렸다.'],
    close: ['통산 기록은 리그 역사에 길이 남게 됐다.', '그 발자취는 후배들의 이정표가 된다.', '은퇴는 끝이 아니라 또 다른 기록의 시작이다.', '코트를 떠나도 이름은 전당에 걸린다.'],
  },
  retire: {
    open: ['긴 여정의 마침표가 찍혔다.', '한 베테랑이 코트와 작별한다.', '오랜 시간 코트를 지킨 이름이 떠난다.', '한 시대를 함께한 선수가 유니폼을 벗는다.'],
    close: ['수고했다는 말로는 부족한 세월이었다.', '팬들은 그 마지막 시즌을 오래 기억할 것이다.', '코트를 떠나도 쌓아온 기록은 남는다.', '이제 새로운 세대가 그 자리를 이어받게 됐다.'],
  },
  injury: {
    open: ['반갑지 않은 소식이다.', '팀에 전력 공백이 생겼다.', '예상치 못한 부상 변수가 발생했다.'],
  ...
  triple: {
    open: ['한 경기에서 세 부문을 동시에 채웠다.', '공격·블로킹·서브에서 고르게 활약한 한 경기였다.', '좀처럼 보기 힘든 대기록이 나왔다.'],
    close: ['트리플 크라운은 공·수·서브를 두루 갖췄다는 증표다.', '공·수·서브 전 영역에서 존재감을 보인 한 경기였다.', '시즌을 통틀어 손에 꼽을 장면으로 기록됐다.'],
  },
  debut: {
  ...
  transfer: {
    ...
    close: ['새 팀에서의 활약이 기대된다.', '이적은 두 팀의 전력 지형을 흔들 전망이다.', '한 시즌의 새 출발이다.', '익숙한 코트를 떠나 새 도전을 시작한다.'],
  },
  ...
  clinchTitle: { // 정규리그 1위(챔프전 직행) 확정 — 최고 영예, 골드 톤(§3.1)
    open: ['정규리그 1위의 주인이 가려졌다.', '길고 치열했던 선두 다툼이 끝났다.', '순위 레이스의 정점에 한 팀이 우뚝 섰다.'],
    close: ['챔피언결정전 직행 티켓을 손에 넣었다.', '정규리그 1위의 프리미엄을 안고 봄배구를 기다린다.', '남은 건 우승을 향한 마지막 한 걸음이다.'],
  },
...
// ── 부제(teal 한 줄) 풀 + 선택기 — 상세 화면(app/news/[id].tsx)의 표시 로직을 순수 계층으로 끌어와 테스트 가능하게(가드 대조).
//    kind별 톤(§11.3): 우승=축하 / 부상·징계=담담 / 이적·외인·모기업=시장 / 데뷔=기대 / 은퇴·HOF=회고 / 수상·기록·순위=객관.
//    변형은 뉴스키 해시(vp — 결정론). 억지 사실 금지 — 톤 프레이밍만(실사실은 카드가 전달). RN 무의존 순수 데이터.
export const SUBTITLE_BY_KIND: Record<NewsItem['kind'], string[]> = {
...
// 프리시즌 예상순위 기사(§3.7·§10b) 전용 부제·리드 풀 — kind=standing을 재사용하지만 톤은 **시즌 결산이 아니라 개막 전망**이라야 한다.
//   (버그: standing 재사용이라 부제가 SUBTITLE_BY_KIND.standing('다음 시즌의 출발선이 여기서 정해졌다' = 결산 톤)에서 뽑혔다.)
//   두 풀은 문구가 서로 겹치지 않는다(전망 vs 결산) — 가드 _dv_preseasonnews가 disjoint를 상시 검증.
export const PRESEASON_SUBTITLES = [
  '전문가들의 시즌 전망이 공개됐다',
  '개막을 앞두고 예상 순위가 발표됐다',
  '우승 후보와 다크호스가 윤곽을 드러냈다',
  '출사표는 던져졌다, 이제 검증은 코트에서 시작된다',
  '예상은 예상일 뿐, 결과는 시즌이 말해줄 것이다',
  '각 팀의 시즌 전망이 팬들의 관심을 모으고 있다',
];
/** 프리시즌 예상순위 기사 리드(본문 폴백) — 전망 톤. 실전에선 body(preseason 풀 조립본)가 항상 있어 폴백은 드묾. */
export const PRESEASON_LEAD = '개막을 앞두고 전문가들이 매긴 예상 순위가 공개됐다. 예상은 예상일 뿐, 답은 코트에서 가려진다.';

/** 프리시즌 예상순위 기사 식별 — kind=standing 재사용이지만 ref('preseason:시즌')로 시즌 결산 순위 기사와 구분(§10b). */
export const isPreseasonRankNews = (n: NewsItem): boolean =>
  n.kind === 'standing' && (n.ref ?? '').startsWith('preseason:');

/** 상세 화면 부제 1줄 — 프리시즌 예상순위는 전용 풀(전망 톤), 그 외는 kind별 풀. vp=상세의 dpick과 동일 해시(결정론 일치). */
export function newsSubtitle(n: NewsItem, key: string = newsKey(n)): string {
  const pool = isPreseasonRankNews(n) ? PRESEASON_SUBTITLES : SUBTITLE_BY_KIND[n.kind];
  return vp(pool, key, 0);
}
