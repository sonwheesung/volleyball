  ...
  return (
    <Screen title="게임 가이드">
      ...
      <Card>
        ...
        <Accordion icon="airplane-outline" title="전지훈련 — 어린 선수일수록 이득이에요">
          <P>전지훈련은 오프시즌에만 할 수 있어요. 선수 한 명을 해외 캠프로 보내는 거예요 — 다이아 300이 들어요.</P>
          <P>코스(공격·수비·블로킹·세터·서브) 하나를 고르면, 그와 관련된 3가지 능력치가 바로 +2 오르고, 그 능력치들의 성장 한계(잠재력)가 +7 올라가요(최고 99까지). 선수 한 명은 오프시즌에 딱 한 번만 보낼 수 있고, 효과는 영원히 남아요(되돌리거나 환불할 수 없어요).</P>
          <P>★ 여기가 가장 중요해요. 진짜 가치는 바로 오르는 +2가 아니라 '성장 한계 +7'이에요. 능력치는 앞으로 경기와 훈련을 하면서 그 한계까지 천천히 자라거든요. 그래서 앞으로 뛸 시즌이 많이 남은 어린 선수일수록 이득이 커요 — 열어준 한계까지 쭉 자랄 시간이 있으니까요. 반대로 나이 많은 선수는 더 자랄 여지가 적어서 효과가 작아요. 돈을 낸 만큼 지금 당장 세지는 게 아니라, 앞으로 더 클 수 있는 '한계'를 열어주는 거라고 생각하면 돼요.</P>
        </Accordion>
      </Card>
