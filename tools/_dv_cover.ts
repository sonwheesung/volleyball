...
function coverPin(): boolean {
  ...
  // R5 n=2 층 핀: 전면 n=2가 근접(y≤0.58)+2선(y≥0.68) 두 층인가(구 로직=둘 다 근접 y0.56).
  let layerOK = true;
  for (const [side, axf] of [['away', 0.800], ['home', 0.200]] as const) {
    const s2 = coverSpots(side, axf * W, 2, W, H, false);
    const nY = fy(s2[0], side), sY = fy(s2[1], side); // [근접, 2선]
    const ok2 = nY <= 0.58 && sY >= 0.68 && sY - nY >= 0.10; // 두 층 분리
    if (!ok2) layerOK = false;
    msgs.push(`${side} x${axf} n=2: 근접y=${nY.toFixed(3)} 2선y=${sY.toFixed(3)} (층분리 ${ok2 ? '✅' : '✗'})`);
  }
  // A/B: 구 값(근접 y0.645·대칭 ±34px=0.094)이면 ①근접(0.645>0.58 위반) ②대칭(|0.094−0.094|=0<0.005 위반) / n=2 구 로직=둘 다 근접(2선층 없음) → 핀 깨짐.
  const oldNearY = 0.645, oldSym = 34 / W, oldN2SecondY = 0.56; // 구 n=2는 둘 다 y0.56(2선 없음)
  const abBreaks = oldNearY > 0.58 && Math.abs(oldSym - oldSym) < 0.005 && oldN2SecondY < 0.68;
  for (const m of msgs) log('   · ' + m);
  log(`   [A/B] 구 값(근접 y0.645·대칭 ±34px·n=2 둘 다 근접)이면 근접+비대칭+층 핀 위반 → ${abBreaks ? '✅ 오라클 유효' : '⚠'}`);
  return ok && layerOK && abBreaks;
}
...
// ── R5(#131): 커버 n=2 층 배정 — 근접=리베로(수비 전문), 2선=후위 공격수(파이프 준비) ──
// 구 로직은 n=2 둘 다 근접(층 없음) → 리베로·후위공격수 모두 네트 밴드에 몰림. 신 로직은 근접 1(리베로)+2선 1(후위 공격수).
const n2front = group((r) => r.cover.length === 2 && r.atk !== 'back');
const n2LibInCover = n2front.filter((r) => r.libero !== undefined && r.cover.includes(r.libero));
const n2LibNotNear = n2LibInCover.filter((r) => !(r.near.length === 1 && r.near[0] === r.libero)).length; // 위반: 리베로가 근접 단독 아님(기대 0)
...
log('');
log('── R4 커버 반원 좌표 핀(전면 공격 타이트닝) + R5 n=2 층 핀 ──');
const covPinOK = coverPin();

log('');
log('── R5 커버 n=2 층 배정 (근접=리베로 · 2선=후위 공격수) ──');
log(`n=2 전면 커버 ${n2front.length}건 · 리베로 포함 ${n2LibInCover.length}건 중 리베로≠근접단독(위반): ${n2LibNotNear} (${pct(n2LibNotNear, n2LibInCover.length)}%) — 기대 0`);
log(`   2선 슬롯이 후위(공격수) 배정: ${n2SecondBackAtk}/${n2front.length} (${pct(n2SecondBackAtk, n2front.length)}%)`);
