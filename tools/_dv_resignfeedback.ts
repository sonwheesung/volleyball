...
// ── ⑤ 오퍼 레버(FA §2.5c-격상): 조합 동일성·bit-동일·단조 A/B·전원보장 no-op·파기 세탁 봉인 ──
{
  ...
  const reconstruct = (p: Player, override: Contract): number => {
    ...
    const refuseBias = 0;
    const accum = topic === 'minutes' ? sustainedBenchRefuse(playRatio, weight, coachLeadershipOf(my, DAY)) : 0; // 리더십 완화(§9.6-D)
  ...
  for (const p of players) {
    ...
    const stdAccum = stdTopic === 'minutes' ? sustainedBenchRefuse(stdPR, stdW, coachLeadershipOf(my, DAY)) : 0; // 리더십 완화(§9.6-D)
