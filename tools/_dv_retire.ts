...
import { importAgesOut } from '../engine/foreign';
...
// ── ⑥ 수입선수 정년(FOREIGN_SYSTEM §1.6) — importAgesOut 게이트 (순수 + A/B) ──
console.log('── ⑥ 수입선수 정년: importAgesOut(다음시즌 40+)=true · 39세 마지막 시즌 ──');
{
  const fgn = (age: number): Player => ({ ...mk(`f${age}`, age, 'OP', 88), isForeign: true });
  const asn = (age: number): Player => ({ ...mk(`a${age}`, age, 'OH', 82), isForeign: true, isAsianQuota: true });
  ok(importAgesOut(fgn(40)) === true, '외인 다음시즌 40세 → 정년(리그 이탈)');
  ok(importAgesOut(fgn(44)) === true, '외인 44세 → 정년 (실측 잔류 케이스 차단)');
  ok(importAgesOut(fgn(39)) === false, '외인 39세 → 마지막 시즌 가능(잔류 허용)');
  ok(importAgesOut(asn(40)) === true, '아시아쿼터 40세 → 정년');
  ok(importAgesOut(asn(39)) === false, '아시아쿼터 39세 → 마지막 시즌 가능');
  // A/B: 정년 제거 모사(나이 무관 잔류) → 40세도 잔류 = 불변식 위반
  const mutantGate = (_p: Player) => false; // "정년 없음" 모사(구 동작)
  ok(mutantGate(fgn(40)) !== importAgesOut(fgn(40)), '[A/B] 정년 제거 모사 → 40세 잔류(불변식 위반 — 가드 이빨 존재)');
}
...
(async () => {
  ...
  let over40Import = 0, maxImportAge = 0; // ⑥ 수입선수(외인·아시아쿼터) 정년(FOREIGN_SYSTEM §1.6)
  ...
  for (let s = 0; s < SEASONS; s++) {
    ...
    if (s >= 15) {
      for (const p of currentBasePlayers()) {
        if (p.isForeign) { // 수입선수도 40+ 현역 0(정년 적용) — isForeign 제외 사각 확장
          maxImportAge = Math.max(maxImportAge, p.age);
          if (p.age >= RETIRE_AGE) over40Import++;
          continue;
        }
  ...
  ok(over40 === 0, `40세+ 국내 현역 0 (관측 최고령 ${maxAge}세)`);
  ok(over40Import === 0, `40세+ 수입선수(외인·아시아쿼터) 현역 0 (관측 최고령 ${maxImportAge}세) — 정년 적용`);
