...
//
// ⚠ **범위 경고(2026-08-08 사각 규명)**: 이 가드는 **함수**(achTotals가 옳은 값을 내나)만 봉인한다 — **배선**
//   (어느 화면이 그 함수를 실제로 쓰나)은 **`tools/_gt_achdot.ts`** 가 본다. 여기 (A) 블록은 raw careerTotals 사용을
//   "버그 재현 PASS"로 **박제**하고 있어서, 나중에 추가된 마이페이지 탭 빨간 점이 같은 raw 입력을 새로 써도
//   이 가드는 계속 초록이었다(운영 실피해: 신규 유저가 첫 시즌 내내 다이아 미수령, 6건 60💎 지연).
//   → 새 호출부가 생기면 반드시 `_gt_achdot`(C7 배선 정적강제)에 등록할 것. 방법론 교훈은 docs/TEST_METHODOLOGY.md §4.
