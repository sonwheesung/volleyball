// 성장 리포트 — 전지훈련 구매분 차감 검증 (TRAINING §성장리포트 재정정, 2026-07-11).
//   스펙: 누적(입단 이후)은 이제 **선수 상세**가 `playerCareerGrowth(id, atDay, campLog)`로 계산(모달은 구간 전용).
//        그 누적 표기에서 전지훈련(다이아 캠프) 구매분(cur)을 스탯별로 차감 → 순수(유기적) 성장만.
...
//   추가: 모달용 `growthReport`(구간 diff)는 캠프가 양끝에 동일 반영되어 delta 불변임을 재확인.
//   npx tsx tools/_dv_growthcamp.ts
import { resetLeagueBase, LEAGUE, currentRosters, getPlayer, commitPlayerBase } from '../data/league';
import { growthReport, playerCareerGrowth, type CampLogLike } from '../data/growthReport';
...
const exclCareer = playerCareerGrowth(id, TO, campLog);
const exclSeg = findSeg(growthReport(team, FROM, TO));
ok(!!exclCareer, '대상 선수 career 셀렉터 여전히 존재');
ok(!!exclSeg, '대상 선수 여전히 구간 리포트에 존재(구간 변화 유지)');
ok(JSON.stringify(exclCareer!.statDeltas) === baseCareerSig, 'career 누적(캠프 제외) == 캠프 전 baseline (A/B 동일)');
ok(JSON.stringify(exclSeg!.deltas) === baseDeltasSig, '구간 deltas도 캠프 무영향(양끝 동일 반영 — 배경 사실 재확인)');

// ── ③ 차감 OFF(campLog=[]): 같은 캠프 상태인데 career가 부풀어야(다르게) → 차감이 실제로 작동함을 증명 ──
console.log('\n── ③ 차감 OFF(민감도) — 같은 상태에서 career가 부풀어 baseline과 달라야 ──');
const rawCareer = playerCareerGrowth(id, TO, []);
ok(!!rawCareer, '대상 선수 career(차감 OFF) 셀렉터 존재');
ok(JSON.stringify(rawCareer!.statDeltas) !== baseCareerSig, 'career 누적(차감 OFF) != baseline (구매분이 부풀림 — 가드가 잡는 대상)');
// 캠프 3스탯 각각 정확히 +CAMP_CUR_GAIN 부풀었는지 확인
let inflateOk = true;
for (const s of CAMP_COURSES[course].stats) {
  const label = ([['jump','점프력'],['agility','민첩성'],['staminaMax','체력'],['staminaRegen','체력재생'],['reaction','반응속도'],['positioning','위치선정'],['focus','집중력'],['consistency','기복'],['vq','VQ'],['skSpike','공격기술'],['skBlock','블로킹기술'],['skDig','디그기술'],['skReceive','리시브기술'],['skSet','세팅기술'],['skServe','서브기술']] as [string,string][]).find(([k]) => k === s)![1];
  const rawD = rawCareer!.statDeltas.find((d) => d.label === label)?.delta ?? 0;
  const baseD = baseCareer!.statDeltas.find((d) => d.label === label)?.delta ?? 0;
...
// ── ④ 결정론 ──
console.log('\n── ④ 결정론 ──');
ok(JSON.stringify(playerCareerGrowth(id, TO, campLog)) === JSON.stringify(exclCareer), 'career 셀렉터 반복 호출 동일(결정론)');
