...
import { teamOverall, teamOverallRaw, displayOvr } from '../engine/overall';
...
const rows = st.map((s) => ({
  ...
  disp: displayOvr(teamOverallRaw(getEvolvedTeamPlayers(s.teamId, 164))),
  wins: s.wins,
}));
...
for (const r of rows) console.log(`  ${r.name.padEnd(6)} raw ${r.ovr} → 표시 ${r.disp} · ${String(r.wins).padStart(2)}승`);
