...
export interface NewsItem {
  season: number;                                            // 0-based
  kind: 'champion' | 'award' | 'milestone' | 'hof' | 'injury' | 'scandal' | 'owner' | 'streak' | 'standing' | 'match' | 'debut' | 'transfer' | 'release' | 'retire' | 'sponsor' | 'offseason' | 'draft' | 'foreign' | 'playoff' | 'clinch' | 'coach';
  headline: string;
  big: boolean;                                              // 헤드라인급
...
export interface Transfer {
  ...
  reason?: 'refused' | 'notOffered' | 'capSqueezed' | 'rosterExcess'; // 재계약 불발/정리 사유(FA §2.5c-격상, release에만) — 캡압박/뿌리침/미제안/정원초과정리. 옵셔널=구세이브 호환·마이그레이션 불요
}
