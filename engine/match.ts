...
export function simulateMatch(
  ...
  while (homeSets < SETS_TO_WIN && awaySets < SETS_TO_WIN) {
    ...
    const subIn = (side: Side, slot: number, player: Player | null, kind: TacticalSubKind): void => {
      ...
      // ── FIVB 15.6.1 합법 재진입 판정(2026-07-30, #124 F2 정정) ──
      //   선발 A가 B로 교체된 슬롯 S에서, A는 **자기 자리 S로·자기 교체선수 B와 교대로만·세트당 1회** 재진입할 수 있다(FIVB 15.6.1).
      //   ★ 결정론 안전장치: `kind === 'manual'`(유저 개입)일 때만 허용한다. AI 자동교체(rest/pinch/block/def)는 이 경로에
      //     절대 진입 못 함 → 나간 선발을 자기 슬롯에 재투입하는 일이 AI 경기엔 없어 **무개입 경기 바이트 동일**(AI는 kind!=='manual'
      //     이라 isReentry 항상 false → 아래 모든 분기가 구 로직과 동일값). 유저 개입 subIn만 재진입을 연다.
      //   조건: (a) 유저 개입 (b) player가 이번 세트 아웃된 선발(usedStarterOut) (c) 그 슬롯에 활성 교체가 있고(active) 그 원선발이
      //     바로 player 자신(자기 자리) (d) 그 교체가 비복원형(manual/rest — pinch/block/def는 복원 루프가 자동 되돌리니 재진입 대상 아님).
      const active = activeSubs[side].get(slot);
      const isReentry = kind === 'manual' && usedStarterOut[side].has(player.id)
        && !!active && active.orig.id === player.id && !isRestorable(active.kind);
      ...
      //   재진입도 enter 1건이라 예산 −1(A→B, B→A = 각 1회 소비, 총 2/6). isRestorable(manual)=false → reserve=pendingRestores+1.
      ...
      // 재진입은 그 슬롯이 활성(B 점유)이어도 통과시킨다(B를 A로 교대). 일반 IN은 활성 슬롯이면 거부(구 동작).
      if ((activeSubs[side].has(slot) && !isReentry) || subBudget[side] < reserve) return;
      // 이미 코트에 있으면 불가 — 같은 벤치 스페셜리스트가 두 슬롯에 중복 투입되는 것 방지(재진입 A는 코트에 없음 → 통과)
      ...
      if (!isReentry && usedSubIn[side].has(player.id)) return; // FIVB: 교체선수는 세트당 1회만 진입(재진입 금지) — 재진입 A는 usedSubIn에 없지만 방어적으로 감쌈
      if (usedStarterOut[side].has(player.id) && !isReentry) return; // 불법 재진입 차단(구 F2 line): 나간 선발을 타슬롯·kind≠manual 등으로 넣는 시도 거부. 합법 재진입(위 isReentry)만 통과. EC-SUB-02.
      const outP = st.six[slot];                                // 재진입 시 outP = B(자기 교체선수)
      if (usedStarterOut[side].has(outP.id)) return; // B는 선발아웃 아님→통과 / 일반: 선발은 세트당 1왕복만(돌아온 선발 재이탈 금지)
      // ── 회계 분기 ──
      if (isReentry) {
        // A 자기 자리 복귀 — 활성 교체 해제. A는 usedStarterOut 유지(재이탈 금지), B는 usedSubIn 유지(재진입 금지).
        //   A를 usedSubIn에·B를 usedStarterOut에 추가하지 않는다(선발 A는 교체선수가 아니고, B는 선발이 아님).
