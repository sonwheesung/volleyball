...
/** 모기업 지원금 베이스(만원) — 팀별 차등 24.3~32.3억(모기업 크기가 다르다). 시드 결정론.
 *  2026-06-14 상향(20~28→22~30): 리그 성장으로 연봉↑ → 보전 62% → 베이스 +2억.
 *  2026-06-27 재상향(22~30→25~33, +3억): payroll 인플레 누적 net −2.87억(만성적자) → +3억으로 회복(옛 체력 기준 잔고 8억·건강).
 *  2026-06-28 재조율(25~33→24.3~32.3, −0.7억): **체력 튜닝(ENGINE_VERSION 2)이 경기결과→내 팀 성적↑→수입↑로
 *  잔고 8억→18.8억 과잉**(A/B 입증: 옛 체력 8.0억 vs v2 18.8억, 차이=체력뿐) → "캡OK·현금부족" 드라마 0회.
 *  base 250000→243000으로 v2 기준 옛 healthy 패턴 복원: simFinance 120 잔고 9.1억·보전 8%·좌절 28/105·✅ 건강(FINANCE 2.0 Stage1). */
export function sponsorBase(teamId: string): number {
  return 243000 + Math.floor(createRng(strSeed(`sponsor:${teamId}`)).next() * 80000);
}
