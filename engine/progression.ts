 ...
 */
export function evolvePlayer(base: Player, focus: TrainingFocus, days: number, effects: StaffEffects = NO_EFFECTS): Player {
  if (days <= 0) return base;
  const rng = createRng(playerSeed(base.id));
  let p = base;
  for (let d = 0; d < days; d++) {
    p = applyTrainingDay(p, focus, rng, effects.trainBoost, effects.potBonus);
    p = applyAgingDay(p, rng, effects.ageSlow);
  }
  return p;
}
