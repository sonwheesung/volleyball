...
import type { Player, TrainingFocus, TrainingId } from '../types';
 ...
 */
export function evolvePlayer(base: Player, focus: TrainingFocus, days: number, boosts?: Partial<Record<TrainingId, number>>): Player {
  if (days <= 0) return base;
  const rng = createRng(playerSeed(base.id));
  let p = base;
  for (let d = 0; d < days; d++) {
    p = applyTrainingDay(p, focus, rng, boosts);
