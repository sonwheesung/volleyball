...
> **왜 여기 적나**: 사용자 지시 = **"모든 작업은 문서부터 진행 후 개발 착수"**(2026-06-24). 컴팩트로
> 직전 맥락이 날아가면 "코드부터 고치고 문서는 나중에"로 흐르기 쉽다. 이 프로젝트는 단일 기준
> 문서가 정본(DOC_DISCIPLINE)이라 **설계 결정·단계 계획이 문서에 먼저 박혀야** 코드가 그걸 구현하는 구조다.

어떤 작업(기능·수정·연출·엔진 변경)이든 코드를 건드리기 **전에**:

0. **기존·상충 결정부터 읽는다 (선행 — 가장 자주 빼먹는 단계)** — 내 계획을 적기 **전에**, 그 영역의
   `docs/*_SYSTEM.md`(+`BOARD_RULES`/`UI_RULES`)에서 **그 주제에 이미 내려진 결정**을 `grep`/읽기로 확인한다.
   "이미 정해진 게 있나? 내 변경이 그걸 뒤집나?" **"작은 표시/UI 한 줄 수정"도 문서화된 동작**(컷오프·스포일러
   정책·귀속·밸런스 계수)을 바꾸면 시스템 변경 → 먼저 검색. 만지는 파일이 `app/`·`engine/`·`data/` 면 그 시스템
   문서를 의심부터. (실패: results 표시 기준을 SEASON_SYSTEM §3.1 안 읽고 반대로 고쳐 충돌, 2026-06-24.)
1. **설계/단계 계획을 해당 문서에 먼저 적는다** — 새 결정은 `docs/*_SYSTEM.md`(또는 `BOARD_RULES`·
   `CLAUDE.md`)에 **먼저** 반영하고 색인(`docs/README.md`)을 갱신한다. 기존 결정을 뒤집으면 취소선 정정으로 보존(DOC_DISCIPLINE).
2. **그다음 개발에 착수한다** — 문서에 적힌 계획·기준(통과 조건·측정 도구)을 코드가 구현한다.
3. **단계마다 문서를 진실과 일치시킨다** — 완료하면 그 단계를 "✅ 완료(날짜)"로 표시하고, 검증 명령을
   `docs/README.md` 검증 루틴에 등록한다(취소선 정정 보존).

> 다단계 작업이면 **각 단계 착수 전** 그 단계 계획이 문서에 있는지 확인하고, 없으면 먼저 적고 시작한다.
> (예: 터치 스크립트 1→2→3단계는 `BOARD_RULES.md` "설계 결정" 절에 3단계가 먼저 박혀 있고, 각 단계 착수 때 갱신.)
