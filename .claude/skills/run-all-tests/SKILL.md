...
### 2.5 가드 감사 스텝 (exit만 믿지 말 것 — 기법 ⑥, 2026-07-06 신설)

exit 코드 수집으로 끝내면 **가드 자신의 버그**(`_ev_recvmatch`가 ❌를 출력하면서 exit 0으로 12일 잠복)와
**수치 드리프트**(문서 "100%"인데 가드는 94%를 흘리며 통과)를 놓친다. 배터리를 돌린 뒤 아래를 확인한다:

- **판정줄 ↔ exit 일치**: 각 가드의 **출력 마지막 판정 줄(✅/❌·PASS/FAIL)** 이 exit 코드와 **일치**하는지
  본다. ❌를 출력했는데 exit 0이면 **가드 자신이 버그**(exit 미배선) — `TEST_METHODOLOGY §4` 판정 미배선 행.
- **핵심 수치 ↔ 문서 대조**: 가드 출력의 핵심 수치(%·건수·분포)를 해당 `docs/*_SYSTEM.md`·`BOARD_RULES`의
  **주장 수치와 대조**한다. 조용한 드리프트(리시버 100%→94%, 포지션 세트당 수치 stale)는 exit 0이어도 **발견**.
- **올그린은 회귀 무위반일 뿐** — *발견*(불변식에 없는 새 버그)은 `spec-audit` 스킬(기법 M)로 별도 실행한다.
  최근 변경 폭이 컸거나 신규 시스템이면 배터리 PASS로 끝내지 말고 spec-audit 발견 스윕을 권한다.
