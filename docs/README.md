...
(cd server && npx tsx --env-file=.env.local tools/_dv_coupon_live.ts)  # 쿠폰 발급·사용 라이브 가드(BACKEND §13.14·13.15·13.17 P0-5, 2026-07-06) — 라이브 dev DB: 발급→redeem(+reward DB대조)·이중사용 used·기간(만료/시작전)·개인타겟 은폐 invalid·disabled·무토큰 redeem 401+dev-user-1 무변화(C1)·date-only endsAt KST(C2)·미존재 targetUserId 400 no-such-user(C3)·중복 409·사용기록 DELETE 409·무토큰 admin 401 12항목 + 이중사용 UNIQUE 게이트 민감도 A/B. _DVCPN_ 테스트쿠폰·유저 자동정리. exit 0/1
(cd server && npx tsx --env-file=.env.local tools/_dv_achearn.ts)  # 업적 적립 라이브 가드(BACKEND §13.12 P0-2·H3, 2026-07-06 발견·검증=Fable 5/수정·문서=Opus) — 라이브 dev DB 실토큰: 정상 earn(+applied)·멱등 재호출(applied:false)·호출당 클램프(99999→1000)·평생합 경계(19,900→+100 클램프)·초과 409 cap·A/B(백스톱 없으면 통과 대조). _DVACH_ 테스트유저·원장 자동정리. exit 0/1
