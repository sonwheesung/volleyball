...
npx tsx tools/_dv_fogleak.ts                 # 스카우팅 안개 누출(EC-DR-03, 2026-07-07) — 공용 fogOvr(prospectScout): (a)120유망주×reveal 임계 아래=범위(정확치 0누출)·위=정확 + A/B(누출 변종=아래서 정확치 재현) (b)형제 정적 grep(draft.tsx·draft-live.tsx의 유망주 overallRaw 렌더가 reveal≥0.92/fogOvr 게이트 뒤에만, 우회 0). "내 지명 결과" 미리보기 안개 우회 회귀 차단. exit 0/1
