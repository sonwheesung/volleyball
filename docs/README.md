...
| [STAFF_SYSTEM](./STAFF_SYSTEM.md) | 스태프 계약(감독·전문코치·스카우터)·예산·훈련부스트·드래프트 안개. **📋 설계 확정·미구현(2026-07-20, §9 스태프 3.0)**: 감독 능력 3축(경기 운영·육성 철학·리더십, charisma 대체)+명성(시장가치 — 경력 로그 파생·기대 대비 평가·언론 예상 순위 개막 뉴스·거장 티어·명장 열전)+시장 UX(재계약 카드·놓아주기·AI 경쟁·카운터오퍼 1회), 게이트 ①~⑥ | `engine/staff.ts`, `data/league.ts`, `app/staff.tsx`, `app/draft.tsx` |
