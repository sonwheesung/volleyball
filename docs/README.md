...
npx tsx tools/_dv_retire.ts                 # 은퇴 재정비(FA §1.2·MONETIZATION §11.2, 2026-07-08) — ①40세+현역0(200시즌sim) ②HIGH(medOvr+δ7)이상 은퇴0·연속곡선(OVR 1점 항상 유효=절벽금지)·40정년=1 ③결정론+외인 은퇴루프 제외(rng 미소비=국내 스트림 불변) ④39세 전지훈련 차단(store reason=retiring, 38세 대조) ⑤계약연한 40−나이 초과0(capContractYears) + A/B 뮤턴트 이빨(하드월/HIGH0/캡 제거 모사→FAIL). exit 0/1
...
npx tsx tools/_dv_recap.ts 50               # 시즌 결산 포스트시즌 결말(myPostseasonOutcome, SEASON §5.5) — kind 분류(integrated/champion/runnerUp/poOut/missed)·시즌당 분포 1/1/1/4·스코어=독립오라클(series.games 재집계) 전수일치·결정론·A/B 이빨(오염 outcome 3종 검출). exit 0/1
