...
npx tsx tools/_dv_resignoutlook.ts          # 계약 관리 '잔류 전망' 셀렉터 엔진 위임(FA §2.5c-보완 3단계) — resignOutlookNow.prob==refuseResignProb+가산항(재구현 아님)·band 경계 0.15/0.45·override money 반영(stingy 전망↑/generous↓). exit 0/1
npx tsx tools/_dv_resignfeedback.ts         # 재계약 UX 격상(FA §2.5c-격상·EC-FA-17, 2026-07-11) — ①옵션 프리뷰==resignOutlookNow(옵션 override, 재구현 아님)+money 후하게≤표준 단조 ②reason/myResigned가 실제 buildOffseason 버킷과 일치(refused/notOffered/capSqueezed·은퇴 제외, 3시나리오) ③도장 뉴스=수락자 집합(결산 1건)·resign 개별 미발화(스포일러 무해)·release 사유 리드 ④결정론. exit 0/1
npx tsx tools/_ms_resignfeedback.ts [560]   # (측정) 재계약 UX 설계 근거 — ①만료자 불만 topic 분포 ②money 불만 3택 밴드 divergence(핵심·15% 문턱) ③오퍼일→시즌말 flip율. N≥1만 만료자(≈560시즌). on-demand(무거움)
