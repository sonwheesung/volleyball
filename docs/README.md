...
| [STAFF_SYSTEM](./STAFF_SYSTEM.md) | 스태프 계약(감독·전문코치·스카우터)·예산·훈련부스트·드래프트 안개. **✅ 스태프 3.0 구현 완료(2026-07-20, §9 — Phase A~E)**: 감독 능력 3축(경기 운영·육성 철학·리더십, charisma 대체·ENGINE_VERSION 11)+명성(시장가치 — 경력 로그 파생·기대 대비 평가·언론 예상 순위 개막 뉴스·거장 티어·명장 열전)+시장(AI 경쟁·선호·카운터오퍼·공석 폴백)+실효과 훅 8종, 게이트 ①~⑥ 전건 실측 통과(가드 _dv_coach3axis·_dv_reputation·_dv_preseason_pred·_dv_reputation_dist·_dv_head_market·_dv_staff3_hooks·_dv_hall_of_coaches) | `engine/staff.ts`, `data/league.ts`, `app/staff.tsx`, `app/draft.tsx` |
