...
npx tsx tools/_dv_walletauth.ts && npx tsx tools/_dv_coupon.ts && (cd server && npx tsx tools/_dv_security.ts) && (cd server && npx tsx tools/_dv_ratelimit.ts) && (cd server && for t in _dv_purchase _dv_announce _dv_devnote_live _dv_ticket_live _dv_coupon_live _dv_achearn _dv_walletreplay _dv_account_live walletConcurrency _dv_pass _dv_1p1 _dv_pass_live _dv_mail _dv_mail_live; do node_modules/.bin/tsx tools/$t.ts || exit 1; done)  # 라이브 8+패스 3+우편 2(2026-07-23): --env-file 없이 → _env.ts가 dev(.env.development.local) 우선 로드
