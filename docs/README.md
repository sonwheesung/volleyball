...
| [WORLDCUP_SYSTEM](./WORLDCUP_SYSTEM.md) | **월드컵 참가 ~~유료 DLC~~ 무료 추가 콘텐츠**(2026-08-03 전환 — 4년 비시즌 국가대표 차출→성장+사고면제+업적, **스카우팅 쇼케이스=차출선수 FA 영입**) — 📋 설계 완료·구현 추후. 100세션 재검증·현실일정 출처 반영 | `engine/nationalTeam.ts`·`engine/seasonBake.ts`·`data/worldCup.ts`(예정) |
...
| [MIGRATION_CAUTIONS](./MIGRATION_CAUTIONS.md) | **마이그레이션 주의사항(2026-07-27 신설)** — 클라 세이브(버전 게이트 일회성·결정론·필드만 교체·**과거 아카이브 무접촉 보존**·리플레이 재계산 인지) vs 서버 DB(Expand/Contract 3단계) 구분 + 착수 전 7문 체크리스트 + 실사례(특성 v4→v5) | `store/saveMigration.ts`·drizzle, 메모리 `prod-schema-migration-caution` |
...
| [MONETIZATION_SYSTEM](./MONETIZATION_SYSTEM.md) | **수익화**(무료+광고+다이아 IAP+광고제거 ~~+DLC~~) — 다이아(소비성=전지훈련)·광고 쿨다운 카운트다운(A4 ✅ 2026-07-01)·광고="시즌 시작하기" 버튼. **온라인 전환(2026-07-01)**: 오프라인/RevenueCat 폐기→Vercel(BACKEND_SYSTEM). **월드컵 무료 전환(2026-08-03)**: ~~월드컵 유료 DLC~~ → 무료 추가 콘텐츠(수익 항목 아님). 📋 **추후 무료 콘텐츠 후보: 올림픽·올스타전(2026-07-07 아이디어, 미설계, §4.1 — 올스타는 세러머니 연출 선결 필요로 로드맵 강등)** · **스킨 BM(코스메틱 — 경기장·UI테마·BGM·프로필·엠블럼, P2W 아님)** · **프리미엄 시즌 투자(구단주 특별투자 +운영비, FINANCE #5)** | `engine/diamonds.ts`·`app/(tabs)/mypage.tsx`·`app/training-camp.tsx`·`lib/ads·iap·log` |
...
| [SERVER_OPS](./SERVER_OPS.md) | **개발/운영 서버 운용 절차서(2026-07-15 신설)** — ~~두 세계~~ → **세 환경 매트릭스**(dev=로컬 next:3000+로컬 DB / **stg=Vercel Preview `staging` 브랜치+Supabase stg**(2026-08-07 신설) / prod=Vercel+Supabase)·**stg 배포 절차 §3.6**·**DB 비밀번호 회전 §3.5**·개발 테스트 체인(DB 기동·WinNAT 우회 임시 PG·앱 LAN 재포인트 `EXPO_PUBLIC_SERVER_URL` 오버라이드)·배포 체인(마이그레이션 먼저·vercel·스모크)·증상→원인 표("연결 필요"·dev 401 등). BACKEND §13.7이 설계 정본, 이 문서는 운용 | `server/`(next dev·drizzle-kit·vercel)·루트 `.env` |
| [GLOBALIZATION_PLAN](./GLOBALIZATION_PLAN.md) | **글로벌 출시 로컬라이제이션 계획(2026-08-03 신설, 📋 미착수·결정변수 미정)** — 전세계 출시 목표 하 KOVO/한국어 결합도 코드 실측(이름·통화·팀 / 아바타 / 외국인·국적 / i18n). 지배 비용 = **① UI+서사 다국어화(XL, josa 엔진+news.ts 재작성) ② "외국인=예외" 전제 뒤집기(XL, 캡·FA·정원 `!isForeign` 수십곳 + 밸런스 재튜닝)**. 이름·팀·통화는 잘 격리(S~M), 얼굴은 코딩M·에셋XL. 옵션 A(MVP 1~2개월+)/B(제대로 3~6개월+) + 결정변수 3(언어수·구세이브전환·얼굴국적정합) | `data/names.ts`·`data/faceSheetMeta.ts`·`engine/foreign.ts`·`lib/josa.ts`·`data/news.ts` |
| [STAGING_PROD_RESET_RUNBOOK](./STAGING_PROD_RESET_RUNBOOK.md) | **스테이징 구축 + 운영 DB 출시 초기화 런북(2026-08-03 신설, 1회성)** — 별도 Supabase 프로젝트(stg)로 운영 데이터 복제(백업 겸)→row 대조 검증→운영 테스트 데이터 초기화. 게이트 5개(복제→검증→백업→**사용자 명시 승인**→초기화→검증), 순서 절대 준수(G2 전엔 운영 무손상). 테이블 18개 분류(⚙️설정 `proj_info`·`server_setting` 삭제금지 / 📢콘텐츠 공지·노트·쿠폰·전체우편 유지 / 👤유저·결제 초기화). ⚠ 원장·결제 초기화는 **출시 전·테스트데이터 한정 1회**(실결제 후 불가) | `server/.env.staging`·`server/db/schema.ts`·pg_dump/psql |
| [VERCEL_PRO_LAUNCH_RUNBOOK](./VERCEL_PRO_LAUNCH_RUNBOOK.md) | **Vercel Pro 전환 + 출시 전 env 확정 런북(2026-08-05 신설, 실행 대기)** — Hobby=비상업 ToS라 수익화 출시 전 Pro 필수(빌링만 변경·배포 무파급). 오늘 핵심 동반작업 = **`TELEMETRY_SALT` 시크릿 지정**(가명화 완전화 [[telemetry-pseudonymized]], 미설정 시 폴백상수). prod env 전수 체크리스트(코드 `grep process.env` 근거 — 필수/결제/레이트리밋/마이그레이션/선택/자동 분류) + 재배포·스모크. ⚠ 결제·시크릿=사용자 직접, `vercel link/env pull` 금지(env 클로버). | (전환=빌링) `TELEMETRY_SALT`·Vercel env |
...
| [IAP_SETUP](./IAP_SETUP.md) | **결제(IAP) 셋업 런북(#43 실행용, 2026-07-31 신설)** — 서버 코드 완성(`_dv_purchase` money-path green) 위에 배구명가 실값을 얹은 콘솔 셋업 절차: 패키지 `com.son0925.volleyball`·웹훅 URL·6팩 SKU/가격·엔타이틀먼트(remove_ads·dlc_worldcup)·패스 SKU. ⚠ **조직 전환(PLAY_ORG_CONVERSION) 먼저** — 개인계정 merchant 세팅 후 전환 시 재작업 위험. 정본 절차·트랩=`store-iap-setup` 스킬 | `lib/iap.ts`·`data/diamondTiers.ts`·`server/app/api/purchase/` |
| [PLAY_ORG_CONVERSION](./PLAY_ORG_CONVERSION.md) | **Google Play 개인→조직 계정 전환 런북(2026-07-31 신설, 실행용)** — 개인 계정 "비공개 테스트 12명×14일" 프로덕션 게이트 면제 목적. D-U-N-S `696493045`(발급 완료) 반영(24~48h) 뒤 실행. 폼 정본 값(D&B/홈택스 영문 증명서 글자 단위 일치)·브랜드(Vivace Games/휘성게임즈) 정렬. 진행 중 비공개 테스트엔 무영향 | (Play 콘솔 계정 — 코드 무관) |
| [DIAMOND_PASS_SYSTEM](./DIAMOND_PASS_SYSTEM.md) | **다이아 패스 + 월 1+1(2026-07-23 신설, 구명 출석 패스)** — 패스(₩9,900·28일·매일 100💎 **우편함 발송** — 구매 즉시 1일차 우편+매일 KST 00시 스케줄러, ~~접속 자동수령·유예 3일·04시 리셋~~ 취소선 정정)·소비성·`attendance_passes` 진실·중첩 큐잉·환불 전액 클로백+미수령 우편 recall(B1~B4 봉인) + 월 1회 1+1(6팩 서버 2배·월키 멱등·환불 미복구). 미결=Q4 법률자문뿐. Phase①서버 ✅→스케줄러·앱 재작업(§9 배너)→③실 SKU(#43 뒤) | `server/lib/pass.ts`·`server/db/schema.ts`·`lib/iap.ts`·`app/buy-diamonds.tsx` |
| [MAILBOX_SYSTEM](./MAILBOX_SYSTEM.md) | **우편함(2026-07-23 신설, 사용자 확정)** — 운영 보상·개별 지급 채널: 개별+브로드캐스트 우편, 첨부 다이아/패스(브로드캐스트는 다이아만), 상태 3필터(기본 안받음)·보존 30일(패스 첨부 60일)·마이페이지 진입+빨간 점 2곳(read 소등)·관리자 발송 탭(멱등키·통당 캡·recalled_at 소프트 회수·관측행). 독립 리뷰 블로커 3건 반영(grantPassTx 원자 합성·rejectOnQueueFull·store_txn_id 합성키 `mail:<id>`). 구현 Phase①서버→②앱→③관리자 | `server/lib/pass.ts`(grantPassTx)·`server/db/schema.ts`(mails)·`app/(tabs)/mypage.tsx`(예정) |
...
| [DEVNOTES_SYSTEM](./DEVNOTES_SYSTEM.md) | **개발자 노트/패치노트**(관리자 에디터→서버 원격 콘텐츠, 앱 업데이트 없이 게시) — 구분 탭(패치노트|개발자 노트, 2026-07-08 사용자 결정)·안읽음 배지(로컬)·오프라인 캐시·무푸시. 공지(차단성)와 역할 구분. ✅ Phase1·2·3 구현 — ~~라이브 가드는 DB 기동 후 보류 / 앱 화면(Phase3) 미착수~~ → 정정(2026-07-24 코드 대조): 서버 CRUD+관리자 에디터(2026-07-15) · `_dv_devnote_live` exit 0 실행 확인(보류 해제) · **앱 화면 구현 완료**(목록·상세·마이페이지 안읽음 배지·오프라인 캐시) | `server/db/schema.ts`(devnotes)·`server/app/api/{devnotes,admin/devnote}/route.ts`·`server/app/ops-9f3a2c/page.tsx`(노트 탭)·`server/tools/_dv_devnote_live.ts`(가드)·`app/devnotes.tsx`·`app/devnotes/[id].tsx`·`lib/server.ts`·`store/useAuthStore.ts`(readDevnotes·pruneReadDevnotes) |
...
npx tsx tools/_dv_seasontele.ts            # 시즌 종료 행동 텔레메트리 집계(BACKEND_SYSTEM §13.27, 2026-07-30) — computeSeasonTelemetry 순수(입력 무변경)·정확(개입/방출/지휘/훈련 집계)·A/B(지휘=최신일·빈개입 0·훈련null·팀null 방출0). exit 0/1
# 서버 라이브 가드(dev DB 기동 후): cd server && npx tsx tools/_dv_telemetry.ts (텔레메트리 저장·업서트교체·401·관리자집계) · tools/_dv_earnedach.ts (지급완료 업적 pre-mark)
npx tsx tools/_dv_libero_toss.ts           # 리베로 전위 점프 토스 금지(BOARD_RULES 룰 73, 2026-07-30) — 전위 리베로 세트에 noJump 스탬프(백존 점프 세트는 허용)·오탐 0 + A/B(noJump 강제 false면 위반 검출). exit 0/2
...
npx tsx tools/_dv_ivbudget.ts              # 개입 잔여 예산 표시=엔진 정합(같은 데드볼 연속 교체 즉시 차감·후보 배제·게이트=실수락·세트개막 0:0 TO·예약회계, 2026-07-21 테스터 보고+스펙 감사 수정)
npx tsx tools/_dv_setsubs.ts               # FIVB 세트당 교체 ≤6 총량 오라클(복원형 겹침창+수동 IN 포함 — P0 예약 회계 봉인, 구 게이트 재주입 시 7교체 검출. 2026-07-21 스펙 감사 A)
npx tsx tools/_dv_boardlineup.ts           # 보드 base 라인업 == 엔진 라인업(dvPhilosophy 전달 — dv 누락 구버그 A/B 재현. 2026-07-21 스펙 감사 B, BOARD_RULES 룰 30d)
...
npx tsx tools/_dv_draft_poster.ts          # 지명 포스터(UI_RULES DL-9): draft_stage 패널 아웃라인(74~94.4%) 내포·세로 예산(12.56≤18.9%h)·소스 좌표 드리프트 + A/B(뚫는 좌표·과대폰트 검출 — 2026-07-22)
npx tsx tools/_dv_awardvalue.ts            # 시상 value 기준 라벨(AWARDS §10, 2026-07-24) — 상별 value 산식이 다른 걸(정규MVP=impactScore×팀가중·챔프/신인=원값·기록상=실카운트) 화면에 무엇의 수치인지(공헌지수/득점·킬·블록·개) 명시 + 정규MVP 각주(팀순위 반영). V1~V4 실산출 대조·합성 A/B. exit 0/1
npx tsx tools/_dv_seedforeign.ts           # 시드 수입선수 계약 1년 고정(FOREIGN_SYSTEM §1, 2026-07-24) — 외인·아시아쿼터 생성 시 remaining=1(국내는 1~3 유지) + rng 스트림 불변(추첨 소비·값만 대체, 골든 해시 불변) + willBeFA/결산 브리핑 누수 0 + A/B(구로직 뮤턴트 다년 검출). exit 0/1
npx tsx tools/_dv_ceremony.ts              # 시상식 이어보기 진행도(SEASON §5.3.1, 2026-07-24) — recordChampion(championId=허브 게이트) 시점 불변 + 관람 진행도 ceremonyProgress 별도(campDone 패턴, 구세이브 0). -1 끝남/0 보러가기/n 이어보기 분기, 본 비트부터 재개(차단 아님). C1~C6 + A/B(진입=완료 구 오라클과 갈림). exit 0/1
...
npx tsx tools/_dv_traits.ts                 # 선수 특성 엔드투엔드(TRAIT_SYSTEM §5, 발견·실측=Fable 5/가드=Opus) — ①전원 1개 이상(무특성 0)+상극쌍 0건+검사기 A/B(합성 {clutch,choke} 검출) ②서브머신 에이스·범실 ON>OFF+liveness>0 ④노쇠 서열 ⑤노력형 전스탯합 서열(⚠기술합 함정 대조) ⑥부상 배수 1.70·0.55 ±0.01 + A/B(injuryTraitMult≡1 mutant→⑥ FAIL 증명) + **상시형 6종 방향검증(2026-07-27)** ⑦폭격기(킬%↑+공격범실%↑+무효과세계 OFF/OFF→FAIL 증명) ⑧수비벽 ⑨황금손 ⑩꾀돌이 ⑪강철체력 ⑫지구력. ③클러치는 소폭·고분산이라 제외(measTraits N≥3000 별도). ~5초. exit 0/1
npx tsx tools/_dv_reactive.ts               # 반응형 특성(TRAIT_SYSTEM §6.3, Phase 2a — 조커/유리멘탈/오뚝이) — (a)결정론·무해성(반응형 미부여=버프0·동일시드 바이트동일) (b)트리거 발동(교체투입 조커·블록당함 유리멘탈/오뚝이) (c)지속 5랠리 자동만료·타임아웃 즉시clear·세트끝clear (d)선수당 1개(Map 덮어쓰기) (e)±10% 하드캡 clamp (f)방향 A/B(조커 킬%↑·유리멘탈↓ + 무효과세계 오라클 FAIL 자가검증). exit 0/1
npx tsx tools/_dv_golden.ts                 # 골든 카나리아 — 고정 시드 3경기 sha256 박제(ENGINE_VERSION 14). 의도 엔진변경=버전범프+`--update` 재생성 동반. `--selftest`(오라클 비공허)·rng 스트림 사고 조기경보. exit 0/1
...
npx tsx tools/_dv_gp.ts                     # 출전 경기수·세트수(ProdLine.gamesPlayed·sets, UI-46·MATCH §1.3c-2·SALARY §1) — A~C: gamesPlayed 정수·경기당 0/1·subUse 분수 matches와 독립 A/B / D: setUse 정수·1≤s≤nSets·클린경기 전세트출전 ≥6/팀+리베로·prod.sets==setUse & gamesPlayed==setUse존재. 뮤턴트(libero capture 누락→D2 FAIL) 실증. exit 0/1
...
npx tsx tools/_dv_rosterexcess.ts           # 내 팀 로스터 정원 초과 자연 정리(FA §1.7-내팀, 2026-07-28) — 위시 지명(캡 미검사)이 내 팀을 24까지 부풀린 초과를 다음 오프시즌 buildOffseason이 캡(20) 초과분만 자연 정리(다년 잔여 후순위·포지션 floor 보호·최저가치 우선)하는지. A/B(정리 비활성 뮤턴트=초과 잔존). exit 0/1
...
npx tsx tools/_dv_initfa.ts                 # 초기 FA 풀(TRANSACTION §5c, 2026-07-28 — 테스터 "초반 영입 불가" · 구현=Opus/검증=Fable) — 게임 시작 시 잉여 국내 FA 10명(표시 OVR 70~80대·포지션 스프레드) 시드. A.구조(밴드·5포지션·playerMap 해석·결정론·재로드 생존) B.무결성 스윕(teamless 존재로 시즌0 순위·전력·라운드로빈 crash/NaN 0) C.밸런스 A/B(N≥1만 실측 — 최약팀 Δ+2.3%p·최강팀 Δ0.0%p=원시60~69는 약포지션만 메움, 게임파괴 아님) + C0 비공허 오라클. D.소급 백필(2026-07-29 — pre-feature 시즌0 세이브: `initFaBackfillPool` 진리표 D1~D6, 시즌0·빈풀·무거래·팀선택만 주입/그 외 불변). GOLDEN 불변은 _dv_golden 별도. exit 0/1
...
npx tsx tools/_dv_draftorigin.ts             # 드래프트 히스토리 영구기록(#76, 2026-07-27) — sequence round/overallPick 명시 emit 정확성(패스 낀 케이스)·draftOrigin set-once 멱등·지명값 오라클 일치·유찰/제네시스 미부여 + A/B(round+1 mutant FAIL 재현·옛 휴리스틱 발산 대조). exit 0/1
...
npx tsx tools/_dv_campprune.ts              # campLog 롤링 프룬 무한증가 봉인(MONETIZATION §11.2, 2026-07-27) — 다시즌 진행+은퇴 시 endSeason 프룬이 campLog를 리그 잔류 선수로 유계 수렴(은퇴/이탈 엔트리 제거) + careerGrowthOf(선수별 전 시즌 cur-gain 누적 차감) 프룬 후 정확. A/B: 프룬 없는 누적(everInjected)은 시즌수 비례 무한증가·slice cap이면 careerGrowthOf 오답. exit 0/1
...
npx tsx tools/_gt_achdot.ts                 # 마이페이지 탭 **빨간 점** 배선(ACHIEVEMENT §입력 배선, 2026-08-08 운영 버그 — 유저가 다이아 못 받음) — 실 store 구동: C1 tabIds⊆cardIds(거짓양성0)·**C2 n≥1⇒점 켜짐**·C3 오프시즌 하한==정확·C4 고아 fixtureId 내성·C5 claimed 정합(resetSave 포함)·C6 정확값 캐시 신선/stale 폴백·C7 5개 호출부 셀렉터 경유 정적강제. A/B: `--mutant`(구 raw 배선 → C2 뒤집힘 증명). exit 0/1
...
npx tsx tools/_dv_ovrbadge.ts               # OvrBadge 이중 스트레치 봉인(UI-52, 2026-07-30) — OvrBadge 내부 displayOvr 1회 계약: 호출부는 raw만 넘긴다. 소스 계약 A~C(본문 preScaled 분기·모달=overallRaw(p)·카드 2곳 preScaled) + 수치 A/B D1~D4(단일=displayOvr(raw)≠이중=99 상한, 카드 preScaled=per-player 정본 평균≠이중). exit 0/1
npx tsx tools/_dv_bignews.ts                # biggame "한 경기 N점 폭발" 기사 승패 인지(NEWS §4.6, 2026-07-30) — 진 경기 개인 폭발이 승리 톤으로 나오던 버그. (A)POOLS.biggame/biggameLoss 톤 정적 분리(승리 풀에 패배어·패배 풀에 승리어 누출 0) (B)실측 buildNewsFeed 승34·패21건 승패↔톤 매핑 일치 (C)구버그 재현 body 변이가 "진 경기=loss" 규칙에 걸림(허위 오라클 방지). exit 0/1
npx tsx tools/_dv_awards_season.ts          # 기록 아카이브 시즌 탭 시상식 노출 정책(AWARDS §6, 2026-07-30) — 진행 중엔 MVP·신인상·베스트7·부문기록상 미노출(시즌 종료 후에만), 순위표·개인 기록 리더보드만. 시상식 그룹 게이트=!provisional 정적 확인 + A/B(awardsReady 회귀 검출). exit 0/1
npx tsx tools/_dv_reentry.ts                # FIVB 15.6.1 합법 재진입(MATCH_INTERVENTION §4.2, 2026-07-31, ENGINE v20) — 뺐던 선발 A가 자기 슬롯·자기 교체선수 B와 교대로 세트당 1회 재진입(유저 manual만). (a)합법 재진입(A복귀·예산2·B이탈) (b)불법 3종 차단(타슬롯·B재진입·A재이탈 no-op) (c)AI 격리 무개입 재진입위반 0 + A/B(게이트 revert시 FAIL). 바이트동일=골든 해시 불변(v19=v20). exit 0/1
...
npx tsx tools/_dv_stambypoint.ts            # SimResult.stamByPoint 코트 체력 스냅샷 가드(경기 보드 "코트 체력" 표시) — ①길이 정합(==points.length) ②각 엔트리 home/away 6~7명(선발6+리베로)·stam∈[0,1] ③결정론(동일 시드=동일 바이트) ④골든 무영향(serializeMatch가 stamByPoint 미참조, A/B) + 생리 sanity. exit 0/1
...
npx tsx tools/_dv_josa.ts                  # 동적 값 뒤 하드코딩 조사 파손 방지(UI 텍스트 josa 헬퍼 6+2 사이트) — (A) hasBatchim 경계((글자-0xAC00)%28) 독립 오라클 (B) 각 사이트 조사 종류를 받침/무받침 양쪽 문법 검증+옛 하드코딩 A/B FAIL 재현 (C) 소스 스캔 회귀 트립와이어(헬퍼 호출 유지·옛 리터럴 없음). exit 0/1
npx tsx tools/_dv_newsscore.ts             # 경기 뉴스 상세 스코어보드·득점원 Top3(NEWS §11.5, 2026-07-24) — match/debut=day+teamId 유일 fixture→buildMatchBox·playoff 경기별=ref 숫자g→buildPlayoffBox / 세트칩 합=homeSets·awaySets·Top3 득점=box 정렬·byte 일치(box↔seasonMatchProds 304명 0불일치) / 배제(po:clinch·champion·clinch·기타 kind→null)·정규 유일성(2건↑ 생략) + A/B 뮤턴트 2종(정규↔플옵 경로 뒤섞기·시리즈 ref 허용) + **(g) 멀티 트리플크라운(count≥2) 헤드라인/본문 스탯 ↔ mp.lines@n.day·스코어보드 박스 교차 정합(2026-07-24 버그1 봉인 — 헤드라인 첫경기·박스 최근경기 모순, 구로직 A/B FAIL)**. exit 0/1
...
npx tsx tools/_dv_hub.ts                   # 오프시즌 "완전 허브" IA 회귀(SEASON §5.6·UI-50, 2026-07-24 FA 렌더 크래시 소프트락 봉인) — A(정적): 체인 push 잔재 0·앞단 6화면 일정 복귀 출구·오프시즌 라우트 9+루트 ErrorBoundary·두 위상 허브 + 최종 버튼 무게이트(완료 게이트 금지)·endSeason 진입점 공용훅(모듈 래치) / B(런타임): draftSelections 무효화 대칭 12레버·허브 목록 정본·untouched 판정·개막 상시 도달·endSeason 더블탭 2전진 0·성장 모달 반복 포커스 재발화 0·확정픽 경고 게이트. A/B 뮤턴트 6종(체인 복원·무효화 누락·EB 제거·campDone 게이트·경고 제거·래치 제거) 전부 FAIL 검출. exit 0/1
...
npx tsx tools/_dv_ota.ts                   # OTA 부팅 자동적용 순수 헬퍼(AUTH_SYSTEM §4.1, 2026-07-29, 검증=Fable/구현=Opus) — 킬스위치 otaAutoApplyEnabled 진리표(undefined/null/{}→on·false→off)·withTimeout 타임아웃 실증·상수 sanity + A/B 감도. 실제 reload는 preview/실빌드 E2E 전용(node 무동작). exit 0/1
...
> - **일회성 세이브 마이그레이션 검증(버전 게이트 — 해당 migrate 경로 손대면 재실행)**: `_dv_trait_migrate`(v4→v5 특성 20종 확장 — 구세이브 base 선수 전원 traits가 rollTraits로 재부여: 1~3개·상극 0쌍·반응형 최소 1명·아카이브 바이트 불변·version≥5 게이트, A/B=재부여 스킵 뮤턴트 FAIL, SAVE §6·TRAIT §1)
...
```
# ── 전체 배터리 복붙 체인(순수 5 + 루프 18, 하나라도 실패 시 중단) ──
npx tsx tools/_dv_walletauth.ts && npx tsx tools/_dv_coupon.ts && (cd server && npx tsx tools/_dv_security.ts) && (cd server && npx tsx tools/_dv_ratelimit.ts) && (cd server && npx tsx tools/_dv_sentry_gate.ts) && (cd server && for t in _dv_purchase _dv_announce _dv_devnote_live _dv_ticket_live _dv_ticket_anon_live _dv_admin_scope_live _dv_coupon_live _dv_achearn _dv_walletreplay _dv_account_live _dv_backup_live _dv_signup_notify walletConcurrency _dv_pass _dv_1p1 _dv_pass_live _dv_mail _dv_mail_live; do node_modules/.bin/tsx tools/$t.ts || exit 1; done)  # --env-file 없이 → _env.ts가 dev(.env.development.local) 우선 로드. ※개수 정정(2026-08-07 실측): 순수는 5(walletauth·coupon·security·ratelimit·sentry_gate), 루프 18 중 _dv_pass·_dv_1p1·_dv_mail 3종은 DB 불필요(순수)라 "라이브"가 아니다 — 구 주석 "순수 3 + 라이브 8"은 어느 쪽으로도 실제와 안 맞았다. **체인 밖(온디맨드)**: `_e2e_purchase_live`(실행 중 dev 서버 :3000 전제)·`_dv_stgisolation`·`_dv_prodconn`(운영/stg DB 왕복)
...
(cd server && npx tsx tools/_dv_stgisolation.ts) # stg↔prod 격리 가드(STAGING_PROD_RESET_RUNBOOK §3.5.7, 2026-08-07) — **stg 구성이 바뀌거나 Vercel env를 추가할 때 온디맨드**(배터리 체인 밖 — stg DB 네트워크 왕복이라 느리고, `.env.staging` 없는 기기에선 SKIP 종료). 검사: DB 호스트 분리·시크릿 분리(JWT 공유 시 stg 토큰이 운영에서 유효)·자리표시자 잔존(`REPLACE`류가 진짜 값으로 통과한 실사고)·연결 포트 운영 동형(6543)·stg DB 실접속(18테이블+proj_info 시드). A가 깨지면 **운영 DB에 붙지 않고** 종료. A/B 자가검증(DATABASE_URL 운영값·ADMIN_TOKEN 자리표시자 변이 주입→각각 FAIL). ⚠ 로컬 env만 봄 — Vercel Preview 스코프는 배포된 stg `/api/health`의 `dbRef`로 확인(가드가 기대값 출력). exit 0/1
(cd server && npx tsx tools/_dv_prodconn.ts) # 운영 DB 연결 2종 점검(SERVER_OPS §3.5.1 6단계, 2026-08-07) — **비밀번호 회전 직후 온디맨드**(배터리 체인 밖 · 운영 DB 왕복 · 읽기 전용 `select 1`+카운트만). `DATABASE_URL`(6543 런타임)·`MIGRATE_DATABASE_URL`(5432 마이그레이션)의 **포트 기대치·동일 호스트·실접속**을 본다. 왜 필요한가: 회전 때 6543만 고치면 **서비스는 멀쩡한데 다음 스키마 변경에서만 `28P01`로 막힌다**(런타임이 5432를 안 쓰므로 몇 주 뒤 발현 → 원인 추적 난망). 2026-08-07 실제로 그 상태였다. A/B: MIGRATE 비밀번호만 변이 주입 → `28P01` 잡고 원인까지 지목·exit 1, 원복 후 PASS. `.env.local` 없으면 SKIP. exit 0/1
(cd server && npx tsx tools/_dv_sentry_gate.ts) # Sentry 환경 게이트 순수 가드(BACKEND §13.21, 2026-07-24 알림 폭주 사건) — DB·네트워크·실 Sentry 전송 없음: sentryEnabled() 진리표(DSN 유무 × VERCEL_ENV 없음/production/preview/development × 탈출구)로 dev/가드가 만든 500이 운영 Sentry로 새지 않게 `lib/sentryGate.ts`가 사문화 안 됐는지 봉인. exit 0/1
...
(cd server && node_modules/.bin/tsx tools/_dv_pass.ts)  # 다이아 패스 순수(DIAMOND_PASS §2, 2026-07-23 — ⚠스케줄러 우편 전환·리셋 00시 재작업 예정, 가드는 구 스펙 검사 중) — DB불필요: 창 산술 28슬롯·2800 총합·B3 유예(만료+1일 [26,27]/만료+G+1일 [])·KST 04:00 리셋 보정·멱등키 user×pass×dayIndex·econ 상수 미러 + A/B(G=0 당일소멸 재현·리셋 0시 오프바이원·nonce 키 이중지급 검출). exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_1p1.ts)  # 월 1+1 순수(DIAMOND_PASS §3, 2026-07-23) — DB불필요: 월×팩 멱등키·R4 purchased_at 월귀속·R3 샌드박스 스코프·환불 월키 미복구 + A/B(txn 섞인 키=월1회 붕괴·처리시각 귀속=경계 오귀속·스코프 무시=실 월키 소진 검출). exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_pass_live.ts)  # 다이아 패스+1+1 라이브(DIAMOND_PASS §2~4, 2026-07-23 — ⚠스케줄러 우편 전환 재작업 예정) — dev DB: B4 day-0 즉시지급·confirm dedup·일일수령 멱등·B3 유예·B1 tombstone(환불 선착→구매 후착 활성화 금지)·B2 클로백 단일 트랜잭션(claim↔환불 동시 레이스 Σ 정합)·1+1(2배/재구매 base만/월키 미복구)·Q1 큐잉(깊이 1·만료 후 활성화)·PROMO_1P1_ENABLED 게이트 A/B·테스트 유저 자동정리. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_mail.ts)  # 우편함 순수(MAILBOX §3~4, 2026-07-23) — DB불필요: 멱등키 빌더(userId 포함)·만료 경계·MAIL_MAX_GRANT 캡·상태분류(안받음/받음/만료/회수)·패스 첨부 만료 60일 파생·상수 미러 + A/B(30일 뮤턴트·경계 오프바이원·캡 미적용·userId 누락 키·회수 필터 누락 검출). exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_signup_notify.ts)  # 신규 가입 디스코드 알림 2티어(BACKEND §13.28, 2026-08-07) — ①순수: notifySignup 계약(env 미설정 no-op·폴백 체인·전용채널 우선·임베드 내용·**PII 부재**(providerId/userId 원문/이메일 미전송)·전송 실패·행에도 throw 0) ②라이브(dev DB): `/api/auth/login` 직접 호출 — **신규 가입 정확히 1발·재로그인 0발**·디스코드 500이어도 로그인 200(가입 비차단)·countSignups는 실가입만(ensureUser 행 제외). A/B: "재로그인 0발" 오라클이 허위가 아님을 증명하려고 **잘못된 부착 지점**(ensureUser 자리) 변이를 하니스에서 재현해 같은 오라클이 1발로 FAIL 뒤집히는지 확인. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_heartbeat.ts)  # 접속 핑(하트비트) 2티어(BACKEND §13.29, 2026-08-08) — ①순수(DB불필요): 라우트 계약(무토큰/위조 401·`force-dynamic`·GET 미노출·**경량 계약 소스 린트** = getWallet/walletLedger/passStatus/mailCounts/balance/ledger 심볼 0 — 빈도 높은 UPDATE 1건 경로에 지갑 조회가 끼어드는 회귀 차단) + 클라 `sendHeartbeat` 5성질(URL 미설정·미로그인 no-op·**60초 최소 간격**·응답 미파싱 json()/text() 0회·반환 undefined=fire-and-forget·reject/401/500 전부 **throw 0 + logError 0**[errorSink·console.warn 양쪽 스파이] = 기기 진단 롤링 버퍼 무오염) ②라이브(dev DB, 없으면 SKIP): 인증 200 `{ok:true}`·**응답 키가 정확히 `['ok']`**·lastSeenAt 실갱신·계정 자동정리. A/B: 내장 양성대조 3종(getWallet 주입 변이를 같은 린터가 검출·1번째 발사는 관측됨=카운터 비blind·**같은 실패 fetch를 기존 `call()` 경로로 태우면 errorSink/console.warn이 실제로 울림**=스파이 유효) + 일회성 변이 3종 실측(60초 가드 제거→C3 FAIL·`call()` 재사용→C4·C5 FAIL·라우트 getWallet 주입→A5 FAIL). exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_kst.ts)  # 운영 지표 시간대 KST 계약(BACKEND §13.15, 2026-08-08) — **DB 불필요 순수**: 시간대 오류는 **화면이 멀쩡해 보여서**(막대도 그려지고 숫자도 나오는데 9시간 밀림) 운영자가 이용 시간대를 정반대로 읽는다. ①`kstHour`(자정 넘김 랩어라운드 포함) ②`kstYmd`(**UTC 00:00~09:00 구간**이 실제 버그 지대 — 15:00Z가 KST 다음날) ③`kstDayStart`(오늘 00:00 KST = 전날 15:00Z) + 불변식 0≤now−dayStart<24h ④배선(stats `kstDayStart` 경유·구 `setUTCHours(0,0,0,0)` 부활 금지 / adminStats `kstHour`·`getUTCHours` 잔존 0 / series 버킷을 KST 벽시계에서 계산) ⑤**읽기·쓰기 대칭**(statsDaily 라이터도 KST — revenuecat `kstYmd`, retention SQL `AT TIME ZONE 'Asia/Seoul'`). A/B: `--mutant`(오프셋 0=UTC 가정) → ①②③ **8건 FAIL**. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_opsrefresh.ts)  # 운영 콘솔 새로고침 계약(BACKEND §13.15, 2026-08-08) — **DB 불필요 순수 소스검사**: 이 버튼의 고장은 **조용해서**(눌러도 무반응인데 화면은 멀쩡) 운영자가 낡은 숫자를 최신으로 착각한다. ①`api`가 `nonce` 의존(=탭 17곳의 `useEffect([api,…])`를 한꺼번에 재조회시키는 유일한 지렛대) ②새로고침이 nonce를 올림 ③`load()`를 **await한 뒤** 토스트(데이터 도착 전 '완료' 거짓신호 금지) ④최초 로드 1회 가드(없으면 nonce마다 공통 6종 이중 호출) ⑤버튼 진행표시(disabled·라벨전환·폭고정 118px) ⑥연타 차단 ⑦탭의 `[api]` 의존 개수 ≥15(줄면 새로고침이 그만큼 안 닿음) + 구 형태 `onClick={() => { load(); flash(…) }}` 부활 금지. A/B: `--mutant`(nonce 의존 제거) → ① FAIL로 뒤집힘. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_authexpire.ts)  # 죽은 토큰 → 강제 로그아웃(AUTH §3.4, 2026-08-08) — **DB 불필요 순수**: ①Bearer 실은 401 → 콜백 1회 발화 + bearer 즉시 무효화 ②**Bearer 없는 401은 미발화**(익명 401은 '로그인 필요'라는 정상 응답 — 여기서 세션을 지우면 최악) ③**offline·타임아웃·5xx·200 전부 미발화**(네트워크가 나빠서 로그아웃되는 오작동 차단) ④연속 401에 발화 1회 + 2번째부터 Authorization 헤더 미탑재 ⑤배선(useAuthStore: 핸들러 등록·sessionExpired를 **signOut 전에** 세움·로그인 성공 시 해제·이미 로그아웃이면 no-op) ⑥배선(LoginScreen이 이유 표시 + '구단은 보존' 고지 — 데이터 소실 오해 차단). 토큰 사망 경로 3종(180일 TTL·시크릿 회전·서버측 계정삭제) 공통 방어라 **회전과 무관하게 상비**. A/B: `--mutant`(bearer 유무 무시하고 무조건 발화) → ② FAIL로 뒤집힘. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_internal.ts)  # 내부(운영자) 계정 지표 제외(BACKEND §13.30, 2026-08-08) — **DB 불필요 순수**: ①`lib/adminStats.aggregateUsers`가 내부 계정을 **정확히 그만큼** 뺀다(혼합 입력 제외 결과 == 실유저 전용 입력 결과 — 총가입·DAU·WAU/MAU·active30m·비활성·14일 시계열 2종·시간대 분포·리텐션 D1~D30 전부 등호) + 고지 카운트 `internalExcluded` ②`includeInternal=1` 원복(총6·DAU4·excluded 0) ③**배선 봉인**(stats 라우트가 lib/adminStats를 import·aggregateUsers 호출·includeInternal 파싱·원장은 isExcludedUser 경유 + **인라인 `totalUsers++` 루프 부활 금지** / users 라우트 PATCH·internal 컬럼) ④`isExcludedUser` 계약 3종. **양성대조**: 실유저 기준값이 비어있지 않음(총4·DAU2·비활성1·D1 non-null) — 전부 0이면 ①의 등호가 공허하게 참이 된다. A/B: `--mutant`(내부 플래그 무력화) 실측 → ① **8건 FAIL로 뒤집힘**(가드가 실제로 보고 있음을 자기 고발로 증명) + **배선 변이 실측**(achievements에서 internalScope import 제거 → ③ FAIL, 복원 후 PASS). **Phase 2(2026-08-08)**: ③이 집계 라우트 **4종(stats·achievements·series·bm)** 전수로 확장 — 각 라우트가 공용 `lib/internalScope`를 import·`internalScope(req)` 호출·`internalMeta` 고지·실제 제외 적용을 하는지 + **자체 internalIds Set 생성 금지**(스코프 진실 단일화) + **users 목록은 제외하지 않음**(설계 경계 — 개별 레코드는 다 보여준다)를 함께 봉인. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_ticket_anon_live.ts)  # 익명 문의 라이브(`/api/ticket/anon` — 멀티프로젝트 수신, 2026-08-06) — dev DB·라우트 직접 호출: allowlist 게이트(미설정·미허용 proj=404)·정상 왕복(줄바꿈/이모지)·익명 유저 1행 재사용·입력 검증(5자 미만 400·이상 category→etc·본문 상한)·24h 총량 캡 429·응답에 ticketId 미노출·osVersion 미수집·**기존 `/api/ticket`은 여전히 무토큰 401**(익명 추가가 기존 경로를 열지 않음). A/B: 404 검사마다 허용 proj 대조군 동반(허위 오라클 배제). 정리는 이번 실행이 만든 proj만. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_mail_live.ts)  # 우편함 라이브(MAILBOX §5·§8~9, 2026-07-23) — dev DB: 발송 idem dedup(더블클릭 이중 발송 0)·상태 필터 3종·수령 단일tx·이중수령 0·만료/회수 후 미수령·패스 첨부 원자성(B1 — 롤백 시 패스 행 0)·큐 만석 rejectOnQueueFull 분기(우편 롤백/구매 overflow A/B)·브로드캐스트 lazy 멱등·음수 잔액 수령·admin fail-closed·관측행 3종·모두받기 집계·테스트 데이터 자동정리. exit 0/1
...
(cd server && node_modules/.bin/tsx tools/_dv_admin_scope_live.ts)  # 관리자 write proj 스코프 가드(BACKEND §13.17 R1~R3, 2026-07-24) — 라이브 dev DB: 타 proj 티켓 환불 404+**전액 롤백**(지갑 차감·원장까지 되돌림 — 부분 성공 금지)·미존재 티켓 404(허위 ok 금지)·**타 proj 유저 지갑 차감/지급 차단**(우리 원장에 남 게임 거래 0행)·balance=Σledger 실측·쿠폰 POST/PATCH·우편 대상 유저 proj 스코프(400 no-such-user) + 민감도 A/B(우리 proj엔 200·차단된 대상은 DB에 실재). 자기 생성 id만 정리. exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_ticket_live.ts)  # 문의(티켓) 라이브 가드(BACKEND §13.17·§13.2, 2026-07-24) — 라이브 dev DB: 등록→유저GET→관리자GET 왕복 일치(줄바꿈·이모지)·**타 proj 티켓 답변 404+DB 무변화**(T1)·미존재 uuid 404(허위 ok 금지)·**타 proj 스냅샷 404**(T2 세이브 유출)·상태 어휘(reviewing/answered)+repliedAt 노출·목록 proj 스코프·fail-closed 인증 9항목·빈 상태 200 [] + 스코프 A/B 민감도(같은 호출이 우리 proj엔 200·막힌 스냅샷은 DB에 실재). 자기 생성 id만 정리. exit 0/1
