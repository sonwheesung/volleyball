...
| [ATTENDANCE_PASS_SYSTEM](./ATTENDANCE_PASS_SYSTEM.md) | **다이아 출석 패스 + 월 1+1(2026-07-23 신설, 사용자 확정)** — 패스(₩9,900·28일·매일 100💎, 소비성·자체 `attendance_passes` 진실·KST 04:00 리셋·자동 수령+토스트·유예 3일·중첩 큐잉·환불 전액 클로백 B1~B4 봉인) + 월 1회 1+1(기존 6팩 서버 2배 지급·월키 멱등·환불 시 월키 미복구). 무료 출석 보류. 독립 리뷰 반영, 미결=Q4 법률자문뿐. 구현 Phase①서버→②앱→③실 SKU(#43 뒤) | `server/db/schema.ts`·`server/app/api/pass/`(예정)·`lib/iap.ts`·`app/buy-diamonds.tsx` |
...
(cd server && node_modules/.bin/tsx tools/_dv_pass.ts)  # 출석 패스 순수(ATTENDANCE_PASS §2, 2026-07-23) — DB불필요: 창 산술 28슬롯·2800 총합·B3 유예(만료+1일 [26,27]/만료+G+1일 [])·KST 04:00 리셋 보정·멱등키 user×pass×dayIndex·econ 상수 미러 + A/B(G=0 당일소멸 재현·리셋 0시 오프바이원·nonce 키 이중지급 검출). exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_1p1.ts)  # 월 1+1 순수(ATTENDANCE_PASS §3, 2026-07-23) — DB불필요: 월×팩 멱등키·R4 purchased_at 월귀속·R3 샌드박스 스코프·환불 월키 미복구 + A/B(txn 섞인 키=월1회 붕괴·처리시각 귀속=경계 오귀속·스코프 무시=실 월키 소진 검출). exit 0/1
(cd server && node_modules/.bin/tsx tools/_dv_pass_live.ts)  # 출석 패스+1+1 라이브(ATTENDANCE_PASS §2~4, 2026-07-23) — dev DB: B4 day-0 즉시지급·confirm dedup·일일수령 멱등·B3 유예·B1 tombstone(환불 선착→구매 후착 활성화 금지)·B2 클로백 단일 트랜잭션(claim↔환불 동시 레이스 Σ 정합)·1+1(2배/재구매 base만/월키 미복구)·Q1 큐잉(깊이 1·만료 후 활성화)·PROMO_1P1_ENABLED 게이트 A/B·테스트 유저 자동정리. exit 0/1
