...
| [ROTATION_MORALE](./ROTATION_MORALE_SYSTEM.md) | **선발 휴식(순위 기반 로드매니지먼트) + 벤치 사유 인지 + 감정=f(사유,성격)** SPEC. 확정 후 구현 | `engine/lineup.ts`·`data/owner.ts`·`engine/owner.ts`(예정), `tools/simStarters.ts` |
...
npx tsx tools/simMood.ts                    # 선수 심리(벤치 사유 귀속·부상자 불만없음·성격/기대치별 기분 A/B)
