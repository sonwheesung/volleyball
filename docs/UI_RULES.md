...
>
> **← 폐지 정정(2026-07-31, 사용자 결정 — 뒷단 체인 완전 제거)**: ~~위 "뒷단 체인 잠금 유지"~~는 **무효**다. 뒷단 체인(`enshrine`→`training-camp?chain=1`→`season-opening`)과 개막 브리지 화면(`app/season-opening.tsx`)을 **삭제**하고, 뒷단도 앞단처럼 **일정 탭 순차 단일 카드 허브**(SEASON_SYSTEM §5.6.5: 헌액→전지훈련→개막전으로)로만 그린다 — 전부 뒤로가기 정상. 헌액(열람)·전지훈련(1회 상호작용)은 되돌아가도 누수될 소비 상태가 없고 `enshrineSeenSeason`·`campDoneSeason` 시즌-키 마커로 멱등이라 잠금이 불필요. **UI-28은 이제 어떤 오프시즌 화면에도 적용되지 않는다.** ⚠ 단 `app/season-start.tsx`의 **로딩 중 back-lock**(하드백/POP 무력화)·**스택 리셋**(#113)은 별개 — 체인 잠금이 아니라 무거운 `endSeason` 동기 블록 보호라 **유지**된다(체인 규칙과 무관). 상비 가드 `_dv_hub` [C]블록이 체인 잔재 재유입을 봉인.
...
> **정정(2026-07-28, 테스터 — 일정 탭 표시만 순차화)**: 일정 탭은 5단계를 나란히 열지 않고 **"다음 경기" 카드처럼 한 번에 한 업무 버튼**(`OO 하러 가기 →`)으로 `offseasonStep`(forward-only) 커서를 따라 진행한다. ~~+ `이 업무 건너뛰기 →`~~ **재정정(2026-07-28, 사용자 "무조건 다 하게")**: 건너뛰기 링크 제거 — 각 화면을 눌러 거쳐야 다음이 뜬다. **소프트락 안전판은 `하러 가기`의 순서**(`advance()`를 `openStep()`보다 먼저 호출 → 화면이 render throw해도 커서가 이미 전진 + ErrorBoundary가 일정 탈출) — 건너뛰기 버튼 없이도 보장. **트레이드오프**: 일정 탭에서 지나친 업무 재방문 불가(office 탭도 오프시즌 라우트 미노출). 잠금 없음·최종 버튼 상시 노출·`preOffseasonSteps()` 라우트/순서/개수는 불변. 상세 SEASON_SYSTEM §5.6 정정.
> **정정(2026-07-29, 테스터 — 뒷단도 순차 단일화 + 첫 시즌 명전 숨김)**: ~~뒷단 카드=헌액·전지훈련(나란히 목록, 규칙 ③)~~ → 뒷단도 앞단처럼 **순차 단일 카드**(SEASON_SYSTEM §5.6.5): 현재 스텝 하나만 `OO 하러 가기 →` + **[개막전으로 →] 상시 노출**(규칙 ①, `onOpenSeason` 확인 유지). 현재 스텝은 순수 함수 `postHubCurrentStep(campDone, hasHonorees, enshrineSeen)`. **헌액은 대상자 없으면 목록에서 숨김** — `hasEnshrineHonorees(hallOfFame, season)`(=`legend && retiredSeason===season-1`)가 false면 `postOffseasonSteps`가 헌액 스텝을 빼고 전지훈련을 n:1로(테스터: "첫 시즌인데 명전이 왜 뜨냐" — season0은 항상 false). ~~enshrine 0명 진입 시 '헌액자 없음' 한 장(2026-07-08)~~ 화면 로직은 안전판으로 남기되 **허브 진입 경로가 사라짐**(정상). 헌액 진행 마커 = 새 영속 필드 **`enshrineSeenSeason`**(`campDoneSeason` 미러, 기본 −1, 시즌-키라 자동 리셋 · `partialize`+`SAVE_DEFAULTS` 등재) — 헌액 hub 완료(`useOffseasonExit` 경로)에서 `markEnshrineSeen()`. reload/뒤로가기 멱등(마커 미세팅 시 헌액 유지, 소프트락 아님 — 개막은 항상 가능). ⇒ 규칙 ④ "신규 영속 필드 0"은 이미 `offseasonStep`(2026-07-28)·이제 `enshrineSeenSeason`으로 완화됨(둘 다 시즌-키 진행 마커, 진행 중 세이브는 DEFAULT로 호환).
...
> **규칙**:
> ① **진입 차단 없음 · 완료 잠금 없음** — 어느 단계든 언제나 열리고, 앞 단계를 안 봐도 다음이 열린다. 최종 버튼(새 시즌 시작하기 / 개막전으로)은 **항상 노출**(완료 게이트를 걸면 그게 새 소프트락이 된다). 안 본 단계가 있으면 **카드 안내 한 줄 + `showAlert` 확인**만(새 Modal 금지 — iOS 모달 레이스 #129).
> ② **✅/🔒 3상태 금지** — 앞단 6단계는 전부 미리보기라 "완료" 판정이 데이터에 없고, 아무것도 안 하고 나오는 게 정상 완주다. **데이터로 진짜 판정되는 것만** 체크: 전지훈련(`campDoneSeason===season`)·시상식(`archive[season].championId`).
> ③ **두 위상은 다른 목록** — `endSeason` 앞(`action.kind==='seasonOver'`, `currentDay≈183`)과 뒤(`currentDay===0`, season=S+1)는 일정 탭에서 **완전히 다른 분기**로 렌더된다. 앞단 카드=결산·외국인·아시아쿼터·FA·드래프트+[새 시즌 시작하기], 뒷단 카드=헌액·전지훈련+[개막전으로]. 하나의 카드로 그리지 않는다.
> ④ **신규 영속 필드 0** — 진행 중 세이브 그대로 호환.
> ⑤ **`endSeason` 진입점은 공용 훅 하나**(`lib/seasonStart.ts` `useSeasonStartEntry()`) — 진입점이 3개(draft·draft-live·일정 허브)가 되며 `showSeasonStartAd`가 중복 노출될 수 있다. **모듈 레벨 래치**(화면 로컬 `useRef` 아님 — 화면이 다르면 ref가 공유 안 됨)로 동기 차단하고, `store.endSeason`의 `planNextAction!=='seasonOver'` 즉시-return이 **최종 방어선**이다(§6 진행 게이트).
> ⑥ **하류 파생 무효화 대칭** — 허브에선 "드래프트 확정 후 FA/외인으로 되돌아가기"가 상시 가능하다. `draftSelections`(라이브 확정 픽)를 무효화하는 상류 레버는 **FA·재계약뿐 아니라 외국인/아시아쿼터 결정까지 전부**여야 하고(외인 영입은 전 구단 로스터 인원을 바꿔 AI 지명/패스 판정을 바꾼다 — `engine/draft.ts` `neededPositions`), **확정 픽이 1건 이상일 때만 경고 후 삭제**한다(0건이면 조용히 통과 — 소음 금지). 공용 `components/draftPickGuard.ts` `confirmDraftPickReset()`.
> ⑦ **ErrorBoundary 병행** — 허브는 증상 우회일 뿐이라, 원인(화면 throw = 앱 프로세스 사망, EC-UI-04)은 **route-level `export const ErrorBoundary`**(expo-router 6)로 봉인한다. 폴백은 반드시 **일정으로 나가는 버튼**을 제공하고 에러를 `diag`(#44 진단 스냅샷)에 남긴다. 루트 `app/_layout.tsx`에도 전역 폴백.
> ⑧ **`season-start` 스택 리셋 유지** — `app/season-start.tsx`의 `navigation.reset` (#113 병목 102.8→23.2s)은 허브 도입 후에도 **지운다는 판단 금지**(허브에선 대개 스택이 이미 얕아 no-op처럼 보이지만, 결산→…→드래프트를 push로 훑고 온 세션·구세이브 경로에선 여전히 유효).
...
> **UI-51 (2026-07-28 — 아바타 시트 무재시도 박제 = onLoad 전 백오프 remount)**: 테스터 실기기 제보 — **처음 게임 진입 시 선수 4명을 제외한 나머지 얼굴이 ~10분간 아이콘(빈 틴트)으로 남는다**. UI-45의 프리워밍은 **디코드 시작 시점**을 앞당길 뿐, 첫 진입에 여러 시트가 동시에 디코드 경쟁을 벌일 때 **경쟁에서 진 시트가 blank로 박제**되는 별개 문제를 못 막는다. **원인**: RN `<Image>`는 로컬 `require()` 자산이라도 **디코드 실패/메모리 압박으로 못 그리면 스스로 재시도하지 않는다** — 처음 성공한 소수(제보의 "4명")만 뜨고, 진 나머지는 리렌더 트리거가 없어 그대로 멈춘다(캐시가 채워지는 다른 화면 방문 전까지 = 체감 "10분"). **처방**: `PlayerAvatar`의 얼굴 크롭을 **`FaceCrop`** 서브컴포넌트로 분리 — `onLoad` 전까지 지수성 백오프(`1000 + gen*700`ms)로 **`key={gen}` 범프 → Image remount → 재디코드**를 유도하고, `onError`도 즉시 gen 범프. 로드되면(`loaded`) 멈추고, 상한 `FACE_MAX_RETRY=8`로 무한 churn 방지(그 무렵 다른 시트 디코드가 풀려 재시도가 성공). 새 네이티브 모듈 0·결정론 무관(표시 레이어). **UI-45(워밍)와 병행**: 워밍은 첫 표시를 빠르게, UI-51 재시도는 실패분을 반드시 회복 — 둘 다 필요. **원칙**: 크롭형 `Image`(시트 기반 아바타)는 실패-무재시도 박제를 항상 재시도 게이트로 감싼다(raw `<Image>` 크롭 직접 사용 금지 → `FaceCrop` 경유). 검증: tsc0 · 시각은 에뮬(첫 진입 전원 표시 확인).

> **UI-52 (2026-07-30 — OvrBadge 이중 스트레치 봉인)**: 일정 화면의 예상 선발 OVR이 선수 상세(`player/[id].tsx` = 정본 `overallRaw(p)`)와 어긋나고 엘리트팀은 전부 99로 부풀려짐(사용자 보고: 인천 예상선발 per-player 평균 98인데 카드는 99). **원인**: `components/Screen.tsx OvrBadge`는 **내부에서 `displayOvr(value)`를 1회 적용**하는 게 계약(호출부는 항상 raw 연속값을 넘긴다 — 앱 전체 정본 호출부 draft/fa/contracts/player detail/transactions/index/team/exhibition은 모두 `overallRaw(p)`·`teamOverallRaw(...)` 준수). **유일 위반 2곳이 `app/(tabs)/schedule.tsx`**: ① 모달(상대 라인업 per-player)이 `displayOvr(overallRaw(p))`를 넘겨 **이중 스트레치**(상한 99에 박힘) — `overallRaw(p)` raw로 교정해 선수 상세와 동일 정본 OVR. ② 카드(우리/상대)가 `preview.myOvr/oppOvr`(=`lineupOvr` = `round(avg of displayOvr(overallRaw(p)))` = **이미 표시 스케일 평균**)를 넘겨 OvrBadge가 또 displayOvr → 이중 → 99. **처방**: OvrBadge에 `preScaled` prop 추가 — `preScaled=true`면 value가 이미 표시 스케일이라 재변환 없이 `Math.round`만(색 임계 88/80/74·링은 표시 스케일 기준이라 그대로 유효). 카드 두 곳만 `preScaled`로, `lineupOvr`는 그대로(이제 반환값이 수정된 모달 per-player 정본 평균과 정확히 일치). 기존 raw 호출부는 `preScaled` 기본 false라 **동작 불변**(회귀 0). 표시 전용·결정론 무관. **원칙**: OvrBadge 호출부는 raw 연속 OVR(`overallRaw`/`teamOverallRaw`)만 넘긴다 — 이미 `displayOvr`된 값(평균 등)을 렌더할 때만 `preScaled`. **사각 분석**(왜 이전 검증이 못 잡았나): 일정 OVR을 에뮬로 "라인업 평균과 일치" 확인할 때 손계산 평균을 화면 숫자에 맞췄으나 **모달 per-player 자체가 이미 이중 스트레치**라 "스케일이 옳은지"를 검증하는 독립 오라클이 없었다(displayOvr 적용 횟수 = 검사 사각). 교훈: OvrBadge 렌더값은 "정본 호출부(overallRaw/teamOverallRaw)와 같은 스케일인지"를 대조해야 함. 검증: `tools/_dv_ovrbadge.ts`(preScaled 왕복 항등·schedule 호출부 계약 정적 체크·A/B) · tsc0 · 시각은 에뮬(메인 세션).
...
> **정정(2026-07-28, 테스터 — "1자리 소수"도 코보 관례 아님 → 정수 출전경기수로 근본 해결)**: `fmtMatches`는 float 잔여만 가렸을 뿐 **"35.8경기" 같은 소수 자체가 코보에 없다**(경기수=GP는 정수, 어떤 출전이든 1경기; 부분 출전은 세트수로 별도 집계). **근본 원인**은 `ProdLine.matches`가 표시용 경기수가 아니라 **XP·연봉·팬심·시상 자격에 물린 "코트타임 가중 참여량"**(작전/피로 교체를 `subUse/40`로 분수 적립)인데 그걸 화면에 그대로 썼다는 것. **처방**: 밸런스 결합값 `matches`(분수)는 엔진용으로 **그대로 두고**(XP `experience.ts`·연봉 `salary.ts`·팬심 `owner.ts`·시상 `awards.ts` byte-불변), **표시용 정수 필드 `ProdLine.gamesPlayed`(출전=1, 중복 없음)를 신설**해 모든 경기수 표시가 그걸 쓴다. `career.matches`·`SeasonLine.matches`도 `Math.round(분수)`(교체 선수 과소계상) → `gamesPlayed` 합으로 전환(진짜 GP, 밸런스 무영향·기록용). `fmtMatches`는 잔여 안전망으로 존치(이제 항상 정수). **세트수(sets)는 미도입** — `SimResult`에 선수별 세트 출전 데이터가 없어(subUse=랠리 수) 정직한 집계 불가, `match.ts` 세트 출전 추적 신설이 선행돼야 함(추정 금지). 정본 SALARY_SYSTEM §1(생산 귀속). 가드 `tools/_dv_gp.ts`(gamesPlayed=출전 정수·matches 분수와 독립·A/B).
...
- **"업데이트 적용 중" 로딩(부팅 OTA, 2026-07-29)**: 부팅 시 OTA 자동적용(패치 로딩)의 fetch 구간엔 `<Loading variant="brand" message="업데이트 적용 중" />`를 띄운다(`components/BootGate.tsx`, reload 임박·dismiss 불가). 브랜드 연출(UI-6 — 콘텐츠 모양 없는 긴 대기)을 재사용. **check 구간엔 안 띄운다**(대개 "최신"이라 매 부팅 패치로딩 남발 방지) · 업데이트 없으면 스플래시 없이 즉시 진입. 설계 정본 AUTH_SYSTEM §4.1.
