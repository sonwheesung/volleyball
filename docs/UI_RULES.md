...
> **정정(2026-07-28, 테스터 — 일정 탭 표시만 순차화)**: 일정 탭은 5단계를 나란히 열지 않고 **"다음 경기" 카드처럼 한 번에 한 업무 버튼**(`OO 하러 가기 →` + `이 업무 건너뛰기 →`)으로 `offseasonStep`(forward-only) 커서를 따라 진행한다. **소프트락 안전판은 `건너뛰기`**(크래시 화면도 건너뛰고 최종 버튼 도달) — 방사형 재접근이 아니다. **트레이드오프**: 일정 탭에서 지나친 업무 재방문 불가(office 탭도 오프시즌 라우트 미노출). 잠금 없음·최종 버튼 상시 노출·`preOffseasonSteps()` 라우트/순서/개수는 불변. 상세 SEASON_SYSTEM §5.6 정정.
