...
| 상태 필터 | ~~**전체 / 받음 / 안받음**~~ → **전체 / 안받음 / 받음** 3탭(표시 순서 정정 2026-07-23 사용자 피드백), **서버 재조회**(status 쿼리 파라미터) | 기본 탭 = ~~받음~~ → ~~안받음~~ → **전체**(사용자 재정정 2026-07-23 — 전체 우편을 한눈에. 확정값) |
...
- **상태 필터 3탭**: ~~`전체 / 받음 / 안받음`~~ → `전체 / 안받음 / 받음`(표시 순서 정정 2026-07-23 사용자 피드백) — 탭 전환 시 **서버 재조회**(`GET /api/mail?status=`). **기본 선택 = "전체"**(사용자 재정정 2026-07-23, 확정 — ~~"받음"~~ → ~~"안받음"~~ → "전체". 전체 우편을 한눈에). 첫 진입 시 전체 목록 조회.
...
- **★ 실패는 실패로 — 빈 목록으로 위장 금지(2026-07-24, BUG-01 배치 EC-UI-05)**: ~~오프라인이 아닌 실패(500·401·bad-request)는
  `setItems([])`로 뭉개져 **"우편이 없어요"로 표시**~~ → **`loadError` 상태 분리**. 세 갈래를 각각 다르게 보여준다:
  ① **서버 오류** = 경고색 배너 "우편을 불러오지 못했어요. 잠시 후 다시 시도해 주세요." + **"다시 시도"**(같은 탭 재조회),
  빈 화면 문구·아이콘도 경고형, 받기/모두받기 비활성("지금은 받을 수 없어요"), 캐시가 있으면 목록은 그대로 둔다(정보 손실 방지).
  ② **오프라인** = 기존 구름 배너(+"다시 시도" 추가), 캐시 표시. ③ **진짜 빈 목록** = 기존 빈 상태 카피(위 S5).
  원칙: 장애를 "없음"으로 읽히게 하면 유저가 운영 보상 미지급을 정상으로 오인한다. 오프라인 처리 패턴은 `app/support.tsx`와 동일 결.
...
| # | 결정 | 확정값 | 반영 |
|---|---|---|---|
| Q1 상태 필터 기본 탭 | ~~받음~~ → ~~안받음~~ → **전체**(사용자 재정정) | 전체 우편을 한눈에 | §0·§6.1 |
