...
## 1. 무엇 / 왜
- `engine/`·`data/` **순수 TS**를 esbuild로 브라우저 번들 → 14탭으로 엔진을 직접 실행·검증.
...
| 탭 | 엔진/셀렉터 |
|---|---|
| 경기 | `simulateMatch`(+`box` 싱크) (박스스코어: 공격 성공/시도/성공률·서브·블록·디그·세트·리시브·범실 — 실제 스윙 단위, 네이버 종합 탭 스타일 / N경기 승률·세트 분포) |
