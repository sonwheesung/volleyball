...
## 1. 무엇 / 왜
- `engine/`·`data/` **순수 TS**를 esbuild로 브라우저 번들 → 17탭으로 엔진을 직접 실행·검증.
...
| 경기 중계 · 현수막 | `matchMvp`(경기 MVP+리캡)·`situationFeed`(상황 인지 중계 — 세트/매치포인트·듀스·연속)·`buildMatchBanners`(중계 현수막: 기록 경신·트리플 크라운·PO 확정/탈락) + **5세트 코인토스 미리보기**(MATCH_SYSTEM v2.1 오버레이 룩 — 양 결과). 경기 분석 / 시즌 전 경기 현수막 스캔 / 현수막 4종 미리보기 / 코인토스 미리보기. 게이머 리뷰 1·2 + BROADCAST_SYSTEM |
| 🏆 우승 화면 | 우승 순간 축하 화면 **목업**(연출 미리보기) — 얼굴 없는 블롭 선수 일러스트(벡터 SVG, 앱은 `react-native-svg`) + 팀명·N시즌 챔피언·MVP. **가운데 선수·배경·시즌 텍스트가 우승팀 색**(`lib/teamColor.teamColors` — `CLUB_IDENTITY` 실제 구단 hue, 2026-06-26 연결. 자체 hue 재구현 제거). 팀 바꾸면 그 구단 시그니처 색 따라옴 |
