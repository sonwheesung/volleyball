...
| 🏆 우승 화면 | 우승 순간 축하 화면 **목업**(연출 미리보기) — 얼굴 없는 블롭 선수 일러스트(벡터 SVG, 앱은 `react-native-svg`) + 팀명·N시즌 챔피언·MVP. **가운데 선수·배경·시즌 텍스트가 우승팀 색**(현재 팀 색 데이터 없어 팀 id 해시로 결정론 생성 — 추후 `CLUB_IDENTITY`에 실제 색 추가 시 연결). 팀 바꾸면 색 따라옴. 일러스트 정식 통합 전 룩 합의용 |
