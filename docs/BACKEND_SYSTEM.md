...
- **디스코드 결제 알림(2026-07-05, 사용자 요청)**: 실제 지급된 결제/환불(applied=true)을 디스코드 채널로 통지(`server/lib/notify.ts` `notifyPurchase`). **정확히 1건**(웹훅·confirm 중 원장에 반영된 쪽만 — dedup된 재시도/폴백은 알림 없음). `after(() => …)`로 **응답 후** 전송(웹훅 200 지연 0). `DISCORD_WEBHOOK_URL` 미설정=완전 no-op. PII 금지(userId 뒤 6자만 마스킹). 임베드=상품·다이아·₩금액·환경·경로·유저. 활성화(사용자 몫): 디스코드 웹후크 URL 생성 → Vercel env `DISCORD_WEBHOOK_URL`.
