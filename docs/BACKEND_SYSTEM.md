...
### 13.3 엔드포인트
`/api/health` · `/api/auth/login`(ID토큰→Bearer)·~~`/refresh`~~(**미구현** — 정정 2026-07-31 doc↔code 감사, `auth/`엔 `login`만) · `GET /api/wallet`(balance+최근 원장) · `POST /api/wallet/spend`·`/earn`(멱등키+**행 잠금 트랜잭션**)·`/earn-batch` · ~~`POST /api/purchase/verify`·`/webhook/google`(RTDN)·`/apple`(ASSN)~~ → **정정(2026-07-03 §13.18)**: `POST /api/purchase/webhook/revenuecat`(RC 웹훅, Authorization 시크릿 검증 → applyWalletTx purchase/refund)·`POST /api/purchase/confirm`(클라 폴백, storeTxnId → RC REST 재검증 → 같은 키 지급) · ~~`POST /api/ad/ssv`(AdMob 서명 검증 — RC 무관, 우리 몫) · `POST /api/log`~~(**둘 다 미구현** — 정정 2026-07-31 doc↔code 감사, #43 결제 이후 예정) · `/api/ticket`(create/list)·`/api/admin/ticket`(reply/snapshot) · `POST /api/snapshot` · `POST /api/telemetry` · `/api/admin/{coupon,announcement,setting,refund,mail,grant,devnote,bm,errors,payments,payment-events,series,stats,users,achievements}` · `/api/account` · `/api/devnotes` · `/api/bootstrap` · `/api/cron/{purge,pass-daily}` · `/api/save-backup` · 관리자 콘솔 페이지 `ops-9f3a2c`(인증 보호).
...
- **크론 보안**: `CRON_SECRET`을 **Vercel env에도 설정**해야 Vercel이 크론콜에 `Authorization: Bearer`를 자동 첨부하고 라우트가 검증한다(~~미설정 시 라우트는 통과시키되 무방비 — 출시 전 필수 설정~~ → **정정(2026-07-31 doc↔code 감사)**: **크론도 fail-closed** — `cron/purge`·`cron/pass-daily` 둘 다 `if (!secret || …) return 401`(2026-07-07 #7 교정, 구 `if(secret && …)` fail-open 폐기). CRON_SECRET 미설정이면 **전면 거부**(admin `isAdmin` 패턴 미러). 그래도 정상 동작 위해 출시 전 env 설정 필수). 스케줄 `0 18 * * *`(=3am KST) 일 1회(Hobby 허용).
...
- **우편함 수령 reason `mail`(2026-07-23 설계)**: 운영 보상·CS 개별 지급·이벤트 채널. 수령=`applyWalletTx(+amount,'mail',key='mail:<mailId>')`(전용 claim 라우트 — `/earn` 화이트리스트 밖, fail-closed)·통당 캡 `MAIL_MAX_GRANT`·보존 30일·`reason='mail'`은 매출/payer 집계 제외. 첨부 다이아·다이아 패스(grantPassTx 재사용). 다이아 패스 일일 지급도 우편함 스케줄러 발송(sender=`system:pass`·reason `pass_daily`, `docs/DIAMOND_PASS_SYSTEM.md` §2.3). 미확인 수는 `getWallet` unread/unclaimedMailCount 편입(별 폴링 금지). 정본 `docs/MAILBOX_SYSTEM.md`.
...
- **메뉴 ↔ URL 동기화(2026-08-04 사용자 요청)**: 사이드바 탭이 **URL 쿼리 `?tab=<id>`** 에 반영된다 — 딥링크(특정 메뉴 링크 공유·북마크)·**새로고침 시 그 메뉴 유지**·**브라우저 뒤로/앞으로로 메뉴 이동**. 구현: 마운트 시 `tabFromUrl()`로 URL 읽어 초기 탭 결정(잘못된 값→`overview`, SSR 안전 `window` 가드), 클릭은 `history.pushState`로 URL 갱신, `popstate` 리스너가 탭 동기화. 초기값은 하이드레이션 안전을 위해 `overview` 고정→마운트 useEffect에서 URL로 교정(authed 게이트 뒤라 깜빡임 없음). 순수 클라 History API — 서버·API 무관. 실브라우저 검증(딥링크→쿠폰 직접 진입·클릭→`?tab=anns`·뒤로가기→쿠폰 복귀·콘솔 에러 0).
- **파일**: `server/lib/admin.ts`(**`isAdmin`** — 구 문서 표기 "requireAdmin", 정정 2026-07-31 doc↔code 감사)·`server/lib/coupon.ts`·`server/lib/wallet.ts`·`server/db/schema.ts`·`server/app/api/coupon/redeem/route.ts`·`server/app/api/admin/{coupon,announcement,setting,stats,users,series,achievements}/route.ts`·`server/app/ops-9f3a2c/{page,layout}.tsx`(운영 콘솔 UI — 로그인·대시보드·사용자·결제·광고·업적·쿠폰·공지·설정·문의)·`lib/server.ts`·`app/coupon.tsx`.
...
- **티켓 서버(§B)**: `tickets`(id, proj FK, userId FK, category('bug'|'suggestion'|'question'|'etc'|**'refund'**), content, ~~status('open'|'replied'|'resolved'|'refunded')~~ → **정정(2026-07-24, 문서↔구현 드리프트)**: 실제 어휘는 **`open`(대기)|`reviewing`(확인 중)|`answered`(답변완료)|`refunded`(환불완료)** (+레거시 `replied`/`resolved` 허용 — `admin/ticket/reply`의 `STATUSES` 화이트리스트·콘솔 `StatusBadge`가 정본), reply, platform/osVersion/appVersion 제출스냅, createdAt, repliedAt) + `diagnostic_snapshots`(id, ticketId FK, snapshot jsonb, createdAt) **분리 테이블**(P0-4 — 10시즌 재생 JSON이 커서 목록 쿼리에 안 붙이고 상세 열 때 lazy load). 엔드포인트: `POST/GET /api/ticket`·`POST /api/snapshot`(ticket 소유권 확인)·`GET /api/admin/ticket`(필터)·`GET /api/admin/ticket/snapshot`·`POST /api/admin/ticket/reply`.
...
- **보관·통계**: `snapshot` 90일(진단 티어 — 3년 묵은 재생 JSON은 가치 0), 티켓 3년, `reason='refund'` 원장 5년(감사·retention 이미 제외). ~~**TODO(#43)**: `rollupRecent`가 purchase만 집계 → 실환불 웹훅 붙으면 순매출 과대계상, refund 차감 반영 필요.~~ → **해소(2026-08-07, §13.18 A2 — 실환불로 실증)**: `rollupRecent`는 **gross 건수·다이아 전용**으로 정의를 굳히고(크론이 원장에서 덮어쓰므로 여기서 환불 감산은 불가능·무의미), **순매출은 `revenueKrw`에서** `reverseRevenueKrwOnce`(환불 웹훅 경로, 원구매 적재액을 storeTxnId로 되찾아 차감·`revenue.krw.refund` 마커 멱등)가 담당한다. 관리자 수동 환불(이 절)은 storeTxnId가 없어 KRW 무관.
- **파일**: `server/db/schema.ts`(users 컬럼·tickets·diagnostic_snapshots)·`server/lib/auth.ts`(requireUserId)·`server/lib/wallet.ts`(applyWalletTx refund 음수허용)·`server/app/api/{auth/login,ticket,snapshot}/route.ts`·`server/app/api/admin/{ticket,ticket/reply,ticket/snapshot,refund}/route.ts`·~~`server/app/admin/page.tsx`~~ **`server/app/ops-9f3a2c/page.tsx`**(정정 2026-07-31 doc↔code 감사 — 관리자 콘솔 경로는 `ops-9f3a2c`, `app/admin/`는 미존재)·`server/lib/retention.ts`(snapshot 90일)·`lib/device.ts`(신, getDeviceInfo)·`lib/server.ts`(login+device·createTicket+device·refund 타입·'refund' 카테고리)·`store/useAuthStore.ts`(로그인 시 device)·`app/support.tsx`(환불 카테고리·안내 카피). 검증 `tools/_dv_refund.ts`(음수허용 reason파생·멱등키) + 라이브 E2E.

- **티켓 라우트 정정(2026-07-24, 발견·검증=Fable / 수정·문서=Opus 에이전트) — 공지 F1과 동일 결함 클래스**:
  - **T1(P0) 답변 proj 미스코프 + 허위 ok**: `POST /api/admin/ticket/reply`가 `where(eq(tickets.id, ...))`만으로 update — ①**타 게임(proj) 티켓에 답변이 실제로 기록**(status/reply/repliedAt 전부 반영, 실측 확인)되고 ②`.returning()`이 없어 **0건 매칭(미존재 uuid)에도 `{ok:true}`** → 운영자가 "답변 완료"로 오인. **교정**: `and(eq(projCode, PROJ_CODE), eq(id, ...))` + `.returning({id})` → 0건이면 `{ok:false,reason:'not-found'}` **404**(공지 DELETE/PATCH와 동일 패턴).
  - **T2(P0) 스냅샷 조회 proj 미스코프**: `GET /api/admin/ticket/snapshot`이 `ticketId`만으로 `diagnostic_snapshots`를 반환 → **타 게임 유저의 진단 스냅샷(세이브 통째, 수백KB)이 유출**. **교정**: 티켓을 선행 조회해 **이 proj의 티켓일 때만** 반환, 아니면 404. 스냅샷 쿼리 자체에도 projCode 조건(이중). 정상 경로 응답 모양은 불변.
  - **방어층**: `POST /api/snapshot`(유저 첨부) 소유권 검사에도 projCode 조건 추가 — `requireUserId`가 이미 proj 스코프라 **동작 변화 0**, 격리 의도를 코드에 명시하는 층.
  - **원칙(전 admin 라우트)**: **admin 라우트의 모든 write/read는 ①`projCode` 스코프 ②`.returning()` rowcount 확인 → 0건이면 404**. "성공 응답은 실제로 그 게임의 행 1건 이상을 건드렸을 때만" — 이 두 줄이 공지 F1(2026-07-06)·티켓 T1/T2(2026-07-24)의 공통 처방이다.
  - **왜 배터리 초록 속에 잠복했나**: `server/tools/`에 **티켓을 다루는 라이브 가드가 0개**였다(공지·쿠폰·노트만 존재). 공지에서 잡은 결함 클래스를 **형제 라우트로 전수 확장하지 않은 것**이 근본 원인(TEST_METHODOLOGY 등재).
  - **가드**: `server/tools/_dv_ticket_live.ts` 상설(등록→유저GET→관리자GET 왕복 일치[줄바꿈·이모지]·타 proj 답변 404+DB 무변화·미존재 uuid 404·타 proj 스냅샷 404·상태 어휘 왕복[reviewing/answered + `repliedAt` 노출]·목록 proj 스코프·fail-closed 인증 9항목[무토큰/유저토큰/오타·잘린 admin]·빈 상태 200 `[]` + 각 스코프 검사의 A/B 민감도 대조).
  - **`repliedAt` 유저 노출(P2)**: `GET /api/ticket`(유저) select에 `repliedAt`이 빠져 앱이 답변 시각을 못 보여줬다(관리자 API엔 이미 있었음) → select·`lib/server.ts` 타입·`app/support.tsx` 답변 카드(공지·개발자노트와 동일한 `YYYY.MM.DD` 포맷)에 반영.
  - **앱 상태 라벨(P1)**: `app/support.tsx`의 `STATUS_KO`에 `answered`·`reviewing`이 없어 답변 완료 문의가 폴백 `'답변 대기'`로 표시(답변 본문은 초록 카드로 뜨는데 라벨만 대기 = 화면 내 모순). **뿌리는 위 문서 드리프트**(정정 전 §13.17이 `replied|resolved`로 적혀 있었고 앱 맵이 그걸 따랐다). 교정: 서버 어휘 4종 + 레거시 2종을 모두 매핑하고 **라벨·색을 같은 status 값에서 파생**(콘솔 라벨 대기/확인 중/답변완료/환불완료와 일치).

- **형제 스윕 잔여 3건(2026-07-24, 발견·검증=Fable / 수정·문서=Opus 에이전트) — T1/T2와 **같은 결함 클래스**(admin write의 projCode 미스코프 + rowcount 미확인)**:
  > T1/T2 수정 직후 `server/app/api/admin/**` **전 라우트를 전수 훑어** 나온 잔여분. 아래 R1~R3으로 이 표면의 스윕은 **종결**(안전 확인 목록은 맨 끝).
  - **R1(P0) 환불의 티켓 update 미스코프 + 부분 성공**: `POST /api/admin/refund`가 `where(eq(tickets.id, ticketId))`만으로 update → 타 게임 티켓이 **우리 note로 `refunded` 도장**이 찍혔다(실측: 뮤턴트에서 `status=refunded reply=HACK` 반영 확인). 게다가 **트랜잭션 안**이라 rowcount 0(미존재 uuid)에도 지갑은 차감되고 응답은 `{ok:true}` — **"환불 금액만 나가고 티켓은 남 게임에 찍히는" 부분 성공**이 최악의 결과. **교정**: `and(eq(projCode, PROJ_CODE), eq(id, …))` + `.returning()` → 0건이면 **sentinel throw로 트랜잭션 전체 롤백**(지갑 차감·원장 기록까지 되돌림) 후 **404 `not-found`**. 4xx라 `reportError` 미보고(reply 404와 대칭). **정상 경로 불변**: ticketId 없는 회수·우리 proj 티켓 환불·같은 멱등키 재시도(`applied:false`) 응답 모양 전부 그대로.
  - **R2(P0, 머니패스) `applyWalletTx` 유저 조회 미스코프**: `where(eq(users.id, userId))` — userId가 **관리자 입력**인 `admin/refund`·`admin/grant`가 **타 게임 유저 지갑을 실제로 차감/지급**했다(실측: 뮤턴트에서 타 proj 유저 500→200 차감 + **우리 proj 원장에 남 게임 유저 거래 2행** 기록). **교정**: 잠금 select(FOR UPDATE)·멱등 dup 경로·`applyWallet` 경쟁 재조회 3곳 모두 `and(eq(users.projCode, PROJ_CODE), eq(users.id, …))`. 실패 어휘는 **기존 `no-user` 재사용**(새 코드 발명 없음 — 타 proj 유저 = 이 게임엔 없는 유저 → 라우트는 기존대로 500 `error`; 존재 여부를 관리자 응답으로 흘리지 않는 부수효과도 유지). 토큰 파생 경로(earn/spend/coupon/mail/pass/purchase)는 애초에 우리 proj 유저라 **동작 변화 0**. **`balance == Σledger` 불변식 실측 유지**(가드 ⑥ + `_dv_walletreplay`·`walletConcurrency` 재실행 그린).
  - **R3(P1) 쿠폰·우편 대상 유저 검증 미스코프**: `admin/coupon` POST의 `targetUserId` 존재 확인과 `admin/mail` POST의 `userId` 존재 확인이 proj 무관 → **타 게임 유저 앞으로 개인 쿠폰 발급·우편 발송이 통과**(수신 GET은 proj 스코프라 열람 불가 = **죽은 외래 참조 행**만 남음). **교정**: 두 조회에 projCode 추가 → 기존 실패 경로 그대로 **400 `no-such-user`**. **형제 1건 추가 발견·수정**: `admin/coupon` **PATCH의 `targetUserId`는 존재 검증 자체가 없었다**(POST에만 있었음) → 타 게임 유저로 바꿔치기가 통과·FK만 통과하면 저장. POST/PATCH 공용 헬퍼 `targetInProj()`로 통일(잘못된 uuid 형식 throw도 '없음'과 동일 처리 — C3 규칙 승계).
  - **가드**: `server/tools/_dv_admin_scope_live.ts` 신설(38검사) — 환불 티켓 스코프+**전액 롤백 증명**(잔액·원장 0건)·미존재 uuid 404·정상 경로 불변 5종(ticketId 없는 회수·멱등 재시도·**P0-3 티켓 refunded 수렴**)·지갑 유저 스코프(refund/grant 양방향)·`balance==Σledger`·쿠폰 POST/PATCH·우편 대상 게이트. **A/B 자가검증**: 수정 전 코드(뮤턴트)에서 **15 FAIL** → 수정본 **38/38 PASS**. 각 스코프 검사에 민감도 대조 상주(①동일 호출이 우리 proj 대상엔 200 ②차단된 타 proj 티켓·유저가 **DB에 실재** → "데이터가 없어서 404"가 아니라 "스코프가 막은 404").
  - **미검증(환경 제약)**: 로컬 dev DB에 `mails` 테이블 없음(마이그레이션 0003 미적용, 공유 DB라 적용하지 않음) → 우편 **발송 성공 경로**는 라이브 검증 불가. 대상 게이트(타 proj → 400 `no-such-user` / 우리 proj → 게이트 통과 후 테이블 부재로 500)까지만 실측.
  - **`server/app/api/admin/**` 전수 점검 결과(2026-07-24 종결 — 다시 훑지 말 것)**: 이미 안전(projCode 스코프 + 필요한 곳 rowcount 404 확인) = `announcement`(POST/GET/PATCH/DELETE)·`devnote`(4종)·`coupon` GET/PATCH/DELETE·`coupon/redemptions`·`ticket`(목록)·`ticket/reply`·`ticket/snapshot`·`mail` GET/DELETE(`listAdminMail`·`recallMail`이 lib에서 스코프)·`users`·`stats`·`series`·`bm`·`achievements`·`errors`·`payments`·`payment-events`·`setting`. 이번에 교정 = `refund`·`grant`(경유 `lib/wallet`)·`coupon` POST/PATCH·`mail` POST. 읽기 전용 `getWallet`은 userId가 토큰 파생(uuid PK, proj 유일)이라 위험 없음 — 미변경.
...
- **환불**: RC 환불 웹훅(CANCELLATION/REFUND) → `applyWalletTx(−다이아, 'refund', key=refund:<userId>:<storeTxnId>)`(음수 허용) **+ `reverseRevenueKrwOnce(storeTxnId)`(순매출 KRW 차감 — A2 정정 2026-08-07, 아래 "매출 롤업")**. **관리자 수동 환불(§13.17)과 이중차감 방지**: storeTxnId 파생 공유키로 둘째가 dedupe되게 하거나 "RC 자동 환불분은 관리자 수동 금지" 명문화.
...
- **서버측 구현 완료(2026-07-04, 검증 Opus 4.8)**: 결제 검증 머니패스(라우트·순수판정·매출롤업)를 §13.18대로 구현. **클라 SDK·스토어 상품·EAS·실결제 테스트는 별도**(구조상 서버 밖).
  ...
  - **매출 롤업**: 지급이 **실제 적용된(applied)** 경우만 `statsDaily`(purchaseCount+1·diamondsPurchased += 다이아) 갱신 → 대시보드 매출/전환율 원천. ~~confirm이 grant 경쟁 승리 시 KRW만 유실(재무진실=RC 대시보드라 무해).~~ → **정정(2026-07-16, A1 — 결제표면 감사)**: confirm 선착(KRW 미상 → null) 후 웹훅이 뒤늦게 dedup되면 KRW가 **영구 ₩0**으로 남아 관리자 ⑤ BM 탭 매출이 안 잡혔음(purchaseCount만 증가·DoD "매출 1건 조회" 위반). **`recordRevenueKrwOnce(storeTxnId, priceKrw)`** 신설 — 건수·다이아 롤업(`recordPurchaseRevenue`)과 **KRW 롤업을 분리**하고, applied 경로·웹훅 후착 dedup 보충 경로가 **모두 이 함수를 거쳐 KRW 단일 진실점**. 멱등 = `purchase_event`에 이 storeTxnId의 `revenue.krw` 마커 존재 여부(**새 테이블 금지** — 기존 진단 테이블·pe_txn_idx 앵커, 마커+집계 한 트랜잭션 원자화)로 웹훅 재시도·경로 경쟁에도 KRW 이중집계 0. ~~환불은 원장만(대시보드 매출은 gross).~~
    - **정정(2026-08-07, A2 — 실결제·실환불 실증)**: 실환불(`GPA.3318-1961-7645-34269`, `dia_500` ₩4,800)을 처리했더니 다이아 −500 회수는 정상인데 **관리자 "오늘 매출"이 ₩4,800 그대로**였다. 원인 = `revenueKrw`를 쓰는 함수가 `recordRevenueKrwOnce`(가산) **하나뿐**이라 환불 경로에 대응하는 KRW 차감이 **아예 없음**(단조 증가 값). §13.17 `rollupRecent` TODO가 예고했던 "실환불 붙으면 순매출 과대계상"이 그대로 실현. → **`reverseRevenueKrwOnce(storeTxnId)` 신설**(구매 적재와 **대칭 구조**):
      - **금액 출처 = 원구매의 `revenue.krw` 마커 행 `price`**. 실측한 CANCELLATION 웹훅은 `price=0`으로 온다(prod `purchase_event` 3행 전부 price=0) → **웹훅 바디 가격을 쓰면 0 차감이라 무의미**. storeTxnId로 원구매 적재액을 되찾아 차감 = **"적재한 만큼만 되돌린다"** 불변식(Σ차감 ≤ Σ적재, 다이아 건수 역산 금지 원칙과 같은 결).
      - **멱등 = `revenue.krw.refund` 마커**(구매의 `revenue.krw`와 같은 패턴·새 테이블 금지). 마커 확인 → 원구매 조회 → 차감 → 마커 기록을 **한 트랜잭션**으로 원자화 → 환불 웹훅 재전송(CANCELLATION 후 REFUND 등)에도 이중 차감 0. **호출은 팩/패스 환불의 applied·deduped 양쪽**에서(구매 경로가 A1 보충을 양쪽에서 호출하는 것과 대칭 — 첫 배송에서 차감이 못 일어난 경우 재전송이 메꿈).
      - **귀속일 = 환불일(오늘 UTC)**, 원구매일 소급 수정 아님. 근거: 스토어·RC 리포트가 환불을 **발생 기간의 contra-revenue**로 잡고, 과거 일자를 재작성하면 이미 본 대시보드 수치가 뒤바뀐다. 일별 합 = 총매출은 어느 쪽이든 동일.
      - **음수 클램프 안 함**(`GREATEST(0,…)` 기각). 판매 0인 날의 환불을 0으로 자르면 그 차감분이 **영구 소실**돼 총매출이 다시 과대계상된다(클램프가 오히려 진실을 가림). 음수를 clamp 없이 그대로 표시하는 §13.17 P0-1(음수 잔액) 원칙과 같은 결. 관리자 매출 차트에 음수 막대가 뜨는 날 = 그날 환불이 매출을 넘었다는 **사실**.
      - **건수·다이아(`purchaseCount`·`diamondsPurchased`)는 되돌리지 않음** — 이 둘은 **매일 크론 롤업(`rollupRecent`)이 `reason='purchase'` 원장에서 덮어쓰기 upsert**하는 값이라(★ statsDaily 라이터 둘, 2026-07-18 D1 정정) 환불 때 감산해도 **다음 크론이 gross로 원복**시킨다 = 사라지는 감산(더 나쁜 거짓). 반면 `revenueKrw`는 크론이 안 건드리므로 순액 유지가 안정적. **정의 명시**: `purchaseCount`·`diamondsPurchased` = **gross(결제 발생 건수·지급 다이아)**, `revenueKrw` = **net(환불 차감 후 순매출)**. 환불 측 수치는 이미 `admin/series?metric=refund`(환불 건수·환불 다이아, 원장 기준)로 분리 제공된다. gross KRW까지 병기하려면 `statsDaily.refundKrw` Expand-only 추가가 다음 수(스키마 변경이라 이번 미실행).
      - **부분환불 미지원**(현재 소모성 팩 전액환불만 존재) — 이 함수는 항상 **원구매 적재액 전액**을 차감한다. 향후 부분환불 이벤트가 오면 과다 차감이 되므로, 그때는 웹훅 바디의 부분 금액을 신뢰하고 마커를 txn당 1행 → **누적 다행**(Σ차감 ≤ Σ적재 검사) 구조로 바꿔야 한다(코드 주석 동일 기재).
      - **적용 범위**: 웹훅 **팩 환불·패스 환불 둘 다**(지급 시 둘 다 KRW를 적재하므로 대칭). **샌드박스는 스킵**(적재 자체를 안 하므로 — D1 3경로 대칭 유지). **관리자 수동 환불(§13.17)은 KRW 무관**(storeTxnId 없는 재화 조정이라 원구매 적재와 연결되지 않음 — 무변경). 익명 환불(B1)은 여전히 `refund.anonymous.dropped`로 dropped(원장·매출 둘 다 무변).
      - **관측**: 차감 성공 시 `webhook.revenue.krw.refunded` 감사행, 원구매 적재 이력이 없으면 `webhook.revenue.krw.refund.noorigin`(ok:true·outcome:ignored — 정상 케이스이므로 ⑨ 오류 지표 `ok=false`를 오염시키지 않음)를 기록하고 **마커는 안 남긴다**(순서역전으로 나중에 적재되면 재전송이 그때 차감할 수 있게).
  ...
  - **검증**: `server/tools/_dv_purchase.ts`(인증 fail-closed·샌드박스/엔타이틀먼트/미등록/구독타입/익명 무시·grant/refund·**멱등 dedup**·이중환불 dedup·순서역전 순0·라우트 통합 401/+1000/재전송 dedup/−1000·테스트유저 정리 + **결제표면 감사 2026-07-16**: D1 confirm×SANDBOX 필터·B1 익명환불 `refund.anonymous.dropped` 관측·A1 confirm선착→웹훅후착 KRW 보충 + **S1 `RC_SANDBOX_GRANT` 스위치(2026-07-17)**: off=현행 필터·on=SANDBOX grant 원장 지급+ref `:sandbox` 마커·매출KRW/건수/다이아 무증가·SANDBOX 환불 클로백·스위치 delete 시 재필터(웹훅·confirm 대칭 A/B), 각 A/B 민감도 실증 + 변이 자가검증 + **S1-e 크론 롤업 경로 대칭 제외(2026-07-18)**: dev DB에 실 원장행(ref=`dia_1000`)+샌드박스행(ref=`dia_1000:sandbox`) 삽입 → `rollupRecent()`가 **실 건만** 집계(Δ건수1·Δ다이아1000, 샌드박스 Δ0)·구/신 롤업 쿼리 A/B로 같은 2행 count 2↔1·dia 2000↔1000 민감도 실증(뮤턴트 박제 없이 가드 안 별도 실행)·statsDaily 스냅 원복(newUsers 포함)) + **A2 환불 KRW 차감(2026-08-07)**: 구매 웹훅(₩4,800 적재) → **prod 실측 형태의 환불 웹훅**(currency·price 미제공 = `priceKrwOf` null)으로 CANCELLATION → `revenueKrw` Δ0(순매출 복귀)·다이아 회수·`webhook.revenue.krw.refunded` 감사행, 환불 재전송 ×2(REFUND·CANCELLATION)에도 이중 차감 0, `purchaseCount`는 gross 유지(1). **A/B 민감도**: 같은 데이터에 **구로직(원장 클로백만·KRW 차감 부재)** 을 가드 안에서 조립 재현 → 매출 ₩4,800 잔존("오늘 매출 그대로" 버그 실측) vs 신 함수 적용 → 순매출 0(마커 재호출 `skipped`). no-origin(적재 이력 없는 txn) → 차감 0·마커 미기록 → 뒤늦게 적재되면 그때 차감 가능(순서역전 복구) 전항 PASS + server tsc 0.
...
- **디스코드 알림(2026-07-05, 사용자 요청)**: `server/lib/notify.ts` — 공통 `postDiscord`(url 없으면 no-op·throw 없음·4초 타임아웃) 위에 두 종류.
  ...
  - **신규 가입**(`notifySignup`, 2026-08-07 추가): `/api/auth/login`의 **신규 생성 분기에서만** 통지. 상세는 **§13.28**.
...
> - **placeholder(EAS 후)**: **②리텐션·~~③플레이~~(→ 2026-07-31 telemetry 행동 실데이터, funnel만 EAS-후)·~~④오프시즌~~(→ 2026-07-31 telemetry 재배선, 단 funnel만 EAS-후)·~~⑦경기~~(→ 2026-07-31 v1 순위+v2 승패)·~~⑧선수~~(→ 2026-07-31 v2 로스터 구성)** = 골격 카드("무슨 지표를·언제 보여줄지" 명시) · ⑤ ARPU/ARPPU/상품별 매출액(RevenueCat #43 후) · ⑥ eCPM/노출/수익(AdMob API 후) · ⑨ Sentry(API키 후 pull; `SENTRY_API_TOKEN` 있으면 골격 통과·없으면 "미설정" 배지 throw-none)·Crashlytics(EAS 후) · ⑪ ~~MAU~~(→ 2026-07-31 lastSeenAt 실화)·~~D1/D7/D30~~(→ 2026-07-31 lastSeenAt 근사)·ARPU 등 "—"+~~"EAS 후"~~(→ 지표별 실블로커: 플레이=EAS 후, ARPU류=#43 후) 배지.
...
>
> **✅ 추가 실데이터 재배선(2026-07-31 · DAU 하트비트 §13.15 후속 · 전수조사 3건)** — placeholder를 EAS 없이 지금-가능분으로 승격:
> - **④ 오프시즌 탭**: ~~"오프시즌 funnel — 어디서 이탈하나" placeholder(EAS 후 · foreign_tryout_open·fa_open·draft_pick·special_training 단계 이벤트)~~ → **§13.27 `season_telemetry` 실데이터로 재배선**(신 파이프 없음 — `GET /api/admin/telemetry`가 이미 집계하는 오프시즌 행동을 오프시즌 탭이 필터·렌더). 렌더분 = **전지훈련 인원(`campCount`)·방출(`releases`)·제명(`expels`)·훈련 방향 분포(`trainingFocus`/topFocus)** — 세이브 파생 [자체-롤업]·비식별·결정론 격리. **단 "단계별 도달/이탈 funnel"(tryout→FA센터→드래프트 진입/이탈률)은 season_telemetry에 단계 진입 이벤트가 없어 여전히 불가** — 그 funnel만 EAS 계측(track `*_open`) 후. 즉 오프시즌 탭은 "행동량 실데이터 + funnel은 EAS-후" 혼합으로 정직하게.
> - **⑪ 메인 KPI — MAU/WAU 실화**: ~~MAU·WAU도 "—"+"EAS 후"~~ → **`stats.kpi.mau`·`wau` 실값**(lastSeenAt 기반 — DAU(오늘)와 **동일 규약**, 롤링 최근 30일/7일 내 lastSeenAt 있는 non-deleted 유저 수). DAU 하트비트(GET /api/wallet touchLastSeen, 2026-07-31)로 lastSeenAt이 포그라운드마다 갱신돼 이제 유의미. **나머지 6개는 실 블로커로 라벨 정정**: D1/D7/D30 리텐션·평균 플레이 = `EAS 계측 후`(코호트/플레이 이벤트 필요) · ARPU/ARPPU/월매출 = `#43 결제 후`(결제 원장 필요). ~~뭉뚱그린 "EAS 후"~~ → 지표별 실제 블로커.
> - **결제 이벤트 표(§13.22) rich 필드 노출**: `payment-events` API가 반환하던 `environment(SANDBOX/PRODUCTION)·productId·price·currency·eventType`을 표에 **열/뱃지로 추가**(API가 이미 주던 값 — 렌더만, 새 필드 없음). "돈 내고 0개" 감사에 sandbox/production 구분이 핵심(§13.22)이라 environment 필수 노출. `detail`은 여전히 `?txn=` 상세에.
>
> **✅ 추가 실데이터화(2026-07-31 후속 · telemetry payload v2 §13.27 · 전수조사 4건)** — placeholder 4탭을 자체 텔레메트리로 승격(EAS 불필요):
...
### 13.27 시즌 종료 행동 텔레메트리 (구단주 운영 행동 → 관리자 사용자별 분석) — ✅ 구현(2026-07-30, payload v2 확장 2026-07-31)
> **상태**: 클라 집계·전송 + 서버 테이블·API·마이그레이션 + 관리자 화면 구현(2026-07-30). **payload v2 확장(로스터 구성·정규시즌 승패/세트·은퇴 수) + 플레이·경기·선수 탭 실데이터화 + 리텐션 근사(2026-07-31)**. prod DB 마이그레이션 불필요(payload jsonb — 스키마 무변경). 서버 배포는 후속(메인/사용자). **v2 payload는 앱 OTA + 테스터 시즌 종료 후에야 유입**(서버 집계는 v1도 계속 처리하므로 즉시 배포해도 무해).
...
>
> **🔒 가명화 정정(2026-08-05, 개인정보 판단 후 — 컴플라이언스)**: ~~`user_id` FK로 저장 → 관리자 화면이 users 조인으로 실명 표시~~ 는 **계정 연계 개인정보**였다(userId 인덱스로 사용자별 조회 + displayName 조인 = 재식별). PIPA상 이건 익명이 아니라 개인정보이고, 텔레메트리엔 **법정 보존 근거가 없어** 탈퇴 시 파기 대상인데 파기 배선도 없었다. **교정 = 발생원 가명화**: 저장 키를 `user_id` FK가 아니라 **`analytics_id = HMAC-SHA256(userId, TELEMETRY_SALT)` 앞 32hex**(`lib/telemetryId.ts`)로 바꿔 **users와 직접 조인 불가**하게 함 → **PIPA 가명정보(§28-2 통계 목적)**. 같은 유저=같은 id라 코호트 추이(§13.25 ③④⑦⑧)는 그대로 유지되나, 역산은 별도 관리 SALT 필요. 관리자 라우트는 users 조인 제거·실명 미노출(응답 키 `userId`는 페이지 호환 위해 유지하되 값=가명 id·`name`=null). **탈퇴 파기 결합도 소멸**(텔레메트리에 userId 없음 → `pseudonymizeUser`가 손댈 것 없음). 마이그레이션 `0005_telemetry_pseudonymize.sql`(비공개 테스트라 기존 행 truncate·백필 없음). 가드 `_dv_telemetry.ts`에 가명화 검증 추가(userId 미저장·analyticsId≠userId·롤업 실명 없음). 개인정보처리방침 §1⑥·§3 "가명처리" 반영. SALT는 prod Vercel env로 secret 지정 권장(미설정 시 폴백 상수로도 동작 — 개인정보 제거 효과는 동일).
...
- **payload(~~v1~~ → v2, 2026-07-31 확장. 롤오버 전 상태에서 read-only 파생)**: ~~`{ v:1, season, finalRank, champion, subs:{total,manual,pinch}, timeouts, interventions, lineupChanges, coachMode, releases, expels, trainingFocus, campCount }`~~
  → **v2** = v1 필드 **전부 유지** + `rosterSize, avgAge, avgOvr, foreignCount, retirements, wins, losses, setsWon, setsLost`, `v:2`. **서버 집계는 v1(기존 저장분)·v2 둘 다 graceful**(옛 payload에 없는 필드는 skip/null, crash 없음 — 집계 분모를 필드 보유 행으로만 잡는다).
  ...
  - **v2 추가 필드 read-only 근거(전부 롤오버 직전 상태에서 확실히 파생 — 추정/억지매핑 없음)**:
    - `rosterSize`/`avgAge`/`avgOvr`/`foreignCount` = **내 팀 시즌종료 시점 로스터 스냅샷 파생**(비식별 집계 정수 — 선수 이름·id 0). endSeason이 `commitPlayerBase`(롤오버) **직전에** `finalR[my]`(시즌 최종 명단)의 Player 객체를 `getPlayer`로 캡처해 `TelemetrySource.rosterPlayers`로 넘기고, 순수 빌더 `computeSeasonTelemetry`가 `engine/overall overallRaw`(read-only·rng 미소비)로 avgOvr을, age/isForeign 필드로 나머지를 집계. ⚠ 캡처는 반드시 `commitPlayerBase`(다음 시즌 base로 교체) **전** — 후에 `getPlayer`하면 롤오버된(노쇠·은퇴제외) base라 부정확.
    - `retirements` = 이번 시즌 내 팀 은퇴 수 = `ctx.retired ∩ finalR[my]`(호출부 스칼라, `TelemetryMeta`로 주입).
    - `wins`/`losses`/`setsWon`/`setsLost` = 내 팀 정규시즌 전적 — **endSeason이 롤오버 전 이미 계산한 스칼라를 재사용**(`record[my]`=`computeStandings` 파생 `[wins,losses]`; `setsWon`/`setsLost`=`seasonResults` 순회 집계 — 둘 다 `careerTotals` 누적용으로 이미 산출됨). **경기 재구성 0**(집계된 순위/전적 테이블에서만) → 비차단·결정론 격리 보존. `TelemetryMeta`로 주입.
  - **여전히 의도적 제외(추정/억지매핑 금지)**: 엔진 **자동 교체**(SubKind `block`/`def`/`injury`/`rest`)는 (a) **구단주 행동이 아니라 엔진/감독 자동**이고 (b) subEvents 재구성이 필요한데 그 재구성은 **롤오버 전 엔진 컨텍스트에서만 정확**(롤오버 후엔 다음 시즌 로스터라 부정확) → **비차단 제약과 충돌**(재구성=블로킹). 따라서 개입 로그로 확실히 구하는 사용자 행동만 담고 엔진 자동분은 생략한다. payload가 jsonb라 후속 확장 자유.
...
- **스키마**: `season_telemetry`(id·proj_code FK·~~user_id FK~~ **`analytics_id` text(가명 — 2026-08-05)**·season int·payload jsonb·created_at). 인덱스: ~~`(proj_code,user_id,…)`~~ **`(proj_code,analytics_id,season)` UNIQUE(교체) + `(proj_code,analytics_id)` 조회**.
- **관리자 분석(§13.15 콘솔)**: `GET /api/admin/telemetry`(requireAdmin) — 전체 집계(평균 개입·타임아웃·방출·전지훈련·지휘모드 사용률·우승률·평균 순위) + **사용자별 롤업**(유저→시즌별 payload 추이). ops-9f3a2c에 "행동 텔레메트리" 섹션(분석 그룹).
  - **탭 실데이터화(2026-07-31, §13.25 ②③⑦⑧)**: 이 같은 `/api/admin/telemetry` 집계를 **플레이(③)·경기(⑦)·선수(⑧)** 탭이 재사용해 placeholder를 실데이터로 대체(신 파이프 없음). 라우트 집계에 추가: `interveneRate`(개입 1회+ 리포트 비율, 플레이 탭) · `rankDist`(finalRank 히스토그램, 경기 탭) · v2 집계(`avgRosterSize/avgRosterAge/avgRosterOvr/avgForeignCount/avgRetirements` 선수 탭 · `avgWins/avgLosses/winRate/avgSetsWon/avgSetsLost` 경기 탭 — **분모는 해당 v2 필드 보유 행 수만**, v1 저장분은 자연 제외). 경기 탭의 승/패·세트 카드는 **v2 유입 전엔 정직 노트**("상세 승패·세트 = 다음 빌드부터").
  - **리텐션 D1/D7/D30 근사(대시보드 MainKpi, `GET /api/admin/stats`)**: season_telemetry 아님 — `users`(createdAt·lastSeenAt) 기반. Dk 근사 = "가입 후 k일+ 지난 유저 중 lastSeenAt이 (createdAt+k일) 이후인 비율". lastSeenAt은 **마지막 접속만** 주므로 정밀 코호트 아님("여전히 살아있나" 근사)을 라벨에 명시. 분모(가입 후 k일+ 지난 유저) 0이면 null. ~~설치일 코호트 매트릭스(리텐션 탭 RetentionPH)는 여전히 EAS 계측 후.~~ → **정정(2026-07-31 — `RetentionPH`→`RetentionTab` 교체)**: 리텐션 탭도 이 Dk 근사를 재사용해 **근사 D1/D3/D7/D14/D30**을 노출한다(placeholder 제거). **정밀 설치일 코호트 매트릭스만**(설치 코호트별 D1~D30 격자 — app_open 이벤트 필요) GA4/BigQuery 연동 후로 남는다.
...
### 13.28 신규 가입 디스코드 알림 (2026-08-07 — 프로덕션 출시 직후 사용자 요청 "몇 달간 가입을 눈으로 보고 싶다")
> **상태**: ✅ 구현(2026-08-07). §13.22 디스코드 알림 인프라(`server/lib/notify.ts`)의 **세 번째 종류** — 새 HTTP 클라이언트 0, `postDiscord` 재사용.
> 순수 **관측 사이드채널** — 재화·시드·리플레이·게임플레이에 일절 무관(§8 결정론 격리, §13.21·13.22와 같은 계약).

- **왜**: 출시 직후 몇 달은 "사람이 실제로 들어오는가"가 사실상 유일한 지표인데, 지금은 관리자 대시보드(§13.15)를 **열어봐야만** 보인다.
  가입이 **발생 즉시** 디스코드에 뜨면 1인 운영자가 성장 곡선을 실시간으로 본다(결제·문의가 이미 그렇게 뜨고 있으니 같은 채널 문법).
- **발화 지점 = "진짜 가입" 정확히 1회 (핵심 판단)**: `server/app/api/auth/login/route.ts`의 **`createUser()` 성공 직후**(신규 생성 else 분기).
  - **왜 거기인가**: `lib/wallet.ts`의 유저 생성 경로 두 개는 의미가 다르다.
    - `ensureUser()`는 **저수준 upsert**(있으면 조회·없으면 생성)로 익명 폴백(`resolveUserId`)·가드가 **매 호출** 부르는 자리 — "가입"을 뜻하지 않는다. **여기 붙이면 오발화**.
    - `createUser()`는 **연령 게이트(만14세, AUTH §8)를 통과한 신규 소셜 계정에만** `ageConfirmedAt`을 박는 경로이고, **호출자가 로그인 라우트 단 하나**(전수 grep 확인). 로그인 라우트는 `findUserRow`로 기존 계정을 먼저 찾아 **살아있으면 else 분기에 아예 도달하지 않으므로**, 재로그인·기존 유저 접속은 **구조적으로 무발화**(플래그 비교가 아니라 분기 자체가 다름).
  - **`createUser()` 함수 안이 아니라 라우트에 붙인 이유**: (a) 관측 채널은 **라우트가 `afterSafe`로 응답 후 예약**한다는 것이 §13.22의 확립 계약(lib이 직접 쏘면 응답 경로를 오염 — "돈은 갔는데 500" 사건과 같은 클래스). (b) 기기정보(platform·appVersion)는 **요청 body에만** 있다. (c) `createUser`는 스크립트/가드가 직접 부를 수 있어 lib에 심으면 테스트가 실채널로 샌다.
  - **탈퇴 후 재로그인**은 providerId가 토움스톤이라 `findUserRow` 미스 → **새 계정 = 새 알림**(AUTH §7.3 "재로그인=새 계정"과 일관, 의도된 동작).
- **비차단(절대 제약 — 가입은 사용자 대기 경로)**: `afterSafe(() => …)`로 **응답 후** 실행 → 가입 응답 지연 **0**(§13.22 서버리스 freeze 유실 방지). 3중 안전:
  ① `postDiscord`가 url 없으면 no-op·4초 AbortController 타임아웃·**절대 throw 없음** ② 누적 카운트 조회 `countSignups()`도 **throw-none**(실패 시 `null` → 임베드에 `—`) ③ `afterSafe`가 요청 컨텍스트 밖에서도 안전.
  ⇒ **디스코드가 죽든 DB 카운트가 실패하든 회원가입은 성공**한다.
- **임베드(개인정보 최소화 §13.9)**: 가입 시각(embed `timestamp`) · **프로바이더**(google/apple/dev) · **플랫폼 + 앱버전** · **누적 가입 N번째** · 유저 축약 · footer `proj <PROJ_CODE>`.
  **절대 금지**: 이메일·이름·`displayName`(비필수 PII·파기 대상 §13.9)·`providerId`(구글 sub = 계정 식별자)·`userId` 원문. 텔레메트리 가명화(§13.27)와 같은 기조.
  유저 표기는 **기존 `maskUser`(뒤 6자) 재사용** — 결제·문의 알림과 **같은 마스킹 규약**이라 한 유저의 가입→문의→결제를 채널 안에서 대조할 수 있다(축약 규약이 알림마다 다르면 그 대조가 불가능해진다).
- **누적 가입자 수**: `countSignups()`(`server/lib/wallet.ts`) = `count(*) where projCode=PROJ_CODE and ageConfirmedAt is not null`.
  `ageConfirmedAt`은 **`createUser`만** 박으므로 `ensureUser`가 만든 dev·가드 행, 익명 문의 유저(아래)는 **자연 제외** = **실가입만** 센다. 탈퇴 행도 포함(**누적**의 정의 — 탈퇴해도 "가입은 있었다"). 방금 만든 행이 포함되므로 **"N번째 가입"** 으로 표기.
- **멀티프로젝트 격리(§13.2)**: 이 서버는 배구명가(`volleyball`)와 My Word(`myword`) 문의를 함께 받지만, **타 프로젝트는 익명 문의(`/api/ticket/anon`)만** 쓰고 그 유저 행은 그 라우트 자체의 `ensureAnonUser`(provider=`anon`·`ageConfirmedAt` null·인자 proj)로 만든다 — **`createUser`도 `/api/auth/login`도 타지 않는다**. 로그인 라우트는 `PROJ_CODE` 고정. ⇒ 이 알림은 **구조적으로 배구명가 가입 전용**이고 카운트도 proj 스코프. 그래도 채널 오독(결제 채널 폴백 시 혼재) 방지로 임베드 footer에 `proj <PROJ_CODE>`를 명시한다.
- **채널**: `DISCORD_SIGNUP_WEBHOOK_URL`(전용) → 없으면 `DISCORD_WEBHOOK_URL`(§13.22 결제 채널) 폴백 → **둘 다 없으면 완전 no-op**(dev·미설정 무해). `notifyTicket`의 `DISCORD_TICKET_WEBHOOK_URL` 폴백 체인과 동형. env는 **호출 시점 읽기**(콜드스타트 타이밍·가드 견고, `notifyPurchase`와 동일).
- **검증**: 가드 `server/tools/_dv_signup_notify.ts` — DB 없이 도는 **순수 티어**(`fetch` 스텁으로 발사 캡처) + DB 있으면 도는 **라이브 티어**(로그인 라우트 직접 호출).
  - 순수 티어: ① env 둘 다 미설정 = **발사 0**(no-op) ② `DISCORD_WEBHOOK_URL`만 = 그 채널 1발 ③ `DISCORD_SIGNUP_WEBHOOK_URL` 우선(둘 다 있으면 전용 채널) ④ 임베드에 **PII 부재**(displayName·이메일·providerId·userId 원문 문자열 미포함) + 프로바이더/플랫폼/버전/누적수 **존재** ⑤ fetch가 reject/타임아웃해도 **throw 안 함** ⑥ `countSignups` 실패해도 알림·가입 무영향.
  - 라이브 티어: ⑦ **신규 로그인 = 정확히 1발**, ⑧ **같은 계정 재로그인 = 0발**(A/B 민감도 — 같은 하니스로 신규는 1발이 실측되므로 "항상 0이라 통과"가 아님), ⑨ 디스코드가 500/hang이어도 **로그인 200**(가입 비차단 증명), ⑩ 누적 카운트가 신규 1건마다 +1.
  - 자가검증(허위 오라클 금지): ⑧의 대조로 **발화 지점을 `ensureUser` 계열(= upsert 자리)로 옮긴 변이**를 주입하면 재로그인에서도 발사되어 ⑧이 **FAIL로 뒤집히는지** 확인 → 가드가 "잘못된 자리"를 실제로 잡는다는 증명.
...
> **상태**: ✅ **구현·운영 배포 완료(2026-08-08)**. §13.15의 "⚠ 지표 해석 주의" 블록이 남긴 **육안 보정** 규율을 코드 장치로 승격했다.
> **운영 실측(배포 직후, 개발자 본인 계정 1건 표시)**: 결제 전환율 **33.3% → 0%**(결제자 0명 — 실질값과 일치) ·
> 총가입 3→2 · DAU 2→1 · 시간대 분포 3버킷→2버킷. 대상 판별은 **행동 이력**으로 확정했다(그 계정만 결제 2·환불 1·쿠폰 1·문의 1·원장 22행, 나머지 둘은 전부 0).
> **알려진 표시 불일치(의도)**: 사용자 탭의 "신규 가입" 차트는 `/api/admin/series`(Phase 2 미적용)라 총가입 2 옆에서 **3 합**으로 남는다 — 고지 배지가 이를 명시한다.
> **왜 지금**: 출시 직후 유저가 한 자릿수라 개발자 본인 한 계정이 모든 비율 지표를 지배한다. 실유저가 늘기 전에
> 넣어야 시계열이 오염된 채 굳지 않는다. 늦게 넣으면 "그때 그 숫자는 진짜였나"를 영영 못 푼다.

**A-0. "내부"의 정의 (2026-08-08 사용자 용법으로 확정)**
`internal`은 조직 소속이 아니라 **"실유저로 셀 수 없는 계정"** 을 뜻한다 — 운영자·QA·**지인**·테스터.
사용자가 지인 계정을 직접 표시하며 확정된 의미다(처음 화면 문구는 "운영자·QA"로 좁게 써서, 나중에 보면
지인 계정이 왜 거기 있는지 설명이 안 되는 상태였다 → 문구를 실제 의미에 맞춤).
> 종류별 하위 구분(`internalKind` 같은 컬럼)은 **넣지 않는다.** 지금 필요한 판단은 "집계에 넣나 마나" 하나뿐이고,
> 세분화는 쓰이지 않을 축을 늘릴 뿐이다. 필요해지면 그때 Expand로.
...
**D-2. Phase 2 (2026-08-08 당일 확장) — 사용자 지적 "업적이나 이런 화면에도 내부자 나온다"**
Phase 1을 `stats`에만 적용했더니 **업적·BM·시계열에 내부 계정이 그대로 남아** 사용자가 화면에서 발견했다.
근인은 "라우트마다 제외 조건을 손으로 다시 쓰는 구조" — 그래서 스코프 계산을 **공용 `server/lib/internalScope.ts`** 한 곳으로 모으고,
가드가 **"집계 라우트 4종이 전부 이걸 경유하는가"** 를 소스 수준에서 봉인한다(새 집계 라우트가 빠뜨리면 FAIL).

| 라우트 | 적용 | 비고 |
|---|---|---|
| `/api/admin/stats` | ✅ | Phase 1. 공용 스코프로 리팩터(자체 id 쿼리 제거 — 진실 단일화) |
| `/api/admin/achievements` | ✅ | **분자(달성 유저)·분모(총가입) 양쪽** — 한쪽만 빼면 비율이 더 틀어진다 |
| `/api/admin/series` | ✅ | `signups`·`ad`·`refund`. `revenue`는 사전 롤업이라 불가(응답에 고지) |
| `/api/admin/bm` | ✅ | 상품별 집계·결제자·전환율. KRW 매출은 불가(고지) |
| `/api/admin/{users,payments,payment-events,ticket,mail}` | ❌ **설계상 제외 안 함** | **개별 레코드 목록**은 운영자가 내부 계정 행을 봐야 한다(내 테스트 결제가 원장에 꽂혔나). 대신 목록에 '내부' 표시 |
| `/api/admin/telemetry` | ⚠ **부분** | **뭉뚱그리면 틀린다.** 이 라우트는 반반이다 — `campLedger*`(전지훈련 유저·횟수·소모 다이아)는 **`wallet_ledger(reason='camp')` 파생 = userId 있음 → 제외 적용함**. 나머지(`season_telemetry` 파생 agg·유저 롤업)는 `analyticsId`가 HMAC 가명이라 조인 불가 → 불가(§13.27의 의도된 대가) |

> **⚠ 2026-08-08 자체 오류 기록**: 위 telemetry 행을 처음엔 "❌ 불가"로 뭉뚱그려 적고 라우트에 손을 안 댔다.
> 그런데 **오프시즌 탭의 주 지표(전지훈련 카드)가 그 라우트 안의 원장 파생**이라, 화면에는 내부 계정이 그대로 남았고
> 사용자가 "오프시즌도 막아야지"로 잡았다. 교훈: **라우트 단위로 "가능/불가"를 판정하지 말고 지표(데이터 원천) 단위로 볼 것.**
> 한 라우트가 서로 다른 원천을 섞어 반환하는 일이 흔하다.

> **경계 원칙(2026-08-08 정밀화)** — 판단 기준은 "집계인가"가 아니라 **"오디언스 지표인가, 진단 신호인가"** 다.
> - **제외**: 오디언스 지표 = *누가·얼마나 쓰고 있나*. 가입·활성(DAU/WAU/MAU)·리텐션·결제 전환·광고·업적 수령·전지훈련.
>   내부 계정이 섞이면 **제품 판단을 그르친다.**
> - **미제외**: ① **진단·감사 신호**(결제 오류·원장·payment-event·문의) — 오류는 *누구 계정이든 진짜 오류*라
>   숨기면 false-negative가 된다. ② **개별 레코드 목록**(사용자·결제 내역) — 운영 대상이라 가리면 안 된다.
>
> ⚠ **자체 정정**: 처음엔 `stats.errToday`도 제외했다가 되돌렸다. 같은 화면에서 대시보드 KPI와
> 오류 모니터링 탭(`/api/admin/errors`, 전건 표시)의 **두 숫자가 말없이 어긋나는** 게 첫 신호였고,
> 더 근본적으로 오류는 오디언스 지표가 아니었다. **"가릴 이유가 없으면 가리지 않는다"** 로 판정.
> **화면 고지도 라우트가 준 `notApplied`만 렌더한다** — 미적용 목록을 UI에 손으로 박아두면 Phase가 올라가도 옛 문구가 남아 거짓말이 된다.
