...
- **파일**: `server/db/schema.ts`(purchase_event)·`server/lib/paymentLog.ts`(신)·`server/app/api/purchase/webhook/revenuecat/route.ts`·`.../confirm/route.ts`(단계 로깅)·`server/app/api/admin/payment-events/route.ts`(신·조회)·`lib/iap.ts`·`lib/server.ts`(requestId 상관·클라 컨텍스트).
