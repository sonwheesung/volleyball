...
- **관리자 분석(§13.15 콘솔)**: `GET /api/admin/telemetry`(requireAdmin) — 전체 집계(평균 개입·타임아웃·방출·전지훈련·지휘모드 사용률·우승률·평균 순위) + **사용자별 롤업**(유저→시즌별 payload 추이). ops-9f3a2c에 "행동 텔레메트리" 섹션(분석 그룹).
  ...
  - **리텐션 D1/D7/D30 근사(대시보드 MainKpi, `GET /api/admin/stats`)**: season_telemetry 아님 — `users`(createdAt·lastSeenAt) 기반. Dk 근사 = "가입 후 k일+ 지난 유저 중 lastSeenAt이 (createdAt+k일) 이후인 비율". lastSeenAt은 **마지막 접속만** 주므로 정밀 코호트 아님("여전히 살아있나" 근사)을 라벨에 명시. 분모(가입 후 k일+ 지난 유저) 0이면 null. 설치일 코호트 매트릭스(리텐션 탭 RetentionPH)는 여전히 EAS 계측 후.
