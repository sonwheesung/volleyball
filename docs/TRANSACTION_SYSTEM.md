...
## 5c. 초기 FA 풀 (2026-07-28 — 사용자 결정, 게임 시작 시드)
> **문제(테스터 제보)**: 게임 시작 직후(시즌0)엔 시즌 중 영입 화면(`availableFAsOnDay`)이 **구조적으로 공집합**이라
> 초반 전력 보강이 불가능하다. §5b가 밝히듯 시즌 중 풀은 "방출이 만드는데", 시즌0엔 **직전 오프시즌도 방출도 없어**
> 시드가 `[]`이다(설계상 정상이지만 초반 무영입 = 단장 결정 기둥 #4를 죽임).
>
> **결정**: 게임 시작 시 **잉여 국내 FA ~10명(표시 OVR 70~80대 = 원시 ~60~69, 즉시 전력 로테이션~약팀 주전급)을 초기 풀에 시드**해,
> 시즌0 첫날부터 시즌 중 영입 화면에서 데려올 수 있게 한다. **이는 §5b "풀은 방출이 만든다"의 예외가 아니라 "시작 조건"**
> — `CLUB_IDENTITY_SYSTEM`의 정체성 배정과 같은 결(시드 생성 시점 1회, 시즌≥1은 평균회귀). 시즌≥1의 FA 공급은
> 여전히 방출·만료·오프시즌 FA 센터가 담당(FA_SYSTEM §1 "공급은 시간이 지나며 생긴다" 기둥 불변).
>
> **독립 리뷰 반영(2026-07-28)**: "수정 후 진행" — 기둥·결정론·GOLDEN 무리스크 확인. 톤 정정(원시 60~69는 스타가 아닌
> **로테이션~약팀 주전 뎁스** — "스타 영입"으로 과약속 금지, guide=구현된것만 결) + 검증에 밸런스 측정 추가(아래).

- **범위 = 시즌0 한정**: 미영입 초기 FA는 **첫 롤오버 스냅샷(`currentBasePlayers` = 로스터 기반)에 안 실려 시즌0 종료 후 자연 소멸**한다
  (영입되면 로스터 합류 → 정상 영속). 결정론 단순화 + 기둥 보존 — "창단 시점의 미계약 베테랑, 안 데려가면 흩어진다"는 현실 서사와도 맞음.
- **주입 메커니즘(2곳 동시 — 조사 정본)**:
  1. **선수 객체**: `data/seed.ts generateLeague()`에서 **팀 루프 뒤·`dedupeNames`(431행) 앞에 별도 rng**(`createRng(strSeed('initfa'))`)로
     `LEAGUE.players`에 append(팀 미배정). → `playerMap` 등록(`getPlayer`/`evolveOnDay`의 유일 소스), `resetLeagueBase`도 자동 복원,
     **시즌0 `playerBase=null` 재로드에서도 해석**. **메인 rng 미소비**라 기존 리그 GOLDEN 바이트 불변(필수).
...
- **OVR 타겟 = 표시 OVR 70~80**(사용자 프리뷰가 표시값): `displayOvr(overallRaw)∈[70,80]` ⇔ `overallRaw≈[60,69]`
  (`engine/overall.ts` 표시 스트레치). 결정론 후보 생성 → 밴드 필터로 선별. **포지션 스프레드**(S·OH·OP·MB·L 최소 1 + OH/MB 가중).
  신규 FA는 `awardScoreOf(id)=0`이라 수상 프리미엄 없이 순수 시장가(`inSeasonCost(marketVal)`)로 요구액 산정.
- **영입 경로 불변**: 기존 `signInSeason` 게이트(풀 멤버십 §0①·캡·정원 `ROSTER_CONTRACT_CAP`·운영자금·배신 웃돈·포스트시즌 동결)를
  그대로 통과. 초기 FA도 이 관문을 똑같이 지킨다.
- **검증(`tools/_dv_initfa.ts`)**:
  - **구조**: 게임 시작 시 풀 비지 않음(≥8) · 전 id가 `playerMap`에서 해석(`evolveOnDay`≠undefined) · 표시 OVR 전원 [70,80] ·
    포지션 스프레드(5종 포함) · 결정론(같은 시드 = 같은 초기 FA) · 시즌0 재로드 후 풀 생존.
  - **GOLDEN 불변**: 기존 `_dv_golden` PASS(팀 로스터 경기 산출 무변) — initfa append가 팀 미배정이라 3픽스처 해시 불변으로 확인.
