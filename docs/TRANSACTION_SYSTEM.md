...
- **주의**: 현재 *팀 사기→경기력* 시스템은 **없다**(grep 확인). `ROTATION_MORALE`은 개인 *호감도→재계약 거부/FA 이탈*.
  → 매치 엔진에 새 사기 변수를 만들기보다(밸런스 위험), **기존 호감도 경로 재사용**: 충성·핵심 동료 방출 시
  남은 선수들 **팀 호감도 소폭 하락 → 재계약 거부·FA 이탈 위험↑**(파생·무저장). 매치 밸런스 불변.
  (사용자가 그린 "경기력 소폭↓"는 새 엔진 시스템이 필요해 후순위/별도 결정.)

> 순서: ①②(재정·감정, 밸런스 무관) 먼저 → ③(팬심, 측정) → ④(호감도, 측정). 각 단계 이 문서 갱신.
