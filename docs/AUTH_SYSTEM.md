...
### 3.5 로그인 이메일 수집 (2026-08-08, 사용자 결정)

> **왜**: 관리자 콘솔에서 **어느 행이 누구인지 알 수 없었다** — 가진 게 구글 `sub` 21자리 숫자뿐이라
> 문의·결제 대응 때 계정을 눈으로 특정할 방법이 없었다.

**변경 전 원칙과 그 정정** — 종전 설계는 "이메일·이름·프로필 사진을 받지 않는다"(최소수집)였고 처리방침에도
그렇게 **명시**돼 있었다. 사용자 결정으로 **이메일만** 수집으로 전환한다. 이름·프로필 사진은 계속 안 받는다.

**설계**
- 취득: 구글 `idToken`의 `email` 클레임 중 **`email_verified === true` 인 것만**.
  미검증 이메일은 소유가 확인되지 않아 식별 근거가 못 된다.
- 저장: `users.email`(nullable, Expand-only, 마이그레이션 `0007`).
- 갱신: **매 로그인**(`setUserEmail`). 이게 기존 계정에 소급해 채우는 유일한 경로이고, 계정 이메일 변경도 따라간다.
- **throw-none**: 이메일 저장이 실패해도 로그인은 성공한다. 식별 편의가 로그인을 막으면 손해가 더 크다(§13.22).
- 표시: 관리자 사용자 목록의 '계정' 칸. **이메일이 없으면 `provider + id 뒷 6자`** 로 폴백 —
  아직 안 채워진 계정도 최소한 **행 구분**은 돼야 한다.
- 파기: 탈퇴 시 비필수 개인정보로 **우선 파기**(§13.9).

**동반 개정(코드만 바꾸면 안 되는 부분)**
- 개인정보처리방침 §1①(수집 항목에 이메일 추가) · §2①(이용 목적) · §3⑤(파기) — 앱 `data/legalText.ts` +
  웹 `server/app/privacy/page.tsx` **양쪽**(1:1 미러 원칙). 최종 수정일 2026-08-08.
- **Play Console '데이터 보안' 설문의 수집 항목 갱신 — 사용자 작업(미완).** 스토어 심사 항목이라 빠뜨리면 리젝 사유가 된다.

**적용 범위(정직하게)** — 기존 3계정은 마이그레이션 직후 `email = NULL`이고, **각자 다음 로그인 때** 채워진다.
소급 백필 경로는 없다(우리가 가진 건 `sub`뿐이라 구글에 되물을 수 없다).
...
**✅ 실기기 E2E 검증 완료(2026-08-08, `SESSION_JWT_SECRET` 실제 회전으로)** — 개발자 본인 계정으로 전 구간 실측:
1. OTA 수신 후 **로그인 유지·지갑 동기화 정상** = 거짓 로그아웃 없음(가장 위험한 방향)
2. 회전 + 재배포 → 앱이 **스스로 로그인 화면으로** 전환 + 만료 안내 문구 노출
3. 재로그인 → **같은 userId로 복귀**(운영 DB 유저 수 3명 불변 — 새 계정 생성 0) · 잔액 보존 · 즉시 적립 동작

> 이 회전은 **180일 TTL 만료의 리허설**이기도 하다 — 2027년 초 7월 가입자들이 같은 경로를 탄다.
> **OTA는 콜드 스타트에서만 받는다**(`checkAutomatically: ON_ERROR_RECOVERY` + BootGate `otaAttempted` 생명주기 가드).
> 백그라운드 복귀로는 안 받으므로 "유저가 앱을 열면 곧 반영"이 아니라 **"완전 종료 후 재실행해야 반영"**이다.
> 다만 부팅 OTA 자동적용(§4.1) 덕에 **콜드 스타트 1회면 충분**하다(구 "2회 재시작" 아님).
