...
- **선수 상세**(`app/player/[id].tsx`): "친한/라이벌" 카드(`relationsOf`) ✅ + **이번 시즌 절친 방출 시 "💔 동요 중 — 재계약 거부 위험↑"**(Phase 3 friendLeave를 화면으로, 2026-06-26).
...
**후속(측정/Phase 후 결정)**:
4. **AI 재계약(aiRetainProb)에도 관계 반영**할지 — 1차는 FA 시장(league-wide)만, parity 측정 후 결정.
