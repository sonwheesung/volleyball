...
> **형제 드리프트 — ✅ 재측정 확정(2026-06-26)**: "OP 톱 ~5.3점/세트"는 **stale**. box 단일 진실 실측(N=10,000경기·
> 엔진 76c66ad): **팀별 OP톱 평균 3.26/세트**(~12.8/경기·~459/시즌)·리그 최고 OP 1인 3.70·OP 전체 평균 3.38·
> 외인 OP 2.40/세트. **어떤 "톱" 해석도 5.3에 못 미친다**(정의 무관 stale). 원인: 5.3은 **legacy 오버레이가
> `ATK_FOCUS=2.0`로 OP 1옵션 집중을 과장**한 값 — 실제 엔진 스윙(box)은 공격을 더 분산한다. **"OP 톱" 정의 확정**
> = 팀별 OP 득점 1위의 세트당 평균(가드 `_dv_drift_posrate`). 측정 `tools/_dv_drift_posrate.ts 10000`.
...
> **⚠ 아래 산식은 구(legacy) 초안 — 현행 아님(취소선 정정, 발견 모드 감사 2026-07-15).** 실제 구현은
> **★ 구현 현황 박스의 `marketValue`**(`engine/salary.ts`)다: `BASE × abilityMul × serviceFactor(age) ×
> POSITION_MUL × foreignMul × perfFactor × award`. 구 초안과의 실제 차이 —
> ① `(0.4+0.6×실적점수)` 실적 주동력 → **`abilityMul`(OVR 기반, 시대 앵커) 주동력 + `perfFactor`(±) 보정**,
> ② `ageEarnMul`(나이 구간 절벽) → **연속 `serviceFactor(age)`**(19:0.58→27:1.0→31:0.84, 절벽 없음),
> ③ `positionMul S1.15·OP1.2·OH1.05·MB0.9·L0.8` → **`POSITION_MUL S1.12·OP1.18·OH1.05·MB0.92·L0.82`**(`engine/salary.ts:61`),
> ④ 수상보너스는 **`award`(통산 수상 → 최대 +25%, `AWARD_BONUS=0.25`)** 항으로 별도 곱(추후→구현됨).
...
> **[무문서 결정 등재 — FA 계약 경로별 실적 반영 차 (발견 모드 감사 2026-07-15, WAI 서술)]**: `perfFactor`(시즌 실적)가
> 실제로 **계약에 실리는 경로는 내 팀 인시즌 재계약(오퍼 빌더, §2.5c — `getPlayerProduction(displayCutoff)` 프로덕션을 시장가에 전달)뿐**이다.
> 다른 두 경로는 코드상 prod를 안 넘겨 **`perfFactor=1.0`**(중립)으로 계약가가 산정된다:
> - **FA 요구연봉**(`data/offseason.ts:227 askingPrice(marketVal(p), grade)`): `marketVal(p)`가 prod 미전달 → `perfFactor 1.0`.
>   단 `marketVal`은 `awardScoreOf`를 항상 실으므로 **수상 프리미엄은 반영**(등급 프리미엄·평판 할인과 함께).
> - **오프시즌 자동갱신**(`engine/rollover.ts renewedContract` / `data/awardSalary.ts renewalVal` = `marketValue(p, era, undefined, 0)`):
>   prod 미전달(**perfFactor 1.0**) **AND** awardScore 0 → **시즌 실적·수상 프리미엄 둘 다 미반영**(잔류 확정가가 marketVal보다 보수적 — 코드 주석이 의도로 명기: MVP급 체계적 과대표시 방지).
> **OPEN(재고 여지)**: "그 시즌 잘한 선수가 FA/자동갱신에서도 더 받아야 하나?"는 열어 둔다 — 현 설계는 실적 변동성을 계약가에서 눌러
> 노장 과대·신인 저평가 서사(계약 고착 §3)를 지키는 쪽이다. 바꾸려면 asking/renew 경로에 prod 전달 + 캡·인플레 재측정(추정 금지).
...
- ~~**루키스케일**: 신인 1~2년차는 marketValue 무시, 낮은 고정 연봉 → 떡상해도 싸게 묶임(꿀).~~
  → **정정(발견 모드 감사 2026-07-15)**: 하드 루키캡(절벽) **제거됨** — 신인 저평가는 별도 고정 밴드가 아니라
  **`serviceFactor(age)` 연속 곡선**(≤19:0.58 → 27:1.0 점진, `engine/salary.ts:54`) 안에서 점진 할인된다. 어린 서명은
  serviceFactor가 낮아 싸게 묶이되(꿀 서사 유지) "1~2년차 marketValue 무시" 같은 절벽/무시 구간은 없다(22→23 점프 제거).
