...
> ⚠ **밸런스 함의(별도 결정 — 엔진 미변경)**: 실제 엔진의 OP 에이스 집중(~13/경기, 팀 자력득점의 ~20%)이 **KOVO
> 외인 아포짓(~20~25/경기, ~25~30%)보다 약하다.** box=게임 진실이므로 문서는 3.3으로 교정하되, "외인 에이스를 더
> 몰아줄지"(엔진 `pickAttacker`/공격 점유)는 별도 밸런스 결정 사안(추정·임의변경 금지 — 사용자 합의 후). FOREIGN_SYSTEM 형제 참조.
