...
> **현행(2026-07-08, EC-FN-03 이중과금 제거 + 다중유니버스 재보정)**: `simFinance` **100시즌×8유니버스 평균** — 잔고 **17.5억**·보전 **12%**·좌절 **27%**·파산0·✅ 건강.
>  base **243000→225000**. ⚠ 두 가지가 겹쳐 재보정: ① **FA 첫해 몸값 이중과금 제거**(EC-FN-03 — `faSpend += 영입 salary` 삭제, 몸값은 myPayroll 단일 채널) → 현금 드레인↓,
>  ② **260707 엔진튜닝(부상/피로교체·서브에이스 등) 누적 드리프트**로 수정 전에도 이미 9.1억→19.1억·좌절 27%→5%로 밴드 이탈(문서 stale). 두 요인 합산 → base −18000으로 밴드 복원.
>  ⚠ **단일 궤적 카오스**: cash→FA→로스터→성적→cash 되먹임으로 같은 base라도 왕조 눈덩이 유니버스가 잔고·좌절을 흔든다(base 비단조 — 233k 단일런이 왕조 45억 아웃라이어). `simFinance`를 **reseedLeague 다중유니버스 평균화**로 개편(옛 단일런 노이즈 극복).
> **모기업 베이스 22~30→25~33억(+3억, 06-27) → 24.3~32.3억(base 243000, 06-28) → 22.5~30.5억(base 225000, 07-08)** — 회귀 수정 + v2 재조율 + 이중과금 제거 재보정:
...
### 검증 가드 (커밋 전 전부 0)
- 재정 건강(simFinance): ~~현금부족 좌절 8~15%~~ → **옛 검증 healthy 수준 복원**(좌절 ~20-27%·보전 가끔·파산0). ~~단일유니버스라 좌절% 노이즈 큼(243k=27%·244k=33%·246k=17% 비단조) — 밴드로 판정.~~ → **정정(2026-07-08, EC-FN-03)**: 단일 궤적은 cash→FA→성적 되먹임으로 **카오스**(233k 단일런이 왕조 45억 아웃라이어)라 base가 비단조 — `simFinance`를 **다중유니버스 평균화**(reseedLeague U개)로 개편해 밴드를 안정 신호로 판정. 현행 base 225000 @ 100×8 = 좌절 27%·보전 12%·잔고 17.5억(income 드리프트로 잔고는 옛 9억보다 높음 — 좌절/보전이 설계-의미 지표). 주 압박은 캡, 현금은 그 위 + stance 변동.
...
|---|---|---|---|
| **모기업 지원금** | 팀별 베이스 ~~22~30억~~ ~~24.3~32.3억~~ **22.5~30.5억**(시드 결정론 — 모기업 크기가 다르다. base 225000, EC-FN-03 재보정 2026-07-08) | 수입의 ~60% | 안정 기반 |
...
- **몸값은 단일 채널 = `myPayroll`**(EC-FN-03, 2026-07-08 명문화): 영입 FA(국내·외인 공통)의 연봉은 **매 시즌 `settleSeason.payroll`(명단 전원 salary)** 이 전액 부과한다.
  - 오프시즌 현금 차감(`store.endSeason` `faSpend`)은 **FA 보상금(`compCash`, A/B) 전용** — 영입 FA의 첫해 몸값을 여기에 **더하지 않는다**.
    ~~예전엔 `faSpend += 영입 salary`도 차감했으나~~, 그러면 바로 다음 시즌 `myPayroll`이 같은 몸값을 또 부과 → **첫해 이중과금**(N년 계약이면 총 salary×(N+1)). 실측 국내 1.536×·외인 1.889×(`tools/simFaDoubleCharge`). 제거로 몸값=payroll 단일 채널.
  - 자금 게이트(첫해 연봉 ≤ 잔고, §3)는 *지출 한도 판정*이라 그대로 유지 — 차감이 아니다.
...
## 5. 검증 (simFinance — 다중유니버스 평균, `npx tsx tools/simFinance.ts [시즌=120] [유니버스=8]`)

- 잔고 궤적: 최저/최고/평균, 모기업 보전 발생 빈도(가끔이어야 — 매시즌이면 베이스 상향).
- **"캡 여유 있는데 자금 부족" 좌절률** — 이 연출이 실제로 발생하는가(0%면 의미 없음=장식화, 절반이면 과함). 설계 밴드 좌절 20-27%.
- 리그 건강: 내 팀 평균 순위·우승 분포 유지(재정이 내 팀만 제약하므로 parity 영향 미미 예상).
- **다중유니버스 평균화 필수(2026-07-08, EC-FN-03)**: 단일 궤적은 cash→FA→성적 되먹임 카오스(같은 base가 왕조 눈덩이 유니버스에선 좌절 0%, 제약 유니버스에선 50%+로 바이모달) → **U개 reseedLeague 평균**으로 밴드를 판정. 단일런 base 튜닝은 노이즈.
