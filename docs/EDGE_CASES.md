...
| EC-SUB-02 | **불법 재진입만** 차단 — 나간 선발이 같은 세트 **다른 슬롯** IN으로 재진입(FIVB 15.6.1 위반, iv1이 X를 뺀 뒤 iv2가 다른 선발 C 자리에 X를 IN)은 no-op. **정정(2026-07-30): 합법 재진입(X가 자기 슬롯·자기 교체선수와 교대, 유저 manual·세트당 1회)은 이제 허용** — ~~2026-07-15 F2가 `usedStarterOut` 무조건 차단이라 합법 복귀까지 통째 봉인(휴식 교체한 선발이 그 세트엔 자기 자리로도 못 돌아옴)~~ | ~~`subIn`에 `if (usedStarterOut[side].has(player.id)) return;`~~ → `isReentry = kind==='manual' && usedStarterOut.has(player) && activeSubs[slot]?.orig.id===player && !isRestorable(active.kind)` 계산 후 **`if (usedStarterOut.has(player) && !isReentry) return;`**(불법만 차단)·재진입 회계 분기(`activeSubs.delete(slot)`, usedSubIn/usedStarterOut 무추가). **AI(kind≠manual)는 재진입 불가 → 무개입 바이트 동일**(`engine/match.ts subIn`, 2026-07-30, ENGINE_VERSION 20) | `checkSubs`(불법 타슬롯 재진입 + 개입 주입 묶음 N≥200, AI 우주 0)·**`_dv_reentry`**(합법 재진입 A복귀·예산2·A코트복귀 + 불법 3종[타슬롯·B재진입·A재이탈] no-op + AI 격리 재진입0, A/B: 재진입 게이트 revert 시 합법 재진입 FAIL)·`_dv_ivbudget` (c)(후보 노출 ⇔ 엔진 수락/거절 정합) |
...
>
> **정정(2026-07-30, 합법 재진입 허용):** `reenterViolations`(enter.inId ∈ 나간 선발 집합)는 이제 **불법 재진입 탐지기**로만 유효하다 — 합법 재진입(A→B→A, manual)도 그 패턴을 만들기 때문. 그래서 `checkSubs`는 이 검사를 **AI 우주(무개입, 합법 재진입이 원천적으로 없음) + 불법 개입 주입 묶음(iv2가 타슬롯 IN 시도)** 에만 돌린다(둘 다 0이어야). 합법 재진입의 정상 동작 확인은 `_dv_reentry`가 별도로 담당(A 복귀·예산2·불법 3종 차단·AI 격리). 즉 "재진입 위반 0"의 우주는 개입 없는 경기 + 규칙위반 시도로 한정됐다.
...
| EC-UI-04 | **FA 센터 진입만으로 앱 종료 + 오프시즌 영구 소프트락**. 자금 부족 상태에서 FA 오퍼를 넣고 "영입 제안" 목록을 그리면 Render Error → 앱 프로세스 종료(SIGSEGV). 오퍼가 세이브에 남아 **재시작 후 FA 센터 재진입만으로 재크래시**, 아시아쿼터→FA 센터가 유일 전진로라 드래프트·다음 시즌으로 갈 방법이 없음(에뮬 1시즌 E2E, 2026-07-24) | `app/fa.tsx`가 `getTeam(lost)?.name ?? shortTeam(lost)`를 **`code==='LOST'` 분기 밖에서 무조건** 평가. `lostTo[id]`는 타 팀이 실제 계약했을 때만 채워져(`data/offseason.ts` :342·:372) CASH/CAP/ROSTER/SIT_OUT에선 `undefined` → `shortTeamName`이 `undefined.split(' ')` throw. **렌더 트리 throw = 화면이 아니라 앱이 죽는다.** → ① 배지 계산을 셀렉터 `data/faBadge.ts`로 분리(lostTo는 LOST 분기 안에서만) ② `shortTeamName` 하드닝(빈 입력 throw 금지, 정상 경로 바이트 동일) ③ 소스 회귀 봉인(FA_SYSTEM §2.8.10) | **`_dv_fabadge`**(실게이트 재현·자연런 1,784카드[크래시 조건 자연 발생 6건]·192조합·하드닝·소스회귀 + A/B 구로직 재주입 22 FAIL) |
| EC-UI-05 | **우편함 API 500인데 화면은 "우편이 없어요"** — 서버 장애가 "우편 없음"으로 위장돼 유저가 운영 보상을 못 받은 줄 모른다(에뮬 E2E `61_mailbox.png`, 2026-07-24) | `app/mailbox.tsx fetchList`의 else 분기가 offline이 아닌 모든 실패(500·401·bad-request)를 `setItems([])`로 뭉개 **빈 목록과 실패가 구분 불가**. → `loadError` 상태 신설: 배너(경고색 + "다시 시도")·빈 상태 문구·아이콘을 오류/오프라인/진짜 빈 목록 3갈래로 분기, 오류 중엔 받기 비활성(캐시 표시는 유지). MAILBOX §6.1 | 수동 — 서버 500 주입(가드 없음, §5 감시 대상) |
...
> EC-UI-04 발견 방법(왜 못 잡았나): **렌더 경로 throw 렌즈 부재** — 헤드리스 가드·시뮬은 데이터 정합만 보고 화면 렌더를 태우지 않는다. `_dv_fafail`이 CASH/CAP/ROSTER 코드가 **옳게 나오는지**는 검사했지만 "그 코드를 화면이 그릴 때 죽지 않는지"는 아무도 안 봤다(값 정합 ↔ 표시 가능성 사각). 또 타입상 `lostTo[id]: string`(noUncheckedIndexedAccess 미사용)이라 tsc도 침묵. 환원 = ① 표시 계산을 셀렉터로 내려 가드가 태울 수 있게 ② 렌더 경로 원시 함수는 **입력이 오염돼도 throw 금지**(앱 종료 비용이 표시 오류 비용보다 압도적) ③ 형제 사냥 렌즈 "옵셔널 값이 그 값을 보장하는 분기 **밖에서** 평가되는가"로 `shortTeamName`/`getTeam(...)?.name ?? id` 전 호출부(46곳) 훑음 — 나머지는 배열 원소·루프 키·guard 뒤라 잔존 0.
...
| EC-ACH-02 | **탭 빨간 점이 미수령을 못 잡음**(2026-08-08 운영 — 실유저가 다이아 못 받음) — EC-ACH-01을 고칠 때 **함수만** 고치고 배선을 접지 않아, 나중에 추가된 마이페이지 탭 빨간 점(`app/(tabs)/_layout.tsx`)이 raw `careerTotals`를 새로 먹임 → **시즌 하나 통째로** 점 0(실측 1경기 탭0/카드6, 36경기 탭0/카드9). 통산 업적은 첫 시즌에만 열려 신규 유저 디스커버리 0 | 입력 조립을 셀렉터 `data/achSelect`로 접고 5개 호출부 전부 경유. 탭은 **`'floor'`(시뮬 0회 하한 — `results` 직독, 고아 fixtureId 스킵)** + 화면·수령이 남긴 **비영속** 정확값 캐시(`achTotalsCache{teamId,season,cutoff}`). 탭에서 `achTotals` 직접 호출 금지(앱 루트 상시 마운트 = 동기 프리즈). rehydrate claim 시드는 raw→**exact 교정** | `_gt_achdot`(C1 거짓양성0·**C2 n≥1⇒점**·C3 오프시즌 일치·C4 고아키 내성·C5 claimed 정합·C6 캐시 신선/stale·C7 배선 정적강제) + `--mutant`/소스변이 2종 A/B + `_dv_arch [ach-eval]`(app에서 `evalAchievements` 금지) |
