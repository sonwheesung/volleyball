...
>   - ⚠ **7/17 stats_daily 행 일회성 수동 UPDATE 필요(메인 세션 수행 예정)**: 크론은 지난 데이터를 못 고칠 수 있음(그날 실구매 0이면 `pRows` 2일 윈도우에 그 날짜가 안 잡혀 upsert 스킵) → 이미 오염된 7/17 행은 코드 수정으로 자동 정정 안 됨. 7/17 실결제는 **전부 샌드박스**였으므로 `update stats_daily set purchase_count=0, diamonds_purchased=0 where day='2026-07-17'`(revenueKrw는 이미 0). **prod DB 직접 실행은 메인 세션**(에이전트는 prod 쓰기 금지).
