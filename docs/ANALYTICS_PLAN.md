...
## 6. 관리자 대시보드 11섹션 지표 명세 (🚧 지금-가능분 구현 2026-07-09 · 나머지 EAS 계측 후)

> **상태**: 사용자 확정(2026-07-09). **지금-가능분 구현 완료(2026-07-09)** — 11섹션 골격 재구성 + 원장/서버 파생 실데이터(①⑤⑥⑨⑩⑪). 나머지(②③④⑦⑧·외부-sync 조각)는 **EAS 계측(track()) 이후**(§2·5 — Expo Go서 네이티브 SDK 무동작).
> 서버 구현(롤업 테이블·집계 라우트·ops-9f3a2c 화면·CSV·이상징후 알림)의 정본은 **[BACKEND_SYSTEM §13.25](./BACKEND_SYSTEM.md#1325)**(구현 현황 표 포함) — 이 절은 **"무엇을 보여줄지(지표·출처·분류)"**, 그 절은 **"어떻게 서버가 모아 그리는지(스키마·라우트·화면)"**.
> **구현 매핑(§6.4 지금-가능분)**: ① 가입 추이(`series?metric=signups` 일/주/월)+목록+CSV · ⑤ 상품별 다이아 지급(`/api/admin/bm`, 원장 파생) · ⑥ 광고 시청(원장 reason='ad') · ⑨ 서버 머니패스 오류(`/api/admin/errors`=purchaseEvent 실패) · ⑩ 운영 알림 카드(stats.alerts) · ⑪ 메인 KPI 실값+외부-sync "—"배지. **placeholder**: ②③④⑦⑧ 골격 + ⑤ARPU/ARPPU·⑥eCPM·⑨Sentry/Crashlytics.
...
## 7. 운영 알림 셋업 — Discord 연동 (👤 사용자 직접, 셋업 예정)

> **왜 이 절**: §1 Tier A·§5 데이터흐름에서 "Discord Webhook = 1인 운영 알림"으로 정해뒀으나 **셋업 절차가 없었다**(SERVER_OPS §3.4가 이 문서를 가리킴). 아래는 **퇴근 후 직접 따라 하는 절차** — Sentry/Discord 대시보드는 계정 로그인이 필요해 Claude가 대신 못 한다.

### 7.0 현재 상태 (2026-07-24)
- **Sentry = 서버 오류만** 관측(`server/lib/observability.reportError` 56개 호출 지점). 앱 크래시는 **안 잡힌다**(→ Crashlytics 영역, §7.3).
- **환경 게이트 적용됨**(BACKEND §13.21-a): 운영 DSN은 `VERCEL_ENV=production|preview`에서만 활성. 로컬 dev는 비활성(`SENTRY_FORCE_LOCAL=1` 탈출구). → 2026-07-24 알림 폭주(로컬 검수 500이 운영 Sentry로 샘)를 봉인.
- **알림 채널 = 현재 이메일**. 운영 에러 실측 0건 확인(2026-07-24). Discord로 전환이 이 절의 목표.

### 7.1 방법 A (권장) — Sentry 공식 Discord 통합
서버 코드 변경 0. Sentry 대시보드에서만:
1. Sentry → **Settings → Integrations → Discord** → Add → 알림 받을 **서버·채널** 연결.
2. **Alerts → Create Alert Rule** → 액션 "Send notification to Discord".
3. **폭주 방지(필수)**:
   - 조건에 **`environment` is `production`** 필터(dev 소음 이중 차단 — 게이트가 이미 막지만 방어).
   - **"이슈가 처음 생겼을 때 + 급증(spike)할 때만"** → 같은 에러 100회가 알림 100개가 되지 않게.

### 7.2 방법 B (대안) — 서버에서 Discord 웹훅 직접 (🤖 Claude 구현 가능)
Sentry를 거치지 않고 **운영 에러를 게임 전용 Discord 채널로 바로**. §1 Tier A "지금 가능(서버 라우트에서 catch→POST)"의 구현체.
- 필요: Discord 채널 → 설정 → 연동 → **웹훅 URL** 생성(시크릿 → `DISCORD_WEBHOOK_URL` env, `.env.local`/Vercel).
- 구현: `reportError`(또는 그 옆)에서 `production`일 때만 웹훅 POST(임베드: where·message·userId 마스킹). **Sentry 게이트(`sentryEnabled`)와 같은 환경 판정 재사용** — 로컬 dev 미발송.
- **레이트리밋 필수**(봇 소음): 같은 `where`+메시지는 N분 dedup, 또는 첫 발생만. 안 하면 §7.4 `req.json` 소음이 채널을 도배.
- A와 **택일 권장**(중복 알림 회피). "우리 게임만의 에러 채널"을 원하면 B, 표준 관측이면 A.

### 7.3 앱 크래시(Crashlytics)는 별도 — EAS 빌드 후
Sentry는 서버 전용이라 **앱 크래시는 못 본다**(§4 KPI "Crash Free User"). Crashlytics는 네이티브 SDK라 **EAS 빌드 필요**(Expo Go 불가, §2 원칙 5). Crashlytics도 Discord 알림 가능하나 **EAS 계측 이후 작업**으로 미룬다. → 크래시까지 Discord로 받으려면 **Sentry(서버)+Crashlytics(앱) 두 채널**을 각각 연결.

### 7.4 함께 정리할 백로그 — `req.json()` 파싱 500 (봇 소음원)
`server/app/api/**` **19개 라우트**가 `await req.json()`을 try 안에서 파싱해, 깨진 바디(봇·스캐너)가 **400이어야 할 것을 500 + Sentry**로 낸다. 유저 버그 아님 — **운영 알림의 최대 잔여 소음원**. 공용 `parseJson(req)` 헬퍼(실패 시 400 `bad-request`, Sentry 미보고)로 스윕 권장. 파급 19파일이라 **별도 작업**(2026-07-24 Sentry 게이트 작업에서 발견·미조치). → Discord 전환 **전에** 정리하면 채널이 깨끗하다.
