...
> — 일정 탭에 `시즌 결산 하러 가기 →` 하나만 뜨고, 다녀오면(또는 `이 업무 건너뛰기 →`) 다음 업무 `외국인 트라이아웃 하러 가기 →`가
> 뜨는 식으로 1→5를 밟은 뒤 `새 시즌 시작하기 →`. 진행도는 새 영속 필드 **`offseasonStep`(store, forward-only — `ceremonyProgress` 미러,
> `endSeason`서 0 리셋)** 으로, `partialize`+`SAVE_DEFAULTS` 양쪽 등재(가드 `_dv_migrate`).
>
> **재정정(2026-07-28, 사용자 — "무조건 다 하게" · `이 업무 건너뛰기 →` 링크 제거)**: 순차 카드에서 **건너뛰기 링크를 뺐다.** 각 업무는
> `○○ 하러 가기 →`만 있고, 그 화면을 눌러 거쳐야 다음 업무가 뜬다(FA·드래프트를 실수로 지나치지 않게 강제).
>
> **재재정정(2026-07-28, 테스터 — "뒤로가기가 업무를 끝낸 걸로 처리됨")**: ~~`advance()`를 `openStep()`보다 먼저 호출~~ 하던 것이 문제였다 —
> `하러 가기`로 화면에 들어갔다가 **뒤로가기(하드웨어/헤더 `router.back`)로 나오면** 커서가 이미 전진해 있어 그 업무를 **안 했는데 건너뛴/끝난** 것처럼
> 보였다. → **커서 전진을 "버튼 누를 때"가 아니라 "업무를 명시적으로 마치고 나올 때"로 이동**: `하러 가기`는 `openStep(route)`만(전진 없음),
> 커서 전진은 **`useOffseasonExit`**(앞단 스텝 화면의 "일정으로/완료" 명시 나가기, 라우트→스텝 인덱스 매핑, `setOffseasonStep(idx+1)`)에서만.
> **뒤로가기는 `useOffseasonExit`를 안 타므로 커서 미전진 → 같은 업무 유지**(테스터가 원한 동작).
> **소프트락 안전판(크래시 통과)은 ErrorBoundary로 이전**: 앞단 5화면+`draft-live`은 `RouteErrorBoundary` 대신 **`OffseasonRouteErrorBoundary`**를
> 재노출 — "일정으로 돌아가기" 폴백이 `setOffseasonStep(offseasonStep+1)`로 **깨진 스텝을 통과**시킨다(진입 시 `offseasonStep`==그 스텝 인덱스라 +1=skip).
> 즉 정상 뒤로가기=재시도(같은 업무)·명시 완료=다음 업무·크래시 폴백=통과(소프트락 봉인, a04c0bc). 최종 버튼은 완료 게이트 없이 상시 도달(규칙 2).
> **트레이드오프**: forward-only 커서라 일정 탭에서 지나친 업무 재방문 불가(office 탭도 오프시즌 라우트 미노출). `preOffseasonSteps()`
> 라우트·순서·개수 불변, `_dv_hub`·`_dv_migrate` 그린.
