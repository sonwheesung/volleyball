...
> `endSeason`서 0 리셋)** 으로, `partialize`+`SAVE_DEFAULTS` 양쪽 등재(가드 `_dv_migrate`). **소프트락 불변식은 보존**: 각 단계마다
> `이 업무 건너뛰기 →`가 있어 **크래시하는 화면도 건너뛰고 최종 버튼(새 시즌 시작하기)에 도달** — 이것이 §5.6이 막으려던 소프트락의 실제
> 해소책이다(방사형 재접근이 아니라 **건너뛰기**가 안전판). 최종 버튼은 완료 게이트 없이 상시 도달(규칙 2). **트레이드오프(명시)**: 앞단을
> forward-only 커서로 바꾼 대가로 **일정 탭에서 지나친 업무의 재방문은 불가**하다("다음 경기"가 지난 경기를 재생 못 하는 것과 같은 결 —
> office 탭도 오프시즌 라우트를 노출하지 않음, 2026-07-28 확인). `preOffseasonSteps()` 라우트·순서·개수 불변, `_dv_hub`(방사형 데이터 계약)·`_dv_migrate`(필드) 그린.
