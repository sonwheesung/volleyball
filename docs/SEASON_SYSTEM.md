...
시즌 종료 → 다음 시즌 base를 만드는 **단일 합성 함수**. 단계:

```
1) buildDraftContext: 롤오버(evolve 164일+나이+1+경력+1) · 은퇴 · 경쟁 FA(영입/보상) · 순번 · 신인 클래스
2) resolveDraft:      내 위시리스트 + AI 니즈 기반 지명(순번 존중)
3) fillRosters:       클래스 소진 후 남은 공백 신인 자동 충원
3.5) 경기 생산 → 성장: leagueProduction(전체) → applyMatchXp 로 전 구단 적립  (experience.ts)
4) 이적자 clubTenure=0 (프랜차이즈 판정)
→ commitPlayerBase + commitRosters, 세이브 리셋(season+1, currentDay 0, results/거래 비움; archive 보존)
```
...
- 세이브(영속): `selectedTeamId, season, currentDay, results, 계약/방출 오버라이드,
  playerBase, rosters, FA/드래프트 선택, protectedIds, archive`.
...
| 이어하기(세이브/리하이드레이트) | ✅ |
| MVP·개인 타이틀·명예의전당/영구결번 | ❌ 미구현(통산 기록 적립 후) |
| 포스트시즌 직접 지휘 / 라인업 수동 개입 | ❌ 미구현(자동 완성 후 오버라이드로 개방 예정) |

> 검증: `npm test`(62) · `npx tsc --noEmit`(+ `-p tsconfig.test.json`) · `npx expo export --platform android`.
