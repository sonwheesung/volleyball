...
> `○○ 하러 가기 →`만 있고, 그 화면을 눌러 거쳐야 다음 업무가 뜬다(FA·드래프트를 실수로 지나치지 않게 강제). **소프트락 안전판은 건너뛰기가
> 아니라 `하러 가기`의 순서**로 이전됐다: `onPress={() => { advance(); openStep(route); }}` — `advance()`(forward-only 커서 전진, 영속)를
> `openStep()`보다 **먼저** 호출하므로 **대상 화면이 render throw해도 커서는 이미 다음으로 넘어가 있고**, ErrorBoundary(§5.6.3-5)가 일정으로
> 탈출시킨다 → 복귀하면 크래시 화면이 아니라 다음 업무가 뜬다. 즉 §5.6이 막으려던 소프트락(a04c0bc)은 **건너뛰기 버튼 없이도 보장**된다.
> 최종 버튼(새 시즌 시작하기)은 완료 게이트 없이 상시 도달(규칙 2). **트레이드오프(명시)**: forward-only 커서라 **일정 탭에서 지나친 업무의
> 재방문은 불가**("다음 경기"가 지난 경기를 재생 못 하는 것과 같은 결 — office 탭도 오프시즌 라우트를 노출하지 않음). `preOffseasonSteps()`
> 라우트·순서·개수 불변, `_dv_hub`(방사형 데이터 계약, 건너뛰기 미검사)·`_dv_migrate`(필드) 그린.
