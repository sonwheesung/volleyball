# 월드컵 참가 (World Cup) — 설계 문서 ~~(유료 DLC)~~ (무료 추가 콘텐츠)

> **★ 방향 정정(2026-08-03, 사용자 결정)**: 월드컵을 **유료 DLC → 무료 추가 콘텐츠**로 전환.
> 구매·엔타이틀먼트 게이트 없이 전원에게 제공(구현 시). 아래 본문의 "구매/DLC/미구매자" 서술은 이 정정에 종속된다 —
> 결정론 격리(epoch + forward-only + 비트동일)와 "콘텐츠지 파워 아님" 원칙은 **그대로 유지**(오히려 안티과금 강화: 수익 게이트가 없어 과금이 콘텐츠 접근을 전혀 가르지 않음).
> 기능 자체는 **여전히 미구현**(플래그 off) — 이번 정정은 문서 방향뿐이고 코드(`lib/iap`·`WORLDCUP_ENABLED`)는 손대지 않음.
...
> → **정정(2026-08-03)**: **무료 추가 콘텐츠**로 전원에게 이 레이어가 생긴다(구매 없음). 활성화 이전 시즌엔 0영향(epoch forward-only·비트단위 동일)이라는 결정론 격리는 유지 — "미구매자 0영향"이 아니라 "활성화 전 시즌 0영향"으로 읽는다.
...
- ~~**DLC = 콘텐츠(역사 레이어), 파워 아님.** 구매자는 "내 세계에 월드컵·국가대표·메달이 생기고, 국대 무대가 영입 시장의
  쇼윈도가 되는 것"을 산다.~~ → **정정(2026-08-03)**: **무료 콘텐츠(역사 레이어), 파워 아님.** 이제 사는 게 아니라 전원에게 "내 세계에 월드컵·국가대표·메달이 생기고,
  국대 무대가 영입 시장의 쇼윈도가 되는 것"이 열린다. 무료라 안티과금 기둥이 오히려 강화됨(콘텐츠=무료, 전력 무영향). 차출자 유무와 무관하게 의미를 갖는 이유가 여기 있다(아래 5장).
...
## 2. ~~DLC 게이팅~~ 활성화 게이팅 — 2필드(owned + epoch), forward-only

> **정정(2026-08-03)**: 무료 전환으로 `worldCupOwned`는 **"구매 자산"이 아니라 "무료라 항상 소유(=콘텐츠 노출 flag)"** 로 읽는다.
> 게이팅의 실질은 **활성화 epoch + forward-only**(중도 활성화가 과거 시즌을 재작성하지 않게)이며, 이 결정론 격리는 무료여도 그대로 필요하다.
> "미구매자 desync 차단"은 "비활성 시즌 desync 차단"으로 동형. 구현 시 owned 필드 유지 여부(항상 true라 생략 가능)는 착수 때 판단.
...
7. ~~**IAP 인프라(med, 달력 리스크)** — 비소모 1종은 서포터팩 IAP 토대가 깔리면 상품 등록+entitlement 매핑으로 축소. 코어 dev-day와 분리.~~ → **해소(2026-08-03, 무료 전환)**: 무료 콘텐츠라 IAP 상품 등록·entitlement 매핑 자체가 불필요 — 이 리스크는 사라짐. 노출은 flag로만.
...
## 변경 이력
- 2026-08-03 **무료 전환(사용자 결정)**: 월드컵을 **유료 DLC → 무료 추가 콘텐츠**로 방향 정정. 구매·엔타이틀먼트 게이트 없이 전원 제공(구현 시).
  결정론 격리(epoch+forward-only+비트동일)와 "콘텐츠지 파워 아님" 원칙은 유지 — 무료라 안티과금 기둥은 오히려 강화. 기능은 여전히 미구현(flag off), 코드 미변경(문서 방향만). IAP 인프라 리스크(§11.7) 해소.
