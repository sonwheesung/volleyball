...
> **⚠ 외인 생산 stale → box 재측정(2026-06-26, 엔진 76c66ad)**: 위 "702점/시즌·≈6/세트"는 **box 단일화(2026-06-24) 전
> legacy 귀속 오버레이값**. 게임 truth인 box 실측은 **외인 OP ~2.40점/세트(~9.4/경기·~338/시즌)** — 절반 수준
> (`tools/_dv_drift_posrate`·`_dv_op_interp`). 즉 **설계 목표(§6 "팀 득점 ~30%", KOVO 4.5~6/세트)에 현재 미달**(실측
> ~15~20%·2.4/세트). 원인은 엔진 `pickAttacker`가 OP 에이스에 KOVO만큼 안 몰아줌(SALARY §1.1 형제). **밸런스 결정
> 사안**(엔진 공격 점유 조정 vs 현 분산 수용) — 추정·임의변경 금지, 사용자 합의 후. 멸종·캡·풀오염 등 무결성은 무관(유지).
