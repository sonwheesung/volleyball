...
1. **Phase 1 = B+C (감정·사유)** — ✅ **구현(2026-06-22)**: `benchCauseOf`(사유 귀속)·`discontentOf` 사유×성격×기대치
   분기·`moodOf`(불만/무감정/긍정)·선수 상세 "지금 마음" 표시. 검증 `tools/simMood.ts`(5체크 PASS). 'rested'는 #3 전까지 휴면.
   **남은 것 → C.4 (누적 부정→호감도→재계약 거부→FA)**: 기존 refuseResignProb에 시즌 누적 정교화(다음).
