...
- `seenTips`는 `freshSave`(게임 초기화) **밖**에 둔다 → 초기화해도 유지(`onboarded`와 동일 패턴).
  - **코드 정합(2026-07-30 사용자)**: `resetSave`가 `seenTips: {}`로 **명시 초기화**하던 걸 제거 — 이 선언("초기화해도 유지")과 모순이었다.
    이제 **운영 "세이브 초기화"(설정)는 튜토리얼 보존**(구단만 바꾸는데 스포트라이트를 다시 보게 하지 않음). **dev "게임 초기화"(마이페이지)만**
    온보딩 완전 재현 위해 `resetSave()` 뒤 `resetTips()`를 **명시 호출**(운영 경로엔 없음). ⚠ 에뮬 미검증 — 초기화 후 스포트라이트 미재생 눈 확인 필요.
...
- ⚠ **스포일러 금지**: 두 스텝 body에 스코어·승패·세트 결과를 절대 넣지 않는다(결정론 결과 누출 방지).
- **스텝① 카피 정정(2026-07-18, 사용자)**: ~~제목 "📺 관전 모드" + "결과는 전력으로 이미 정해져 있어요 — 다시 봐도 같은 경기입니다"~~ →
  제목 **"📺 경기 보드"**, 결정론 단정 문구 제거. 사유: 내 팀 경기 직접 개입(교체·타임아웃, MATCH_INTERVENTION_SYSTEM)이 들어온 뒤로
  "결과가 정해져 있다·다시 봐도 같다"가 사용자 눈에 거짓이 됨(개입하면 결과가 바뀜). 새 body는 "지켜봐도 되고 직접 손댈 수도 있다"로
  관전+개입을 함께 안내(개입 버튼 위치·상세는 스텝② match.controls가 담당 — 중복 최소화).
