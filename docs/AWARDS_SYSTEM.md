...
- **배치**: `app/records-archive.tsx`(~~`app/(tabs)/history.tsx`~~ — §5 정정, 2026-06-30 마이페이지 허브 개편) **시상식 카드** 상단 중앙. 카드 배경/텍스트 색은 **정규 MVP 소속 구단**
  ...
  → **정정(2026-07-30, 아래 "잠정 시상 노출 폐지" 참조)**: 시상식 카드는 **시즌 종료 후에만** 뜬다(잠정 미노출). 뜨는 시즌은 그 시즌 MVP 팀색으로.

> **잠정 시상 노출 폐지(2026-07-30, 테스터 결정 — 기록 아카이브 시즌 탭)**: ~~정규리그가 1/3(팀당 `AWARD_MIN_GAMES`=12경기)
> 넘게 진행되면 **진행 중에도** 잠정 MVP·정규/챔프 MVP·신인상·기량발전상·부문 기록상·베스트7을 "(잠정)" 라벨로 미리 노출~~
> → **폐지: 시상식류(정규/챔프 MVP·신인상·기량발전상·부문 기록상·베스트7)는 진행 중엔 전부 숨기고 시즌 종료 후에만 노출한다.**
> 사유(테스터): "저건 시상식에 나와야 의미가 있다" — 시즌이 끝나기도 전에 수상자가 다 까발려지면 시상식의 무게가 사라진다.
> **진행 중 시즌 탭 = 안내 카드("시상은 시즌이 끝난 뒤 시상식에서 공개") + 순위표 + 개인 기록 리더보드(득점/블로킹/디그/어시 TOP5)만**.
> 개인 기록(리더보드)은 "상"이 아니라 진행 중 흐름 지표라 그대로 유지(부문 기록상 카드와 구분 — 부문 기록"상"은 시상식류라 숨김).
> 구현: `app/records-archive.tsx` `SeasonView` — 시상식 그룹 렌더 게이트를 `!provisional && aw && aw.mvp`로(진행 중=`provisional`이면 안내 카드).
> `AWARD_MIN_GAMES`·`awardsReady`·`gamesPlayed` 및 "아직 시즌 초반입니다" 분기 제거. 스텝퍼 태그도 `진행 중`(구 `진행 중 · 잠정`).
> 표시 전용·결정론 무관(archive엔 여전히 `SeasonAwards` 저장, 시즌 종료 시 노출).
...
- **수상자 카드 비주얼(A1, 2026-07-01 — "너무 밋밋")**: 상명+이름+팀명 텍스트만이라 단조로웠다 → 각 수상자 카드에
  **선수 실루엣(유니폼 일러스트 `LegendIllustration`, 구단색 `teamColors` + 선수 번호 `jerseyNumber`) + 구단 엠블럼
  ...
  → **시각 격상(2026-07-28, 테스터 "다른 상보다 빈약")**: 부문 기록왕 7종이 포스터를 받으며 베스트7 코트만 상대적으로 밋밋해져,
  `Best7Court`에 **금 월계관 크레스트 헤더("SEASON / 베스트 7") + SVG 코트**(깊이 그라디언트 바닥·네트/포스트·어택라인·중앙
  스포트라이트·금 프레임)를 입혀 시상식 무게를 줬다. **포스터화 아님**(7인 코트 유지) — 벡터만(AwardIllustration과 동일 결·테마 대응),
  실수상자 7명·"시즌 베스트7" 라벨·겹침 규칙(340px·3단 포메이션·SPOT 좌표)·**가짜드라마 금지(드림팀/최강라인업 X·랠리 애니 X) 전부 불변**.
  → **얼굴 전환(2026-07-28, 사용자 — 코보 인포그래픽처럼)**: 색 마커 → **선수 아바타(`PlayerAvatar`, 팀색 유니폼)**. 슬롯 = **소속팀(얼굴 위) → 얼굴+포지션 배지 → 이름(아래)**,
  **본인 팀이면 팀명·이름 강조색**(theme.accent)·아바타 링 강조. 얼굴 지름 AV=44로 팀+얼굴+이름 블록 ≤ 82px(행간 92px 내 겹침 없음, 목업 검증).
  7명 얼굴 시트는 `FaceSheetWarmup`(UI-45)로 오프스크린 프리워밍(늦은 디코드 방지). 실선수·실포지션만(가짜드라마 불변).
...
- **소속팀명 표기(2026-07-23 사용자 요청, 전 포스터 공통)**: MVP·챔프전 MVP·신인상·기량발전상·기록왕 **전부** 수상자 소속팀명을 표기(엠블럼만으론 부족).
  ...
  패널 세로 예산(풋노트 有 ~~12.89%h~~ 이름 축소 후 12.69%h ≤ 13.2)·타이틀 충돌 마진 전부 **불변**(adjustsFontSizeToFit은 폭에 맞춰 글리프만 축소, lineHeight 고정 → 라인박스 높이 무변). `teamName` 미지정 시 현행 렌더와 동일(무회귀 기본값).
...
- **내 팀 수상자 시각 구분(2026-07-23 사용자 요청, 전 포스터 공통)**: 수상자가 내 팀(`isMyTeam` = 수상자 소속 `teamId` === `myTeamId`, `myTeamId` null이면 false)이면 **포스터 자체**에 구분을 준다 —
  (1) 포지션 줄의 **팀명 텍스트**를 `tone.bright`+볼드(fontWeight 900, 위치 영문은 800 유지)로, (2) 포지션 줄 끝에 **"MY TEAM" 미니 칩**을 인라인(소형 대문자 · `tone.bright` 테두리·글자 · 반투명 흰 배경). 칩 박스 높이(폰트+수직 패딩+테두리, **전부 폭 파생**이라 폭 무관 비율)는 포지션 줄 lineHeight를 **절대 넘지 않는다**:
  칩 높이 스케일 = `0.013(폰트)×1.0(lh) + 2×0.001(padV) + 2×0.003(border)` = **0.021w** ≤ posEn 라인박스 `0.022×1.10w`(풋노트 有, 최소) = 0.0242w. ⇒ 행 높이 = max(posEn, 칩) = posEn 라인박스로 **불변** → **줄 수·줄 높이 불변 → 패널 세로 예산(§8 ~~12.89%h~~ 이름 축소 후 12.69%h)·타이틀 충돌 마진 전부 무변경**.
  행은 `flexDirection:'row'`로 재구성(팀명 텍스트 `flexShrink:1` + 칩 `flexShrink:0` 고정)해 긴 조합에서도 텍스트만 축소(adjustsFontSizeToFit 유지). `isMyTeam` false/미지정이면 **기존 단일 `Text` 렌더 그대로**(무회귀 — posRow·칩 미생성). 셀렉터는 이미 `isMine`(=수상자 소속 === `myTeamId`, null→false)을 조립하므로 **그 값을 컴포넌트 `isMyTeam` prop으로 전달**(신규 셀렉터 필드 불요). 3안 `StatLeadersPoster`·`DraftPoster`(항상 내 팀)는 무접촉.
...
- **이름 폰트 축소(2026-07-23 사용자 요청 — 갤럭시 실기기 이름 과대)**: 수상자 이름 폰트 스케일 ~~`f.name = w·0.056`~~ → ~~`w·0.050`~~ → **`w·0.044`**(2026-07-23 2차, 0.050에도 "아직 이름 크다" 재피드백 → 추가 −12%).
  `lineHeight` 배수(풋노트 有 1.10·無 1.12)는 유지 → 라인박스 비율만 소폭 줄고, 축소로 생긴 세로 여유는 그대로 마진 여유로 흡수(다른 요소 확대 없음 — 이름 단건).
  세로 예산 재측정(가드 실측): 풋노트 無 ~~12.74%h~~ → ~~12.24%h~~ → **12.19%h**, 풋노트 有(압축) ~~12.89%h~~ → ~~12.69%h~~ → **12.69%h**(nameCol이 ovrChip 6.23 아래로 더 내려가 headRow=ovrChip 지배 → 이름만 더 줄여도 有 예산 불변). 둘 다 예산 13.2%h 이내 유지.
  가드 미러(`CFG_NOFOOT`·`CFG_FOOT`의 `nameScale` 항 ~~0.050~~ → 0.044)를 동기 갱신, 레거시 A/B 구성(`CFG_FOOT_LEGACY`)만 `nameScale 0.056`로 역사 보존.
...
현재 시즌 실 MVP + 챔프전 MVP + **상별 톤 샘플(신인=블루·기량발전=~~퍼플~~ 오렌지 footnote `시즌 생산 ▲N`·기록왕=~~레드~~ 실버)** + **기록왕 3안 프로토타입(1안 채택 §8.1.1)** + 레이아웃 스트레스 샘플(긴 이름 OH·세터·**MIDDLE BLOCKER + 긴 팀명 폭 스트레스**) + **내 팀 수상자 샘플(`isMyTeam` — MY TEAM 칩·팀명 강조 + 긴 조합 칩 폭 스트레스)**.
...
- **검증**: tsc 0 · npm test 무회귀(연출 전용 — 엔진·store 무접촉) · 가드 `tools/_dv_award_poster.ts`(A/B 자가검증):
  ...
  ⑥ **내 팀 구분(2026-07-23)**: `isMine` 귀속(내 팀→true·타 팀→false·`myTeamId` null→false, `buildAwardPosterData`·`statLeaderPosterData` 양쪽) · **"MY TEAM" 칩 높이 산식**(칩 높이 스케일 0.021w ≤ posEn 라인박스 0.022×1.10w·0.022×1.15w — 줄 높이 불변 → 세로 예산 무변경).
  — 변이(공격↔디그 스왑·깨진 bright·mip 'full' 강제·구 풋노트 구성·**교체 규칙 제거**·**과대 칩 패딩**) 주입 시 각각 검출됨을 증명(허위 오라클 방지). teamName·칩 표기는 줄 수·줄 높이 불변이라 세로 예산 검사 무변경.
  · 실기기 육안(오버레이 좌표·좁은 기기 폭·엠블럼·톤 색·**팀명 표기 + MIDDLE BLOCKER×긴 팀명 폭 + 내 팀 칩·팀명 색**)은 메인(에뮬).
...
## 10. 수상 수치의 "기준 라벨" — 무엇의 수치인가 (2026-07-24, 사용자 결정 "라벨 붙이기")

**증상(에뮬 E2E 2026-07-24)**: 시즌 결산 → 우리 팀 수상에서 같은 선수가
`정규 MVP · 여미정 · 727` / `챔프전 MVP · 여미정 · 793` / `득점상 · 여미정 · 727`로 떴다.
세 수치는 **서로 다른 계산**인데 단위·기준이 없어 "같은 선수인데 숫자가 왜 다르지?"로 읽힌다(모순처럼 보임).

**원인 = `AwardWinner.value`가 상마다 다른 자(尺)라는 사실이 화면에 없다.** 코드 정본(`engine/awards.ts`) 대조:

| 상 | `value` 계산(코드 정본) | 성격 | 표시 기준 라벨 |
|---|---|---|---|
| 정규리그 MVP | `round(impactScore(prod) × teamWeight(rank))` | **지수**(팀 순위 가중이 곱해짐) | `공헌지수` + 각주("정규 MVP는 팀 순위 반영") |
| 챔피언결정전 MVP | `round(impactScore(prod))` — 가중 없음 | 지수(원값) | `공헌지수` |
| 신인상 | `round(impactScore(prod))` | 지수(원값) | `공헌지수` |
| 기량발전상 | `round(impactScore(올) − impactScore(전시즌))` | 지수의 **증가폭** | `증가폭` (숫자 앞 `▲`) |
| 부문 기록상 7종 | `ProdLine`의 **실카운트**(points/spikes/blocks/aces/digs/assists/receives) | **실적** | 단위 접미사(`842점`·`132킬`·`89블록`·`54개`) |
| 베스트7 | 슬롯별 대표 스탯 실카운트 | 실적 | (수치 미표시 — 코트 배치도) |
| 라운드 MVP | 구간 `impactScore` | 지수(원값) | (수치 미표시) |

> **정규 MVP ≠ 챔프전 MVP 숫자**가 정상이다 — 같은 선수라도 정규는 `×teamWeight`(1위 ×1.0 … 꼴찌 ×0.5)가 곱해져
> 더 작다(위 사례 793 × 0.916 ≈ 727). 이건 버그가 아니라 §1 "성적 없는 MVP 없다"의 설계 결과다.
> 그래서 라벨만으로는 부족하고 **각주 한 줄로 팀 순위 반영을 명시**한다.

### 10.1 표시 규약 (전 표면 공통)

- **지수형(개인상 4종)**: 값 셀을 2줄로 — 굵은 숫자 아래 10.5px 기준 라벨(`공헌지수` / 기량발전상은 `증가폭`).
- **실적형(부문 기록상)**: 한 줄, 숫자 + 작은 단위 접미사.
- **카드 각주(개인상 카드 하단, 1줄)**:
  `공헌지수 = 득점에 세트·디그를 더해 환산한 시즌 기여도. 정규 MVP는 여기에 팀 순위를 반영해 뽑고, 기량발전상은 지난 시즌 대비 증가폭입니다.`
- 계산·수상자 선정은 **일절 불변**(표시 레이어 전용). `AwardWinner`에 새 필드 없음 → 과거 세이브·archive 무영향.

### 10.2 용어 통일 — "생산" → **"공헌"**(사용자 노출 문구 한정)

`impactScore` 파생값의 사용자 노출 명칭을 **"공헌"**으로 통일한다(엔진 식별자 `impactScore`·설계 문서 용어 "생산 임팩트"는 유지 —
체젠→"체력재생", 노쇠→"기량 하락"과 같은 결).
- 근거: 정규 MVP value엔 팀 가중이 섞여 **순수 생산량이 아니다**. "생산 727"로 쓰면 727을 실제 생산량으로 오해하게 만들어
  이번 버그(챔프MVP 793과의 모순)를 재생산한다. "공헌(지수)"은 평가치 뉘앙스라 가중이 섞여도 거짓말이 아니다.
- ~~`app/awards-ceremony.tsx` 기량발전상 포스터 footnote `시즌 생산 ▲N`(§8·§9)~~ → **`공헌지수 ▲N`**(2026-07-24 정정 —
  같은 값이 화면마다 다른 이름으로 뜨는 드리프트 봉인). `app/awards-poster-preview.tsx` 목업 샘플도 동일 교체.

### 10.3 부문 단위 단일 출처 — `data/awards.ts TITLE_UNITS`

부문 기록상 수치의 단위가 **두 벌**로 갈라져 있었다(발견 2026-07-24):
`data/awardPoster.ts STAT_LEADER_META.unit`(득점 `점`·공격 `킬`·블로킹 `블록`·나머지 `개` — §8.1.1 정본, 포스터 footnote 출고 중)
vs `data/news.ts` / `app/news/[id].tsx` 로컬 `TITLE_UNIT`(공격·블로킹까지 전부 `개`).
- **정본 = §8.1.1 세트(`점/킬/블록/개`)** 로 통일하고, `data/awards.ts`에 `TITLE_UNITS`를 신설해 단일 출처로 삼는다
  (`TITLE_LABELS`가 라벨의 단일 출처인 것과 같은 결 — UI-3).
- 소비처: `STAT_LEADER_META`(포스터) · `app/season-recap-detail/[section].tsx` · `app/records-archive.tsx` · `app/news/[id].tsx`(수상 기록 행).
- **예외(무변경) — `data/news.ts` 기사 프로즈**: 헤드라인/본문의 `TITLE_KO`+단위는 게임 내 **언론 보이스**이고,
  §1 하단 "잔여(사용자 판단 대기) — 뉴스 저널리즘 보이스" 결정이 아직 열려 있다. 그 결정 전엔 건드리지 않는다.

### ★ 상비 가드
- `tools/_dv_awardvalue.ts`(신규): ①상별 `value` 산식 = 위 표와 일치(정규 MVP만 `×teamWeight`, 챔프MVP/신인=원 impact,
  기록상=해당 `ProdLine` 실카운트, 기량발전=Δ)를 **실 시즌 산출로 재계산 대조** ②`TITLE_UNITS` 단일 출처를 전 소비처가
  참조(로컬 사본 0 — 소스 정적 검사) ③표시 라벨 문자열이 화면 간 동일. A/B: 산식을 뒤집은 뮤턴트(가중 제거/필드 교차)에서 FAIL.
