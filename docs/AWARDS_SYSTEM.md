...
- **내 팀 수상자 시각 구분(2026-07-23 사용자 요청, 전 포스터 공통)**: 수상자가 내 팀(`isMyTeam` = 수상자 소속 `teamId` === `myTeamId`, `myTeamId` null이면 false)이면 **포스터 자체**에 구분을 준다 —
  ...
  칩 높이 스케일 = `0.013(폰트)×1.0(lh) + 2×0.001(padV) + 2×0.003(border)` = **0.021w** ≤ posEn 라인박스 `0.022×1.10w`(풋노트 有, 최소) = 0.0242w. ⇒ 행 높이 = max(posEn, 칩) = posEn 라인박스로 **불변** → **줄 수·줄 높이 불변 → 패널 세로 예산(§8 12.89%h)·타이틀 충돌 마진 전부 무변경**.
