...
- **이름 폰트 축소(2026-07-23 사용자 요청 — 갤럭시 실기기 이름 과대)**: 수상자 이름 폰트 스케일 ~~`f.name = w·0.056`~~ → **`w·0.050`**(≈−10.7%,
  깔끔한 스텝). `lineHeight` 배수(풋노트 有 1.10·無 1.12)는 유지 → 라인박스 비율만 소폭 줄고, 축소로 생긴 세로 여유는 그대로 마진 여유로 흡수(다른 요소 확대 없음 — 이름 단건).
  세로 예산 재계산: 풋노트 無 ~~12.74%h~~ → **12.24%h**(headRow=nameCol 6.27), 풋노트 有(압축) ~~12.89%h~~ → **12.69%h**(nameCol 5.94 < ovrChip 6.23 → headRow=ovrChip 지배로 전환).
  둘 다 예산 13.2%h 이내 유지. 가드 미러(`CFG_NOFOOT`·`CFG_FOOT`의 신설 `nameScale` 항 0.050)를 동기 갱신, 레거시 A/B 구성(`CFG_FOOT_LEGACY`)만 `nameScale 0.056`로 역사 보존.
