...
### 구분 탭 vs 통합 피드 — ~~권고: 통합 피드 + 종류 배지 + 필터 칩~~ → **정정(2026-07-08): 구분 탭** → **재정정(2026-08-01, 사용자 결정): 별도 메뉴·별도 화면**

- **채택안(재정정 2026-08-01, 사용자 결정)**: 패치노트와 개발자 노트를 **완전히 별개의 메뉴·화면**으로 분리한다.
  - **관리자 콘솔**: 운영 그룹 나비에 ~~"노트" 탭 1개~~ → **"패치노트"·"개발자노트" 탭 2개**(각자 자기 kind만
    목록·에디터, "＋ 새 글"은 그 kind로 **고정** 생성 — 종류 드롭다운 제거).
  - **앱(마이페이지)**: ~~"개발자 노트" 카드 1개(안에서 세그먼트 탭)~~ → **"패치노트"·"개발자 노트" LinkCard 2개**.
    각 카드가 `/devnotes?kind=patch|note`로 진입해 그 kind만 목록 표시(화면 안 세그먼트 탭 제거).
    **안읽음 배지도 kind별로 분리**(패치노트 카드·개발자 노트 카드 각각 자기 kind 미읽음 수).
- ~~구 채택안(2026-07-08): 화면 상단 세그먼트 탭 2개 — 한 화면 안에서 탭 전환, 마이페이지 진입 카드 1개, 탭 라벨 점(dot)~~
  → 재정정(2026-08-01): **한 화면 세그먼트 탭 → 별도 화면**, **마이페이지 1카드 → 2카드**, **탭 점 → 카드별 안읽음 배지**로 승격.
- **근거(사용자, 유지·강화)**: 두 콘텐츠는 소비 목적이 다르다 — 패치노트는 "무엇이 바뀌었나"를 찾아보는 **기록 조회**,
  개발자 노트는 "읽을거리". 섞으면 둘 다 흐려진다. 세그먼트 탭도 여전히 한 지붕 아래라 흐려짐 — **메뉴 자체를 갈라**
  §2의 "소비 목적 분리" 취지를 끝까지 밀어붙인 것.
- ~~통합 피드 근거(원안): 1인 개발이라 발행 빈도 낮음·시간순 연대기 경험·칩이 가벼움~~ — 기각(2026-07-08).
...
- `app/(tabs)/mypage.tsx`의 **"자주 보는 것" 그룹**에 ~~`LinkCard` 한 줄~~ → **`LinkCard` 2줄** 추가(정정 2026-08-01) — 공지사항 카드 바로 아래.
  - **패치노트**: 아이콘 `document-text-outline`, title "패치노트", sub "이번 버전에서 바뀐 것" → `/devnotes?kind=patch`.
  - **개발자 노트**: 아이콘 `sparkles-outline`, title "개발자 노트", sub "개발 이야기 · 다음 예고" → `/devnotes?kind=note`.
  - **안읽음 배지(kind별 분리)**: 각 카드 우측에 그 kind의 안읽음 개수 칩. 0이면 배지 없음.
    (공유 캐시 `devnotes.cache.v1`에서 kind로 필터해 각각 계산 — id 기반 `readDevnotes`는 공용.)
- 상세는 스택 화면 `app/devnotes.tsx`(kind별 목록, `?kind=` 파라미터) + `app/devnotes/[id].tsx`(상세, kind 무관 단일 글).
  기록(records-archive)처럼 **허브는 가볍게, 본문은 스택**으로 분리하는 마이페이지 관례를 따른다.

### 3.2 목록 / 상세

- **목록**: ~~상단 세그먼트 탭(패치노트 | 개발자 노트), 탭별 최신순 카드~~ → **정정(2026-08-01): 세그먼트 탭 제거.**
  화면은 `?kind=` 파라미터 하나의 kind만 최신순으로 나열(패치노트 화면 / 개발자 노트 화면 = 별개 진입).
  화면 제목도 kind별("패치노트" / "개발자 노트"). 각 카드 = 제목 +
  (패치노트면) 버전 태그 + 게시일 + ~~본문 미리보기 1~2줄~~ + 안읽음이면 점(dot).
  무파라미터 직접 진입(딥링크)은 `kind='patch'`로 폴백.
  → **정정(2026-07-17, 사용자 결정): 목록 카드에서 본문 미리보기 제거** — 목록은 제목·버전 태그·게시일·안읽음 점만,
  **본문은 상세 화면 전용**. 공지·패치노트·개발자 노트 3화면의 "목록=제목+등록일 → 행 클릭 → 상세(제목·등록일·본문)" 구조를 통일.
  (오프라인 캐시 §3.4는 그대로 — 목록 응답에 본문을 실어 캐시하되 목록 화면에선 안 그릴 뿐, 상세가 캐시에서 연다.)
...
### Phase 2 — 관리자 에디터(마크다운 작성/미리보기/게시) — ✅ 구현(2026-07-15)
- **작업**: `ops-9f3a2c/page.tsx` 운영 그룹에 "노트" 탭 + 목록 + 행 클릭 모달(마크다운 에디터 + 미리보기 + kind/appVersion + 게시 토글).
- **구현 노트**: 모달은 wide 분할 — 좌 textarea(마크다운) / 우 `<Markdown>` 미리보기(경량 렌더러: 제목#/##·리스트-·**굵게**·`코드`·[링크](url), dangerouslySetInnerHTML 미사용 XSS 안전).
  kind=patch면 appVersion 입력칸 노출·note면 숨김. 상태 select(초안/게시)로 게시 토글. 저장은 서버 응답 후 reload(낙관 반영 금지). 목록 배지: 종류(패치노트=ac·노트=mut)·상태(게시=gd·초안=wn).
- **통과 조건**:
...
1. ~~종류 확장 시 통합 피드 유지 vs 구분 탭 전환~~ → **해소(2026-07-08, 사용자 결정): 구분 탭 채택**(§2).
   잔여 질문만 남음: 향후 "이벤트 회고"·"로드맵" 등 종류가 늘면 탭을 추가할지 기존 탭에 그룹핑할지(종류 4+ 시 재검토).
2. ~~**bootstrap 합류 여부**~~ → **해소(2026-07-15, 구현): 별도 `/api/devnotes` 채택**(bootstrap 미합류 — 부팅 페이로드 비대화 방지).
   안읽음 배지는 마이페이지 진입 시 이 라우트로 계산. "부팅 시 배지 미리 알기"가 필요해지면 bootstrap에 개수만 얹는 절충안 재검토.
...
5. ~~**재게시 시 publishedAt**~~ → **해소(2026-07-15, 구현): 최초값 유지**(재게시 bump 없음 — 연대기 안정).
   PATCH 게시 토글 시 `publishedAt`이 **비어 있을 때만** 세팅하고, 이미 있으면 유지한다(published→draft→published 왕복해도 최초값).
   **재검토 여지 명기**: "이 글 다시 봐 주세요"식 의도적 상단 bump가 필요해지면 명시 파라미터(`bumpPublishedAt`)로 옵트인 추가 검토.
6. ~~**appVersion 강제 여부**~~ → **해소(2026-07-15, 구현): 필수**(권고안 채택 — 패치노트의 정의가 버전).
   `kind==='patch'`면 POST·PATCH 모두 appVersion trim 필수(없으면 400). 교차 검증: PATCH로 note→patch 전환 시에도 appVersion 없으면 거부.
   note는 appVersion 무시(null 저장). 클라(에디터)도 저장 버튼 게이트로 동일 강제.
...
```
| [DEVNOTES_SYSTEM](./DEVNOTES_SYSTEM.md) | **개발자 노트/패치노트**(관리자 에디터→서버 원격 콘텐츠, 앱 업데이트 없이 게시) — 구분 탭(패치노트|개발자 노트)·안읽음 배지(로컬)·오프라인 캐시·무푸시. 공지(차단성)와 역할 구분. 📋 설계만(미구현, 2026-07-08) | `server/db/schema.ts`(devnotes)·`server/app/api/{devnotes,admin/devnote}/route.ts`·`server/app/ops-9f3a2c/page.tsx`·`app/devnotes.tsx`·`lib/server.ts`·`store/useAuthStore.ts`(예정) |
```
