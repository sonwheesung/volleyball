...
### 구분 탭 vs 통합 피드 — ~~권고: 통합 피드 + 종류 배지 + 필터 칩~~ → **정정(2026-07-08, 사용자 결정): 구분 탭 채택**

- **채택안(사용자 결정)**: 화면 상단 **세그먼트 탭 2개 — "패치노트 | 개발자 노트"**. 각 탭은 자기 종류만
  최신순으로 나열한다(종류 배지·필터 칩 불필요 — 탭이 곧 구분). 기본 탭은 첫 번째(패치노트).
  안읽음은 탭 라벨에 점(dot)으로 표시해 어느 쪽에 새 글이 있는지 보이게 한다.
- **근거(사용자)**: 두 콘텐츠는 소비 목적이 다르다 — 패치노트는 "무엇이 바뀌었나"를 찾아보는 **기록 조회**,
  개발자 노트는 "읽을거리". 섞으면 둘 다 흐려진다.
- ~~통합 피드 근거(원안): 1인 개발이라 발행 빈도 낮음·시간순 연대기 경험·칩이 가벼움~~ — 기각(2026-07-08).
  발행량이 늘어 탭이 비좁아지면 그때 탭 내 구조만 재검토.
...
- **목록**: 상단 세그먼트 탭(§2 — 패치노트 | 개발자 노트), 탭별 최신순 카드. 각 카드 = 제목 +
