...
| **유망주 데뷔전** | debut | **유망주(talentBase A급↑, ≥1.12 상위 15%)** 신인 + **나이 ≤23(`ROOKIE_DEBUT_MAX_AGE`, 2026-07-29)** 의 **첫 선발 출전**(서브교체·가비지 제외, 팀 무관) + **포지션별 대표 스탯**(리베로=디그·리시브, 세터=세트, 그 외=득점 — 리베로 득점 0은 정상이라 "0점" 표기 금지) | S급(≥1.25, 특급 기대주)=★ | 경기별 선발(`splitLineup`) + ProdLine + `player.talentBase`·`age` |
| **한 경기 폭발(커리어하이)** | match | 한 경기 개인 득점이 임계 이상(예: 30+) 또는 자기 최고 경신 | 35+=★ | 경기별 ProdLine + **그 경기 승패**(`seasonResults` 룩업, 2026-07-30) |
...
> **데뷔전 나이 상한(2026-07-29, 사용자 보고)**: 데뷔 게이트에 `p.age <= ROOKIE_DEBUT_MAX_AGE`(=23) 추가. **증상**: 게임 시작
> 첫 시즌엔 전 선수 `career.matches=0`(리그를 막 시드)이라 **베테랑(26~32세)** 도 첫 선발 시 "데뷔"로 잡혀, talentBase만 높으면
> "유망주/특급 기대주 데뷔전"으로 기사화됐다(스샷: 26세 리베로를 "S급 재능…팀의 미래를 책임질 재목"). **원인**: 게이트에 나이 조건
> 부재 — talentBase(재능)만 봄. **수정**: 실 KOVO 신인 데뷔 18~22(늦어도 23) 근거로 나이 상한. 실 신인(`makeProspect` age 18~20)은
> 항상 통과 → **후속 시즌 진짜 신인 데뷔 기사는 그대로**, 첫시즌 베테랑 아티팩트만 차단. 실측(fresh 리그): 데뷔기사 12건 중 age≥24
> **10건 제거**(31·32세 포함) → age 22·23 **2건만** 잔존. 가드 `_ev_debutgate`(나이 누수 0 + A/B: 재능-only 구게이트가 베테랑 10건 통과 실증).
...
4. **언론 예상 순위(`kind='standing'` 재사용) — 개막 프리뷰 1건(STAFF §9.3, 2026-07-20)**: 전력 기반 결정론 예상 순위(`data/reputation`
   ...
   스포일러 무저촉, ~~`day=0`~~ **`day=−1`(개막 전 소식, 2026-07-29)**·`season=currentSeason`. 예상 순위 원본은 영속 `mediaPredictionLog`(STAFF §9.6-B) —
   `buildNewsFeed` 15번째 인자로 주입(기본 [] → 미주입 시 기사 생략, 기존 호출 무변).
   - **정렬 위치 정정(2026-07-29, 사용자 보고)**: ~~day=0로 개막 최상단~~ → **day=−1로 개막일(day=0) 경기·데뷔 뉴스 아래**. **증상**: ★ big
     프리시즌 예상순위가 경기(데뷔·폭발) 뉴스 **상단에 고정** — 테스터: "경기 후엔 아래로 가라앉아야(오래된 개막 전 소식)". **원인**: 최신순
     정렬 `day↓·big↓`에서 preseason은 `day=0`·`big=true`라 개막일(day=0) 동률에서 **비-big 데뷔 뉴스 위로 부상**. **수정**: 프리시즌 예상은
     개막 **전**(day 0 이전) 소식이므로 `day=−1` 부여 → day 축에서 모든 day≥0 인게임 뉴스 아래로 자연 하강. `big=true`(★·강조색)는 유지 —
     **순서만** 내린다. 이 뉴스만 이동(다른 뉴스 상대순서 무회귀), `freshNews` 2주 만료 그대로. 가드 `_dv_newsorder`(정렬 검사 + A/B: day=0 되돌리면 재부상).
...
- **트리플 크라운·한 경기 폭발 — 선수당 1건 묶음**: 강한 선수가 한 시즌 트리플을 여러 번 치면 기사도 N건 쏟아졌다
  ...
  - **한 경기 폭발 승패 인지(2026-07-30, 테스터 버그)**: ~~biggame 기사는 승패를 모른 채 항상 승리 톤 풀(`POOLS.biggame`)만 사용~~
    → 그 경기 **승패를 `seasonResults(leagueDay)` 룩업**(day+team → 세트 우열, `seasonMatchProds`와 같은 컷오프)해서
    이겼으면 `biggame`(승리 톤), 졌으면 신설 `biggameLoss`(패배 톤 — "팀은 졌지만…"·"승리로 잇지 못했다")로 분기.
    중간 코어 문장도 승패 인지(승="팀 공격을 통째로 짊어진 하루" / 패="팀은 아쉽게 졌지만, 공격만은 오롯이 이 선수의 몫").
    증상: 대전 2:3 패배인데 33점 기사가 "한 경기를 통째로 끌고 갔다/에이스의 무게를 증명"으로 이긴 것처럼 읽힘(결과-blind 승리 톤).
    **사실 절(헤드라인·득점·스탯 나열)·seed key(`bg:${id}`)·important(35+)는 불변**(읽음추적·박스 정합 보존, newsKey는 kord 기반이라 문구 무관).
    룩업 미스 시 `?? true`(기존 승리 톤) 폴백 — 회귀 안전. 가짜 드라마 금지 유지(실제로 졌을 때만 패배 톤). 회귀 가드 `_dv_bignews.ts`(A/B won flip).
    형제(트리플·데뷔)는 카피가 중립("KOVO 기록에 이름"·"첫 선발 무대")이라 승리 주장 없음 → 변경 불필요.
