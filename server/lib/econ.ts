...
export const MAIL_PASS_EXPIRE_DAYS = 60; // 패스 첨부 우편 보존(R3 상향) — 수령 후 최대 56일 큐 점유 > 30일 보존이면 "대기 중 만료" 모순 창 → 60일로 해소
export const MAIL_MAX_GRANT = 10000;     // 우편 1통당 다이아 상한 캡(관리자 오발송 blast-radius 바운드)
export const MAIL_PURGE_GRACE_DAYS = 30; // 만료·회수 후 크론 물리삭제 유예(Q1) — 만료+30일 경과분만 삭제(원장 보존, 우편 메타만)
// reason 'mail'은 서버 권위 금액이 mails.attach_amount(발송 시 캡 검증) — earnAmount() 화이트리스트 밖(전용 claim 라우트)
